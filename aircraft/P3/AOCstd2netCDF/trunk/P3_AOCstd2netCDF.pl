...
# Sept 2003
# This code was originally written for EPIC, but was then used for SALLJEX, 
# BAMEX, and NAME in it's original form (Version 1.1)
#
# Note: This code attemps to follow the 
# <a href="http://raf.atd.ucar.edu/Software/netCDF.html">
# NCAR-RAF netCDF Conventions</a>
#  However, when these conventions contradict the more recent 
# <a href="http://www.unidata.ucar.edu/staff/russ/netcdf/BestPractices.html"> 
# Writing NetCDF Files: Best Practices</a> 
# document, the Best Practices document takes precedent.
#
#       Version 1.2, Last Updated June 13, 2008
#
# Changes from Version 1.1 used for SALLJEX to Version 1.1 used for BAMEX:
...
# Changed from Version 1.1 used for BAMEX to Version 1.1 used for NAME: None.
# 
# Version 1.2: Moved constants to be command line options so don't have to keep
# 	modifying code for each project. Updated to gap-fill missing times so 
#	ncplot and n2asc will work correctly. Tested with ncplot and n2asc and they
#	both work!
################################################################################
...
#-------------------------------------------------------------
# THESE CONSTANTS MUST BE SET FOR EACH CENTURY! (-:
#-------------------------------------------------------------
$century=2000;
...
if ($#ARGV <2)
  {
  print "Usage: P3_AOCstd2netCDF.pl <raw_file> <project> <project_number>\n";
  print "\te.g. P3_AOCstd2netCDF.pl 20050705I1.std RAINEX 304\n";
  print "This code must be run on tornado!\n";
  exit 1;
  }
...
print "WARNING: This code must be run on a solaris machine\n";
print "\t (cabernet, tornado) or the byte order/values will be wrong\n";
...
# Convert the 1-digit year to a 4-digit year
   ...
   print "Year is ${$records{$type}}[4]\n";
#} else {
#   print "ERROR: Don't know what to add to year ${$records{$type}}[4] to get 4-digit year\n";
#   exit(1);
#}
...
while ($bytes_read != 0) {
   ...
   if ($param[1] == 3) {
      ...
      $flight_length  = ${$records{$type}}[4] + ${$records{$type}}[5]+$gapfill;
...
sub calc_basetime {
  ...
  print "Flight began on ".gmtime($basetime)." UTC so basetime = ".sprintf("%s",$basetime)."\n";
}
#---------------------------------------------------------------------
sub set_global_att {
...
sub put_fillval {
#
# Put a fill value into a previously defined variable
#
   my $id = shift;
   my $name = shift;
   my $time = shift;

   if (NetCDF::varput($ncid, $id, $time, 1, -32767.))
...
sub set_global_attributes {
   &set_global_att("Source","UCAR/NCAR Earth Observing Laboratory");
...
sub put_type_5 {
      ...
      # Correlate each variable id with a name for human identification
      # Note that time offset is not in this list - special case
      my %vars = ($timeerrid=>"TIME_ERR", $laterrid=>"LAT_ERR", $lonerrid=>"LON_ERR",
               $alterrid=>"Ralt_ERR", $preserrid=>"PS_ERR", $temperrid=>"TA_ERR",
               $dewpterrid=>"TW1_ERR", $rderrid=>"RD_ERR", $rserrid=>"RS_ERR",
	       $gserrid=>"GS_ERR", $tserrid=>"TS_ERR", $wgserrid=>"WGS_ERR",
	       $tkerrid=>"TK_ERR", $hderrid=>"HD_ERR", $pcerrid=>"PC_ERR",
	       $rlerrid=>"RL_ERR", $aaerrid=>"AA_ERR", $saerrid=>"SA_ERR",
	       $lwerrid=>"LW_ERR", $pqerrid=>"PQ_ERR", $latid=>"LAT", $lonid=>"LON",
               $altid=>"Ralt", $presid=>"PS", $tempid=>"TA", $dewptid=>"TW1",
               $rdid=>"RD", $rsid=>"RS", $gsid=>"GS", $tsid=>"TS", $wgsid=>"WGS", 
               $tkid=>"TK", $hdid=>"HD", $pcid=>"PC", $rlid=>"RL", $aaid=>"AA", 
	       $said=>"SA", $lwid=>"LW", $pqid=>"PQ", $tdid=>"TD", $ruid=>"RU", 
	       $sw1id=>"SW1", $sw2id=>"SW2", $sw3id=>"SW3", $utailid=>"UTAIL", 
	       $vtailid=>"VTAIL", $wtailid=>"WTAIL", $gaid=>"GA", $paltid=>"PALT", 
	       $dvid=>"DV", $htid=>"HT", $spid=>"SP", $rhid=>"RH", $tvid=>"TV", 
	       $wasid=>"WAS", $gmid=>"GM", $amaid=>"AMA", $daid=>"DA", 
	       $gsxid=>"GSX", $gsyid=>"GSY", $txid=>"TX", $tyid=>"TY", 
	       $wxid=>"WX", $wyid=>"WY", $wzid=>"WZ", $wsid=>"WS", $wdid=>"WD", 
	       $ewid=>"EW", $eeid=>"EE", $mrid=>"MR", $ptid=>"PT", $etid=>"ET", 
	       $wxbid=>"WXB", $wybid=>"WYB", $wsbid=>"WSB", $wdbid=>"WDB", 
	       $av1id=>"AV1", $av2id=>"AV2", $wacid=>"WAC", $bt1id=>"BT1", 
	       $bt2id=>"BT2", $bt3id=>"BT3", $navid=>"NAV", $itmpid=>"ITMP", 
	       $kingid=>"KING", $vgsid=>"VGS", $vasid=>"VAS", $vwid=>"VW");

      # Time should increment 1 second at a time.  If not, warn user and fill gap.
      my $gap = 0;
      if ($time_index > 0 && 
	 ...
	 $gap = $timeoffset[$time_index] - $timeoffset[$time_index-1] - 1;
	 print "Filling time gap of $gap seconds\n";
	 my $savetime = $timeoffset[$time_index];
	 $timeoffset[$time_index] = $timeoffset[$time_index-1]+1;
	 for ($i = 0; $i < $gap; $i++) {
	     #print "Adding data for timeoffset ".sprintf($timeoffset[$time_index])."\n";
	     #print "Index = ".sprintf($time_index)."\n";
	     if (NetCDF::varput($ncid, $timeoffsetid, $time_index, 1, $timeoffset[$time_index]))
	         {die "Couldn't put time offset value into data\n";}
             ($fillsec, $fillmin, $fillhour, $fillday, $fillmonth, $fillyear,$wdady,$yday,$isdst)=
	     gmtime($basetime+$timeoffset[$time_index]);
	     $fillyear = $fillyear + 1900;
	     $fillmonth = $fillmonth + 1;
	     print "Fill with ".sprintf("%s",$fillyear)."/".sprintf($fillmonth)."/".
	     	sprintf($fillday)." ".sprintf($fillhour).":".sprintf($fillmin).":".
		sprintf($fillsec)."\n";
             if (NetCDF::varput($ncid, $yearid, $time_index, 1, $fillyear))
                {die "Couldn't put year value into data\n";}
             if (NetCDF::varput($ncid, $monid, $time_index, 1, $fillmonth))
                {die "Couldn't put month value into data\n";}
             if (NetCDF::varput($ncid, $dayid, $time_index, 1, $fillday))
                {die "Couldn't put day value into data\n";}
             if (NetCDF::varput($ncid, $hourid, $time_index, 1, $fillhour))
                {die "Couldn't put hour value into data\n";}
             if (NetCDF::varput($ncid, $minid, $time_index, 1, $fillmin))
                {die "Couldn't put min value into data\n";}
             if (NetCDF::varput($ncid, $secid, $time_index, 1, $fillsec))
                {die "Couldn't put sec value into data\n";}
             foreach my $id (keys %vars) {
	         my $name = $vars{$id};
                 &put_fillval($id,$name,$time_index);
             }
	     $time_index++;
	     $timeoffset[$time_index] = $timeoffset[$time_index-1]+1;
         }
	 $timeoffset[$time_index] = $savetime;
      }
      if (NetCDF::varput($ncid, $timeoffsetid, $time_index, 1, $timeoffset[$time_index]))
         {die "Couldn't put time offset value into data\n";}
      ...
      $gapfill += $gap;
      $total_count = ${$records{$type}}[6] + ${$records{$type}}[7] + $gapfill;
      ...
      if ($debug2) {if ($time_index == 600) {last;} }
}
#---------------------------------------------------------------------
sub increment_date {
