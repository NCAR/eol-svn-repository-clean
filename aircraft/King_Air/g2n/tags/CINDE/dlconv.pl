...
while (my $line = <FILE>) {
	...
	if ($#split >= 2) { $subdir = $split[2] }

	my @split2 = split('/', $NCfile);
	#If there are duplicates like this then..
	if ($#split2 > 0) { $subdir = $split2[0]; $NCfile = $split2[-1];}
	$GPfile .= "." . $split2[-1];
	...
	#CONVERT!
	print "Converting..\n";
	system("g2n", "temp", "$Bin/$subdir/$NCfile.nc");
	system("rm", "-f", "temp");
	print "\n";
