...
sub main {
	...
	(my $type, my $timestep, my $flight) = parse_filename($file,\%metadata);
	    ...
	    if (${$metadata{comment}}[$i] !~ /CELLSIZES/)
	    {
              # Don't write CELLSIZES to globalatts file, as we don't want
	      # them to be written as global in netCDF file.
	...
	print "*"x80; print "\n";
        if ($metadata{filepattern} =~ /CASF/) {
	    # Make histograms (vector vars) from CASF files for use with
	    # ncpp.
            print "asc2cdf -h -r $metadata{timeStep} -g $file.globalatts ".
              "-a $outfile.ames $outfile.nc\n";
	    print "*"x80; print "\n";
            system("asc2cdf -h -r $metadata{timeStep} -g $file.globalatts ".
	      "-a $outfile.ames $outfile.nc");
	    system("ncrename -v CAS,CCAPS_CAS $outfile.nc");
	    system("ncatted -a units,CCAPS_CAS,m,c,'#/cc' $outfile.nc");
	    system("ncfillvar -v ACAPS_CAS -c CCAPS_CAS $outfile.nc");
	    system("ncatted -a units,ACAPS_CAS,m,c,'count' $outfile.nc");
	    system("ncatted -a long_name,ACAPS_CAS,c,c,'Dummy var so ncpp will work: Fwd(F)-scatter(CAS) count CAPS probe (CIRPAS)' $outfile.nc");
	} else {
...
##### This is a kludge. Change to READ from the config file later!!! #####
    ...
    $meta->{comment} = ["LOCATION = ","PLATFORM = ", "DATA VERSION = ", "REMARKS = ","CELLSIZES = "];
...
} elsif ($metadata->{filepattern} =~ /CASF/) {
    $metadata->{varlist} = {
	'CONC CH1'  => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH2'  => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH3'  => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC Ch4'  => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH5'  => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH6'  => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH7'  => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH8'  => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH9'  => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH10' => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH11' => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH12' => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC Ch13' => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH14' => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH15' => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH16' => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH17' => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH18' => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH19' => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
	'CONC CH20' => ['CAS','CONC (dN/CC)','Fwd(F)-scatter(CAS) size dist CAPS probe (CIRPAS)'],
    };
} else {
    print "Please define varlist for $metadata->{filepattern} in hashdef subroutine.\n";
    exit(1);
}
