...
foreach $variable (sort keys %var) {
   if ($variable ne "base_time" && $variable ne "time_offset") {
       #print "$variable\n";
      if ($var->{$variable}{ndims} == 3) { 
          $varid->{$variable} = 
	    NetCDF::vardef($ncidOut, $variable, NetCDF::FLOAT,
		[$id->{"Time"},$id->{$var->{$variable}{dimname}[2]}]);
      } else {
          $varid->{$variable} = 
	    NetCDF::vardef($ncidOut, $variable, NetCDF::FLOAT, [$id->{"Time"}]);
      }
   }
...
foreach $variable (sort keys %var) {
   ...
   foreach (my $record=0; $record < $recdimLen; $record++) {
	  ...
	  # Read in data that has (Time, sps10)
          ...
          $dim3Len = $var->{$variable}{$var->{$variable}{dimname}[2]};
	  #print "Dims: $record $dim2Len $dim3Len\n";
          @count = (1,$dim2Len,$dim3Len);
          @values = (); 
          NetCDF::varget($ncid,$var->{$variable}{varid},\@start,\@count,\@values);
	  # Write it out as 2-D data - Time, Vector
	  #print "@values\n";
          @start=($record*$dim2Len,0);
          @count = ($dim2Len,$dim3Len);
	  #print "[@start],[@count]\n";
	  #print $variable."\n";
	  #print $varid->{$variable}."\n";
	  ...
	  print "ERROR: Variable $variable has $var->{$variable}{ndims} \n";
	  print "dimensions. Update code.\n";
	  exit(1);
      }
   }
   }
