...
package ISS_SFC_Converter;

##Module-------------------------------------------------------------------
# <p>The ISS_SFC_Converter converts the netCDF data from the EOL ISS
...
##-------------------------------------------------------------------------
sub main {
    my $converter = ISS_SFC_Converter->new();
    $converter->convert();
...
##-------------------------------------------------------------------------
sub load_station {
    my ($self,$station_id,$latitude,$longitude,$elevation) = @_;
    ...
    if (!defined($station)) {
      ...
      $station->setStationName("ISS Station ".$station_id);
...
#    }
    ...
    my $station = $self->load_station($station_id,
				      $var->{"lat"}{"values"}[0],
				      $var->{"lon"}{"values"}[0],
				      $var->{"alt"}{"values"}[0]);

    if (!defined($self->{"accumulators"}->{$station_id})) {
      $self->{"accumulators"}->{$station_id} = 
	...
	if (!defined($missing) || !is_missing($value,$missing)) {
	  ...
	  if ($field eq "rain") {
	    ...
	    $self->{"accumulators"}->{$station_id}->addPrecip($record->getActualDate(),"YYYY/MM/DD",
