...
if ((outfp=fopen("station.array", "w")) == NULL) {
  fprintf(stderr,"snm: error opening station array output file\n");
  exit(1);
}
fprintf(outfp, "The array of stations in memory:\n\n"); 
...
/* 19940401 through 19940831 is used */
  ...
  /* only take data from stations active during the total period */
...
/*	printf("%s %s %s - %ld %ld\n", 
		stnray[l].stnam, tmplat_indegrees, tmplon_indegrees, stnray[l].lat, stnray[l].lon);
*/  
	if (((stnray[l].lat >= 3000L) && (stnray[l].lat <= 4500L)) &&
	    ((stnray[l].lon >= -10900L) && (stnray[l].lon <= -8500L))) {
		fprintf(outfp, "%s %s %s : %ld %.5ld\n", 
			stnray[l].stnam, tmplat_indegrees, tmplon_indegrees, stnray[l].lat, stnray[l].lon);
    }
