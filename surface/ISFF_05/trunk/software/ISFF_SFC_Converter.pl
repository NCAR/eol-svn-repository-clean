...
package ISFF_SFC_Converter;

##Module-------------------------------------------------------------------
# <p>The ISFF_SFC_Converter converts the netCDF data from the EOL ISFF
...
##-------------------------------------------------------------------------
sub main {
    my $converter = ISFF_SFC_Converter->new();
    $converter->convert();
    $converter->clean_empty_files();
}

##-------------------------------------------------------------------------
# @signature ISFF_SFC_Converter new()
# <p>Create a new instance of the ISFF_SFC_Converter class.  This defines
...
##-------------------------------------------------------------------------
sub getFields {
    return ("station","base_time","time","latitude","longitude","P","RH_5m","T_5m","Rain","spd_max_10m","u_10m","v_10m");
}
...
##-------------------------------------------------------------------------
sub load_station {
    my ($self,$station_id,$latitude,$longitude) = @_;
    ...
    if (!defined($station)) {
      ...
      $station->setStationName("ISFF Station ".$station_id);
      ...
      # From http://www.eol.ucar.edu/rtf/projects/trex/isff/report.shtml
      if ($station_id =~ /central/i) {
	$station->setElevation(1155,"m");
      } elsif ($station_id =~ /south/i) {
	$station->setElevation(1141,"m");
      } elsif ($station_id =~ /west/i) {
	$station->setElevation(1529,"m");
      }
      
      $station->setLatLongAccuracy(4);
      $station->setStateCode("CA");
      $station->setReportingFrequency("5 minute");
      $station->setPlatformIdNumber(377);
...
#    }
    ...
    for (my $station_index = 0; $station_index < $var->{"station"}{"station"}; $station_index++) {

      # Determine the station the readings were recorded at.
      my $station = $self->load_station($var->{"station"}{"values"}[$station_index],
					$var->{"latitude"}{"values"}[$station_index],
					$var->{"longitude"}{"values"}[$station_index]);
	...
	# Determine the nominal time of the record since the record did not occur
	# on the 5 minute interval.
	my ($hour,$min,$sec) = split(/:/,$time);
	if ($min =~ /[2|7]$/) {
	  ($date,$time) = adjustDateTime($date,"YYYY/MM/DD",$time,"HH:MM:SS",0,0,-1*($min % 5),-1*$sec);
	  $record->setNominalDate($date,"YYYY/MM/DD");
	  $record->setNominalTime($time,"HH:MM:SS");
	} else {
	  die(sprintf("Don't know how to set nominal date for %s at %s %s\n",
		      $station->getStationId(),$date,$time));
	}

	foreach my $field (&getFields()) {

	  # Ignore all of these fields since they are handled in other places.
	  if ($field eq "station" || $field eq "time" || $field eq "base_time" ||
	      $field eq "latitude" || $field eq "longitude" || !defined($var->{$field})) {
	    next;
	  }

	  # Pull out the necessary information for the field.
	  my $value = $var->{$field}{"values"}[$time_index][$station_index];
	  my $missing = $var->{$field}{"_FillValue"};
	  my $units = $var->{$field}{"units"};
	  $units =~ s/degC/C/;

	  # Ignore any missing values.
	  if (!defined($missing) || !is_missing($value,$missing)) {

	    # Convert the precip to correct units.
	    # This needs to be done after the missing check to prevent the
	    # conversion of a missing value!
	    if ($units =~ /\.01in\/5sec/) {
	      $value = $value * (.01/5) * 60 * 25.4; # Convert to mm/5 min
	      $units = "mm";
	    }

	    # Assign the value to correct measurement.
	    $record->setPressure($value,$units) if ($field eq "P");
	    $record->setRelativeHumidity($value) if ($field eq "RH_5m" && $units eq "%");
	    $record->setTemperature($value,$units) if ($field eq "T_5m");
	    $record->setPrecip($value,$units) if ($field eq "Rain");
	    $record->setGustSpeed($value,$units) if ($field eq "spd_max_10m");
	    $record->setWindUComponent($value,$units) if ($field eq "u_10m");
	    $record->setWindVComponent($value,$units) if ($field eq "v_10m");
	  }
	}
...
sub update_for_TREX {
    ...
    if ($record->getGustSpeed() > 40) {
      $record->setGustSpeedFlag($DUBIOUS_FLAG);
    }
}
