...
package DRI_SFC_Converter;

##Module-------------------------------------------------------------------
# <p>The DRI_SFC_Converter is a script for converting the Desert Research
# Institute (DRI) surface data from its source netCDF format to the QCF format.
...
##-------------------------------------------------------------------------
sub main {
    my $converter = DRI_SFC_Converter->new();
    $converter->convert();
    $converter->clean_empty_files();
}

##-------------------------------------------------------------------------
# @signature DRI_SFC_Converter new()
...
sub getFields {
    return ("julday","hhmm","ss","aveat","averh","avebp","avews","wdir","maxws","stid");
}
...
sub load_stations {
	...
	my @data = split(/;/,$line);
        ...
        my $station = Station::Station->new("DRI_".$data[0],$self->{"NETWORK"});
	$station->setStationName("DRI Station ".$data[0]);

	# These values were pulled from the raw data README.
        $station->setLatitude($data[1],"DDDDDDDDD");
        $station->setLongitude($data[2],"-DDDDDDDDDD");
        $station->setElevation($data[3],"m");

        $station->setLatLongAccuracy(4);
        $station->setStateCode("CA");
        $station->setReportingFrequency("5 minute");
        $station->setPlatformIdNumber(365);
...
sub parse_raw_file {
    ...
    my $year = sprintf("%04d",substr($file,16,4));
    for (my $i = 0; $i < $arrayLen; $i++) {

	if ($var->{"ss"}{"values"}[$i] != 0) { next; }

	my $station = $self->{"stations"}->getStation(sprintf("DRI_%02d",$var->{"stid"}{"values"}[$i]),
						      $self->{"NETWORK"});

	my $record = Surface::QCFSurfaceRecord->new($self->{"WARN"},$station);
	$record->setReadingTime(sprintf("%04d/%03d",$year,$var->{"julday"}{"values"}[$i]),"YYYY/JJJ",
				sprintf("%04d",$var->{"hhmm"}{"values"}[$i]),"HHMM",8);
	...
	foreach my $field (&getFields) {
		...
		$record->setTemperature($value,$var->{$field}{"Unit"}) if ($field eq "aveat");
		$record->setRelativeHumidity($value) if ($var->{$field}{"Unit"} eq "%" && $field eq "averh");
		$record->setPressure($value,$var->{$field}{"Unit"}) if ($field eq "avebp");
		$record->setWindSpeed($value,$var->{$field}{"Unit"}) if ($field eq "avews");
		$record->setWindDirection($value) if ($field eq "wdir" && $var->{$field}{"Unit"} =~ /deg/i);
		$record->setGustSpeed($value,$var->{$field}{"Unit"}) if ($field eq "maxws");
	    }
	}

	if ($record->getNominalTime() =~ /:\d[05]$/) {
...
sub read_raw_files {
    ...
    opendir(my $RAWDIR,$self->{"RAW_DIR"}) or die("Unable to read raw directory: ".
						  $self->{"RAW_DIR"}."\n");
    my @files = sort(grep(/\.cdf$/,readdir($RAWDIR)));
    closedir($RAWDIR);
