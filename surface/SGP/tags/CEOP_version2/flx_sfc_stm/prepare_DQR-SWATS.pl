...
# 24 Jun 10, ds
#   this is only for SWATS flagging text file since
#   it has depths in the lines
#-----------------------------------------------------------
...
# the output file
#--------------------------------------------------------------------------------------
$file_frag = "code_frag/add_to_STM_conversion_software.txt";
open(OUTFILE, ">$file_frag") || die "Couldn't open $file_frag\n";

print OUTFILE "    #---------------------------------------------------------------------------\n";
print OUTFILE "    # following section fixes flags according to DQRs for SWATS data\n";
...
foreach $network (keys (%file_in)) {
	...
	#-------------------------------------------------------------------------------------------------------------
	#          ID             variable         depth   flag   month/day/year hourmin  month/day/year hourmin
	#-------------------------------------------------------------------------------------------------------------
    #	        E9           watcont_W         0.60      D       04/24/2006   1600      06/01/2006   1600     D060531.6
	#-------------------------------------------------------------------------------------------------------------

	while ($this_line = <INFILE>) {
		if ($this_line =~ /(^[CE]\d{1,3}) +(\w+) +(\d.\d{2}) +(\w) +(\d{2})\/(\d{2})\/(\d{4}) +(\d{4}) +(\d{2})\/(\d{2})\/(\d{4}) +(\d{4})/) { 
