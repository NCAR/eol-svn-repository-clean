...
# rev 20 Apr 10, ds
#   parameter names changed for EBBR data in 2009
#     this version of s/w is ONLY for data from 2009 and after
#     renamed ECOR params to match EBBR names at line 399-404
#--------------------------------------------------------------------------------------

$DEBUG = 1;
$DEBUG1 = 1;				# for even more messages
...
#--------------------------------------------------------------------------------------
@ebbr_fields  = ("time_offset", "corr_soil_heat_flow_1", "surface_soil_heat_flux_avg", "latent_heat_flux", "qc_latent_heat_flux", "sensible_heat_flux", "qc_sensible_heat_flux");		  # corrected soil heat flow 1, soil heat flow at the surface 1, latent heat flux, sensible heat flux
...
# (param num)       =             1      2          3           4 
@flx_parameter_list = ("sensible_heat_flux", "latent_heat_flux", "fc", "corr_soil_heat_flow_1");
...
$project_begin_date = 20090101;     # version2 for 2009 NOTE: this s/w only for 2009 and after (see header note for 4/20/10)
    	...
    	while ($this_line = <INFILE>) {
			...
			#			print "the number of obs for $obsName in $infile for $platform = $numObs, last uncounted obs on line = $line_value[$#line_value]\n";
						...
						if ($platform eq "ebbr" || $platform eq "ecor") {
							...
							if ($obsName eq "corr_soil_heat_flow_1" || $obsName eq "surface_soil_heat_flux_avg" || $obsName eq "fc") {			# soil heat flux and flow, and CO2 flux, Scot decided that we should switch the sign for EBBR 
	        	             	for ($i=1; $i <= $numObs; $i++) {
									$line_value[$i] *= -1.0 unless ($line_value[$i] == 0 || $line_value[$i] == -999.99 || $line_value[$i] > 9999);
								}
							} elsif (($obsName eq "latent_heat_flux" || $obsName eq "sensible_heat_flux") && ($platform eq "ebbr")) { 	# sensible and latent heat fluxes, Scot decided that we should switch the sign for EBBR 
	                    		for ($i=1; $i <= $numObs; $i++) {
		                    ...
		                    #------------------------------------------------------------
							$obsName = "sensible_heat_flux" if $obsName eq "h";
							$obsName = "qc_sensible_heat_flux" if $obsName eq "qc_h";
							$obsName = "latent_heat_flux" if $obsName eq "lv_e";
							$obsName = "qc_latent_heat_flux" if $obsName eq "qc_lv_e";
							$obsName = "corr_soil_heat_flow_1" if $obsName eq "c_shf1";
							$obsName = "surface_soil_heat_flux_avg" if $obsName eq "g1";
