...
package ChinaLakeHANDAR05_SFC_Converter;

##Module-------------------------------------------------------------------
# <p>The ChinaLakeHANDAR05_SFC_Converter is a script for converting the 
# NAWS China Lake HANDAR 5 minue weather station data from its source
...
##-------------------------------------------------------------------------
sub main {
    my $converter = ChinaLakeHANDAR05_SFC_Converter->new();
    $converter->convert();
    $converter->clean_empty_files();
}

##-------------------------------------------------------------------------
# @signature ChinaLakeHANDAR05_SFC_Converter new()
...
sub parse_raw_file {
	...
	$data[1] =~ /\"(\d+):(\d+):\d+/;
	my $time = sprintf("%02d:%02d",substr($data[2],0,2) eq "AM" ? ($1 == 12 ? 0 : $1) : 
			   ($1 == 12 ? $1 : $1 + 12),$2);
        $record->setReadingTime($date,"YYYY/MM/DD",$time,"HH:MM",8);

        if ($self->in_time_of_interest($record)) {
            $record->setWindDirection($data[3]);
            $record->setWindSpeed($data[4],"knot");
            $record->setGustSpeed($data[5],"knot");
            $record->setTemperature($data[6],"F");
            $record->setRelativeHumidity($data[7]);
            $record->setPressure($data[8],"mb");
...
sub update_for_TREX {
    ...
    if ($record->getStationId() eq "LAUREL") {
	if ($record->getNominalDate() eq "2006/03/29" && $record->getNominalTime() eq "23:30") {
	    $record->setGustSpeedFlag($DUBIOUS_FLAG) if ($record->getGustSpeed() != $MISSING);
	} elsif ($record->getNominalDate() eq "2006/04/12" && $record->getNominalTime() eq "15:40") {
    ...
    } elsif ($record->getStationId() eq "Parrot") {
	$record->setPressureFlag($DUBIOUS_FLAG) if ($record->getPressure() != $MISSING);
	$record->setCalcSeaLevelPressureFlag($BAD_FLAG) if ($record->getCalcSeaLevelPressure() != $MISSING);
	
	if ($record->getNominalDate() eq "2006/03/01" && $record->getNominalTime() eq "16:55") {
	    $record->setGustSpeedFlag($DUBIOUS_FLAG) if ($record->getGustSpeed() != $MISSING);
	} elsif ($record->getNominalDate() eq "2006/03/03" && $record->getNominalTime() eq "21:00") {
	    $record->setGustSpeedFlag($DUBIOUS_FLAG) if ($record->getGustSpeed() != $MISSING);
	} elsif ($record->getNominalDate() eq "2006/04/13" && $record->getNominalTime() eq "07:30") {
	    $record->setGustSpeedFlag($DUBIOUS_FLAG) if ($record->getGustSpeed() != $MISSING);
	}

    } elsif ($record->getStationId() eq "Tower_8") {
	if ($record->getPressure() < 900) {
	    $record->setPressureFlag($BAD_FLAG) if ($record->getPressure() != $MISSING);
	    $record->setCalcSeaLevelPressureFlag($BAD_FLAG) if ($record->getCalcSeaLevelPressure() != $MISSING);
	}
	
	if ($record->getNominalDate() eq "2006/03/19" && $record->getNominalTime() eq "17:55") {
	    $record->setGustSpeedFlag($DUBIOUS_FLAG) if ($record->getGustSpeed() != $MISSING);
	}
    }
}
