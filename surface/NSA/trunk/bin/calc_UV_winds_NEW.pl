...
# rev 25 Jun 08, SJS
#   added code to return a reference to a hash where
#   the keys are u_wind, u_wind_flag, v_wind and v_wind_flag
#-------------------------------------------------------------------
...
sub calc_UV_winds
{
   ...
   if ($wspd < 0.0 || $wdir < 0.0  || $wdir > 360.0) {
      ...
      $V_wind_flag = "M";
      $hash{'u_wind'} = $U_wind;
      $hash{'u_wind_flag'} = $U_wind_flag;
      $hash{'v_wind'} = $V_wind;
      $hash{'v_wind_flag'} = $V_wind_flag;
      #return;
      return \%hash;
