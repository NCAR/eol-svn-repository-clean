...
package CIMIS_SFC_Converter;

##Module-------------------------------------------------------------------
# <p>The CIMIS_SFC_Converter is a script for converting the California
# Irrigation Management Information System (CIMIS) from its source ASCII
# format  to the QCF format.</p>
#
# @author Joel Clawson
...
##-------------------------------------------------------------------------
sub main {
    my $converter = CIMIS_SFC_Converter->new();
    $converter->convert();
    $converter->clean_empty_files();
}

##-------------------------------------------------------------------------
# @signature CIMIS_SFC_Converter new()
# <p>Create a new instance of the CIMIS_SFC_Converter class.  This defines
...
sub new {
    ...
    $self->{"STATION_LIST"} = "../docs/CIMIS_STATIONS_0905.csv";
...
sub parse_flag {
    ...
    if ($type eq "PRECIP" && ($value eq "R" || $value eq "Y")) { return $DUBIOUS_FLAG; }
    elsif ($type eq "WINDSPD" && $value eq "I") { return $DUBIOUS_FLAG; }
    elsif ($type eq "WINDDIR" && $value eq "R") { return $BAD_FLAG; }


    elsif ($value eq "N") { return $NO_READING_FLAG; }
...
sub parse_raw_file {
    ...
    my $WARN = $self->{"WARN"};
    <$FILE>; # Parse off the header line.
	...
	# Prepare the data to be put into the QCF Record.
        ...
        my $station = $self->{"stations"}->getStation(sprintf("CIMIS_%04d",trim($data[0])),$self->{"NETWORK"});

	# Put the data values into the QCF Record.
        my $record = Surface::QCFSurfaceRecord->new($WARN,$station);
        $record->setReadingTime(trim($data[3]),"MM/DD/YYYY",trim($data[4]),"HHMM",8);

        if ($self->in_time_of_interest($record)) {

            $record->setPressureFlag($NO_READING_FLAG);

            my $flag = parse_flag("TEMP",trim($data[12]));
            if (trim($data[13]) !~ /^\-+$/) {
                $record->setTemperature(trim($data[13]),"C");
                $record->setTemperatureFlag($flag);
            }

            $flag = parse_flag("WINDSPD",trim($data[16]));
            if (trim($data[17]) !~ /^\-+$/) {
                $record->setWindSpeed(trim($data[17]),"m/s");
                $record->setWindSpeedFlag($flag);
            }

            $flag = parse_flag("WINDDIR",trim($data[18]));
            if (trim($data[19]) !~ /^\-+$/) {
                $record->setWindDirection(trim($data[19]));
                $record->setWindDirectionFlag($flag);
            }

            $flag = parse_flag("PRECIP",trim($data[20]));
            if (trim($data[21]) !~ /^\-+$/) {
                $record->setPrecip(trim($data[21]),"mm");
                $record->setPrecipFlag($flag);
            }

            $flag = parse_flag("GUSTSPD",trim($data[22]));
            if (trim($data[23]) !~ /^\-+$/) {
                $record->setGustSpeed(trim($data[23]));
                $record->setGustSpeedFlag($flag);
            }

            $flag = parse_flag("RH",trim($data[26]));
            if (trim($data[27]) !~ /^\-+$/) {
                $record->setRelativeHumidity(trim($data[27]));
                $record->setRelativeHumidityFlag($flag);
            }

            $flag = parse_flag("DEWPT",trim($data[28]));
            if (trim($data[29]) !~ /^\-+$/) {
                $record->setDewPoint(trim($data[29]),"C");
                $record->setDewPointFlag($flag);
            }
...
sub read_station_file {
	...
	my $station = Station::Station->new(sprintf("CIMIS_%04d",trim($data[0])),$self->{"NETWORK"});
	$station->setStationName(sprintf("%s, %s",trim($data[1]),trim($data[2])));

        $station->setLatitude(trim($data[4]),"DDDDD");
        $station->setLongitude(trim($data[5]),"-DDDDDD");
	$station->setElevation(trim($data[6]),"ft");

        $station->setLatLongAccuracy(2);
        $station->setStateCode("CA");
        $station->setReportingFrequency("hourly");
        $station->setPlatformIdNumber(357);
...
sub update_for_TREX {
    my ($self,$record) = @_;
    my ($date,$time) = adjustDateTime($record->getNominalDate(),"YYYY/MM/DD",$record->getNominalTime(),"HH:MM",0,-8,0,0);
    my $datetime = sprintf("%s%s",$date,$time);
    $datetime =~ s/[\/:]//g;


    if ($record->getStationId() eq "CIMIS_0082" && 
        ((200603040600 <= $datetime && $datetime <= 200603100400) ||
         (200603120400 <= $datetime && $datetime <= 200603181500))) {
        $record->setWindDirectionFlag($BAD_FLAG) unless ($record->getWindDirection() == $MISSING);
    }

    if ($record->getStationId() eq "CIMIS_0091" &&
        (200604301400 <= $datetime && $datetime <= 200604301900)) {
        $record->setDewPointFlag($DUBIOUS_FLAG) unless ($record->getDewPoint() == $MISSING);
    }

    if ($record->getStationId() eq "CIMIS_0094" &&
        (200604040300 <= $datetime && $datetime <= 200604091400)) {
        $record->setWindSpeedFlag($BAD_FLAG) unless ($record->getWindSpeed() == $MISSING);
    }

    if ($record->getStationId() eq "CIMIS_0107" &&
        (200604040300 <= $datetime && $datetime <= 200604060000)) {
        $record->setWindSpeedFlag($BAD_FLAG) unless ($record->getWindSpeed() == $MISSING);
    }

    if ($record->getStationId() eq "CIMIS_0122" &&
        ((200603050200 <= $datetime && $datetime <= 200603061000) ||
         (200604030200 <= $datetime && $datetime <= 200604050900))) {
        $record->setWindSpeedFlag($BAD_FLAG) unless ($record->getWindSpeed() == $MISSING);
    }

    if ($record->getStationId() eq "CIMIS_0168" &&
        (200604130300 == $datetime || 200604130400 == $datetime || 200604131400 == $datetime)) {
        $record->setTemperatureFlag($BAD_FLAG) unless ($record->getTemperature() == $MISSING);
        $record->setDewPointFlag($BAD_FLAG) unless ($record->getDewPoint() == $MISSING);
    }

    if ($record->getStationId() eq "CIMIS_0177" &&
        (200603131600 <= $datetime && $datetime <= 200603200900)) {
        $record->setWindSpeedFlag($BAD_FLAG) unless ($record->getWindSpeed() == $MISSING);
    }

    if ($record->getStationId() eq "CIMIS_0191" &&
        (200604062000 <= $datetime && $datetime <= 200604101500)) {
        $record->setWindSpeedFlag($BAD_FLAG) unless ($record->getWindSpeed() == $MISSING);
    }
}
