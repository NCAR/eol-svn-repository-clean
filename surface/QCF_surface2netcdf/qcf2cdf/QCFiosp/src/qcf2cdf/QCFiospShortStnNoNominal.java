 ...
 *
 * This QCFiospShortStnNoNominal program is used to convert the QCF data files with a station
 * value that is 10 characters instead of the usual 15 characters. These data files also have no 
 * nominal date or time.  Each header record is therefore 25 characters shorter.  An 
 * example data set is 1.38. 
 * 
 ...
 */
public class QCFiospShortStnNoNominal extends AbstractIOServiceProvider {
 ...
 * lines at 241 characters each (including new line character).
 */
public boolean isValidFile(RandomAccessFile infile) throws IOException {
      ...
      return (test.substring(727, 728).equals("/") && test.substring(730, 731).equals("/") &&
            test.substring(736, 737).equals(":") && test.substring(739, 740).equals(":") &&
            (fileType.equals("qcf")));
   }
...
// Check for record with latest date time.
         ...
         networkArray.setString(i, data[2].trim());
         idArray.setString(i, data[3].trim());
         var = Double.parseDouble(data[4]);
         latArray.set(i, var);
         var = Double.parseDouble(data[5]);
         lonArray.set(i, var);            
         shortvar = Short.parseShort(data[6]);
         occArray.set(i, shortvar);
         var = Double.parseDouble(data[7]);
         eleArray.set(i, var);
// Convert pressure from mbar to pascal.
         var = Double.parseDouble(data[8]);
         if (var != fillValue) {
            var = var * 100;
         }
         pressArray.set(i, var);             
         pflagArray.set(i, data[9].charAt(0));
         var = Double.parseDouble(data[10]);
         if (var != fillValue) {
            var = var * 100;
         }
         pressRSLArray.set(i, var);             
         pRSLflagArray.set(i, data[11].charAt(0));
         var = Double.parseDouble(data[12]);
         if (var != fillValue) {
            var = var * 100;
         }
         pressCSLArray.set(i, var);             
         pCSLflagArray.set(i, data[13].charAt(0));            
...
// Convert temperature from Celsius to Kelvin         
         tempArray.set(i, var);
         tflagArray.set(i, data[15].charAt(0));
         ...
         dpArray.set(i, var);
         dpflagArray.set(i, data[17].charAt(0));
         var = Double.parseDouble(data[18]);
         wspdArray.set(i, var);
         wsflagArray.set(i, data[19].charAt(0));
         var = Double.parseDouble(data[20]);
         wdirArray.set(i, var);
         wdflagArray.set(i, data[21].charAt(0));
         var = Double.parseDouble(data[22]);
         precipArray.set(i, var);
         paflagArray.set(i, data[23].charAt(0));
         gustindArray.set(i, data[24].charAt(0));
         var = Double.parseDouble(data[25]);
         gustspArray.set(i, var);
         gsflagArray.set(i, data[26].charAt(0));
         intvar = Integer.parseInt(data[27]);          
         preswxArray.set(i, intvar);
         pwflagArray.set(i, data[28].charAt(0));
         var = Double.parseDouble(data[29]);
         visArray.set(i, var);
         visflagArray.set(i, data[30].charAt(0));
...
// Convert to meters
         ...
         intvar = Integer.parseInt(data[32]);
         cindArray.set(i, 0, intvar);
         cindflagArray.set(i, 0, data[33].charAt(0));
         intvar = Integer.parseInt(data[34]);
         camtArray.set(i, 0, intvar);
         camtflagArray.set(i, 0, data[35].charAt(0));
// Original value is hundreds of feet.
// Convert to meters
         ...
         intvar = Integer.parseInt(data[37]);
         cindArray.set(i, 1, intvar);
         cindflagArray.set(i, 1, data[38].charAt(0));
         intvar = Integer.parseInt(data[39]);
         camtArray.set(i, 1, intvar);
         camtflagArray.set(i, 1, data[40].charAt(0));
// Original value is hundreds of feet.
// Convert to meters
         var = Double.parseDouble(data[41]);
         if (var != fillValue) {
            var = var * 100;
            var = LengthUtils.convertLength(var, LengthUtils.FEET, LengthUtils.METERS);
         }
         cloudArray.set(i, 2, var);
         intvar = Integer.parseInt(data[42]);
         cindArray.set(i, 2, intvar);
         cindflagArray.set(i, 2, data[43].charAt(0));
         intvar = Integer.parseInt(data[44]);
         camtArray.set(i, 2, intvar);
         camtflagArray.set(i, 2, data[45].charAt(0));
      }
  ...
  */
   public void build(RandomAccessFile infile, Group.Builder rootGroup, CancelTask cancelTask)
      ...
      try {
         ...
         e.printStackTrace();
         throw new IOException("QCFiospShortStnNoNominal: Parse exception occurred.");
      } catch (ConversionException e) {
         e.printStackTrace();
         throw new IOException("QCFiospShortStnNoNominal: Invalid conversion attempted.");
      }
...
/**
  ...
  */
   protected static void usage(int ret) {
      System.err.println("usage: QCFiospShortStnNoNominal\n" +
...
// Parse arguments on command line
      Getopt g = new Getopt("QCFiospShortStnNoNominal", args, "i:o:u:p:d:");
      ...
      ucar.nc2.NetcdfFiles.registerIOProvider(QCFiospShortStnNoNominal.class);
      ncfile = null;
