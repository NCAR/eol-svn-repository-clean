 ...
 *
 * This QCFiospWind program is used to convert the QCF data files with u/v wind and 
 * specific humidity values with flags at the end of each record.  These data files also have
 * no header records, dates are YY/MM/DD (2 digit year) and times are HH:MM with no seconds. 
 * Each line of data is 286 characters instead of 266 characters.
 * An example data set is 19.014.
 * 
 ...
 */
public class QCFiospWind extends AbstractIOServiceProvider {
 ...
 * lines at 266 characters each (including new line character).
 */
public boolean isValidFile(RandomAccessFile infile) throws IOException {
      ...
      return (test.substring(2, 3).equals("/") && test.substring(5, 6).equals("/") &&
            test.substring(11, 12).equals(":") &&
            (fileType.equals("qcf")));
   }
   ...
   private ArrayDouble.D1 latArray, lonArray, eleArray, humidArray;
   ...
   private ArrayDouble.D1 eastwindArray, northwindArray;
   ...
   private ArrayChar.D1 ewindflagArray, nwindflagArray, humflagArray;
...
// Reposition file pointer to beginning of first data record. This version of the
// QCF data has no header record.  (File was previously
...
// data2 is used to store the final values of the data record.
         String[] data2 = new String[54];
         
// Since station identifier can have spaces in the name, set first six fields by
// position in data record.
                 
         data2[0] = line.substring(0, 8);
         data2[1] = line.substring(9, 14);
         data2[2] = line.substring(15, 23);
         data2[3] = line.substring(24, 29);
         data2[4] = line.substring(30, 40);
         data2[5] = line.substring(41, 56);
// Start line2 with Latitude field and end with Total Precipitation flag field.
         String line2 = line.substring(57, 171).trim();
...
// create dimension for all variables in netCDF file
      ...
      int[] stationShape = new int[]{count, 15};
...
// Set size of data arrays
      ...
      idArray = (ArrayChar.D2) Array.factory(DataType.CHAR, stationShape);
      ...
      eastwindArray = (ArrayDouble.D1) Array.factory(DataType.DOUBLE, shape);
      ewindflagArray = (ArrayChar.D1) Array.factory(DataType.CHAR, shape);
      northwindArray = (ArrayDouble.D1) Array.factory(DataType.DOUBLE, shape);
      nwindflagArray = (ArrayChar.D1) Array.factory(DataType.CHAR, shape);
      humidArray = (ArrayDouble.D1) Array.factory(DataType.DOUBLE, shape);
      humflagArray = (ArrayChar.D1) Array.factory(DataType.CHAR, shape);
...
// if necessary.  If data contains -0.00, change it to 0.00.
      for (int i = 0; i < count; i++) {
         data = records.get(i);
// Change the 2 digit years to 4 digit years
         if (Integer.parseInt(data[0].substring(0,2)) < 50) {
            data[0] = "20" + data[0];
         } else {
            data[0] = "19" + data[0];
         }
         if (Integer.parseInt(data[2].substring(0,2)) < 50) {
            data[2] = "20" + data[2];
         } else {
            data[2] = "19" + data[2];
         }
...
// Convert to meters
         ...
         var = Double.parseDouble(data[48]);
         eastwindArray.set(i, var);
         ewindflagArray.set(i, data[49].charAt(0));
         var = Double.parseDouble(data[50]);
         northwindArray.set(i, var);
         nwindflagArray.set(i, data[51].charAt(0));
         var = Double.parseDouble(data[52]);
         if (var == -9.9999) {  // missing value for specific humodity
            var = fillValue;  // Set to regular fill value
         }
         humidArray.set(i, var);
         humflagArray.set(i, data[53].charAt(0));
      }
  ...
  */
   public void build(RandomAccessFile infile, Group.Builder rootGroup, CancelTask cancelTask)
      ...
      try {
         ...
         e.printStackTrace();
         throw new IOException("QCFiospWind: Parse exception occurred.");
      } catch (ConversionException e) {
         e.printStackTrace();
         throw new IOException("QCFiospWind: Invalid conversion attempted.");
      }
...
// Create time and name string length dimensions for netcdf file.
      ...
      Dimension networkLengthDim = Dimension.builder("network_strlen", networkLength).build();
      rootGroup.addDimension(networkLengthDim);
      Dimension stationLengthDim = Dimension.builder("platform_strlen", stationLength).build();
      rootGroup.addDimension(stationLengthDim);
...
//      .addAttribute(new Attribute("missing_value", fillValue))      
      ...
      rootGroup.addVariable(Variable.builder().setName("eastward_wind").setDataType(DataType.DOUBLE)
              .addDimension(recordDim).addAttribute(new Attribute("standard_name", "eastward_wind"))
              .addAttribute(new Attribute("long_name", "Eastward Wind"))
              .addAttribute(new Attribute("units", "m s-1"))              
              .addAttribute(new Attribute("coordinates", "record"))
              .addAttribute(new Attribute("ancillary_variables", "eastward_wind_flag"))
              .addAttribute(new Attribute("_FillValue", fillValue))
              .addAttribute(new Attribute("missing_value", fillValue))
              .setCachedData(eastwindArray, false));
      rootGroup.addVariable(Variable.builder().setName("eastward_wind_flag").setDataType(DataType.CHAR)
              .addDimension(recordDim).addAttribute(new Attribute("standard_name", "eastward_wind status_flag"))
              .addAttribute(new Attribute("long_name", "Eastward Wind Flag"))
              .addAttribute(new Attribute("flag_values", "U G M D B N X E C T I"))
              .addAttribute(new Attribute("flag_meanings",
          "unchecked good missing questionable unlikely not_available_or_not_observed glitch estimated exceeds_9999.99mm_or_negative trace_amount_recorded not_computed_due_to_insufficient_data"))
              .addAttribute(new Attribute("coordinates", "record"))
              .addAttribute(new Attribute("_FillValue", "M"))
              .addAttribute(new Attribute("missing_value", "M"))
              .setCachedData(ewindflagArray, false));
      rootGroup.addVariable(Variable.builder().setName("northward_wind").setDataType(DataType.DOUBLE)
              .addDimension(recordDim).addAttribute(new Attribute("standard_name", "northward_wind"))
              .addAttribute(new Attribute("long_name", "Northward Wind"))
              .addAttribute(new Attribute("units", "m s-1"))              
              .addAttribute(new Attribute("coordinates", "record"))
              .addAttribute(new Attribute("ancillary_variables", "northward_wind_flag"))
              .addAttribute(new Attribute("_FillValue", fillValue))
              .addAttribute(new Attribute("missing_value", fillValue))
              .setCachedData(northwindArray, false));
      rootGroup.addVariable(Variable.builder().setName("northward_wind_flag").setDataType(DataType.CHAR)
              .addDimension(recordDim).addAttribute(new Attribute("standard_name", "northward_wind status_flag"))
              .addAttribute(new Attribute("long_name", "Northward Wind Flag"))
              .addAttribute(new Attribute("flag_values", "U G M D B N X E C T I"))
              .addAttribute(new Attribute("flag_meanings",
          "unchecked good missing questionable unlikely not_available_or_not_observed glitch estimated exceeds_9999.99mm_or_negative trace_amount_recorded not_computed_due_to_insufficient_data"))
              .addAttribute(new Attribute("coordinates", "record"))
              .addAttribute(new Attribute("_FillValue", "M"))
              .addAttribute(new Attribute("missing_value", "M"))
              .setCachedData(nwindflagArray, false));
      rootGroup.addVariable(Variable.builder().setName("specific_humidity").setDataType(DataType.DOUBLE)
              .addDimension(recordDim).addAttribute(new Attribute("standard_name", "specific_humidity"))
              .addAttribute(new Attribute("long_name", "Specific Humidity"))
              .addAttribute(new Attribute("units", "kg kg-1"))              
              .addAttribute(new Attribute("coordinates", "record"))
              .addAttribute(new Attribute("ancillary_variables", "specific_humidity_flag"))
              .addAttribute(new Attribute("_FillValue", fillValue))
              .addAttribute(new Attribute("missing_value", fillValue))
              .setCachedData(humidArray, false));
      rootGroup.addVariable(Variable.builder().setName("specific_humidity_flag").setDataType(DataType.CHAR)
              .addDimension(recordDim).addAttribute(new Attribute("standard_name", "specific_humidity status_flag"))
              .addAttribute(new Attribute("long_name", "Specific Humidity Flag"))
              .addAttribute(new Attribute("flag_values", "U G M D B N X E C T I"))
              .addAttribute(new Attribute("flag_meanings",
          "unchecked good missing questionable unlikely not_available_or_not_observed glitch estimated exceeds_9999.99mm_or_negative trace_amount_recorded not_computed_due_to_insufficient_data"))
              .addAttribute(new Attribute("coordinates", "record"))
              .addAttribute(new Attribute("_FillValue", "M"))
              .addAttribute(new Attribute("missing_value", "M"))
              .setCachedData(humflagArray, false));
   }
...
/**
  ...
  */
   protected static void usage(int ret) {
      System.err.println("usage: QCFiospWind\n" +
...
// Parse arguments on command line
      Getopt g = new Getopt("QCFiospWind", args, "i:o:u:p:d:");
      ...
      ucar.nc2.NetcdfFiles.registerIOProvider(QCFiospWind.class);
      ncfile = null;
