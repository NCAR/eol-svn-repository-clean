...
# @author Linda Echo-Hawk 01 May 2023
# @version TORUS 2022
#          - These data are in a comma-delimited ASCII format.
#
#            There are a total of 71 soundings from three platforms:
#            Far Field - 48 soundings
#            Lidar - 19 soundings
#            Probe1 - 4 soundings
# 
#            The data are in nearly the same csv format they were for the 2019 
#            processing that was done by Linda. The one format difference I note 
#            is that the 2022 data contains a 'GPS Time Record' parameter that 
#            was not present in 2019. Otherwise the format seems to be the same.
#
#            Assume each record is at a one second interval.
#
#            We should use the same parameters we used in the TORUS_2019 data set:
#
#            Filtered Temperature (K)
#            Filtered Humidity
#            Filtered Dewpoint (K)
#            Filtered Pressure (mb)
#            Filtered Wind Dir
#            Filtered Wind Spd (m/s)
#            Filtered Latitude
#            Filtered Longitude
#            FiltGeoPotHeight (m)
#
#            The wind components will need to be calculated. The temp and dewpt 
#            will need to be converted to C.
#
#            The headers should be set up the same way there were for the 2019 data:
#
#            Data Type: NSSL Radiosonde Data/Ascending
#            Project ID: TORUS_2022
#            Release Site Type/Site ID: Use the station name from the file name (e.g. 
#              Far Field, Lidar, , Probe1)  
#            Release Location: Use the lat/lon and height from the first data record
#            UTC Release Time: Grab from the file name
#            Radiosonde Type: Vaisala RS41-SGP
#            Radiosonde Serial Number: Use the value from the Serial Number parameter
#
#
# @author Linda Echo-Hawk 21 Feb 2020
# @version TORUS 2019
#          - These data are in a comma-delimited ASCII format.
#          - NOTE: Code is in place to read the mobile unit
...
#            Far_Field_MW41_output_20190516_212835.csv
#            LIDAR_1_MW41_output_20190516_222023.csv
#            LIDAR_2_MW41_output_20190517_224146.csv
#            Probe_1_MW41_output_20190517_223619.csv
#
...
printf "\nNSSL_Radiosonde_Converter.pl began on ";print scalar localtime;printf "\n";
my $debug = 0;

&main();
printf "\nNSSL_Radiosonde_Converter.pl ended on ";print scalar localtime;printf "\n";
...
sub parseHeader {
    ...
    $header->setType("NSSL Radiosonde Data");
	...
	#if ($file =~ /FarField/i)
	#{
	#	$header->setSite("Far Field");

	#}
	#elsif ($file =~ /Lidar/i)
	#{
	#	$header->setSite("Lidar");

	#}
	#elsif ($file =~ /Probe1/i)
	#{
	#	$header->setSite("Probe1");
...
sub parseRawFile {
	...
	foreach my $line (@lines) 
	{
		...
		if ($index >= 3)
		{
			...
			# Filtered Temperature (K)  $data[2] s/b $data[3]
            ...
            if ($data[6])
			{
				$record->setPressure($data[6],"mb") if ($data[6] !~ /-32768/);
			}


			if ($data[3])
			{
				my $converted_temp = convertTemperature($data[3], "K", "C") if ($data[3] !~ /-32768/);
				$record->setTemperature($converted_temp,"C");
			}

			if ($data[5])
			{
				my $converted_dewpt = convertTemperature($data[5], "K", "C") if ($data[5] !~ /-32768/);
				$record->setDewPoint($converted_dewpt,"C");
			}
			
			if ($data[4])
			{
                $record->setRelativeHumidity($data[4]) if ($data[4] !~ /-32768/);

			}

			if ($data[8])
			{
				$record->setWindSpeed($data[8],"m/s") if ($data[8] !~ /-32768/);
			}
            ...
            if ($data[10])
			{
				$record->setLatitude($data[10], $self->buildLatLonFormat($data[10]));
			}
			if ($data[11])
			{
            ...
            if ($data[31])
			{
				$record->setAltitude($data[31],"m") if ($data[31] !~ /-32768/);
			}
