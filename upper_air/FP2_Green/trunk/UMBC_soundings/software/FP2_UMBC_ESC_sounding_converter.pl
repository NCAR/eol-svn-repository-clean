...
##Module------------------------------------------------------------------------
# <p>The FP2_UMBC_ESC_sounding_converter.pl script is used for converting high
...
#        - Added code to check for missing wind speed and wind
#          direction. If these are missing, code does not set the
#          U and V wind components. The "missing" value for the
#          U and V columns was not what we expected.
#
#
...
#
##Module------------------------------------------------------------------------
package FP2_UMBC_ESC_sounding_converter;
use strict;
...
printf "\nFP2_UMBC_ESC_sounding_converter.pl began on ";print scalar localtime;printf "\n";
&main();
printf "\nFP2_UMBC_ESC_sounding_converter.pl ended on ";print scalar localtime;printf "\n";

##------------------------------------------------------------------------------
# @signature void main()
# <p>Process the FP2 UMBC ESC radiosonde data by converting it from 
# the native ASCII format into the ESC format.</p>
##------------------------------------------------------------------------------
sub main {
    my $converter = FP2_UMBC_ESC_sounding_converter->new();
    $converter->convert();
} # main

##------------------------------------------------------------------------------
# @signature FP2_UMBC_ESC_sounding_converter new()
# <p>Create a new instance of a FP2_UMBC_ESC_sounding_converter.</p>
#
# @output $self A new FP2_UMBC_ESC_sounding_converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
...
sub parseRawFile {
    ...
    if (defined($header)) 
	{
	    ...
	    foreach my $line (@lines)  
    	{
            ...
            if (($data[10] !~ /-32768/) && ($data[11] !~ /-32768/))
			{
