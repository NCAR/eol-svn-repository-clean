...
# @version VORTEX2 2010 (28 Oct 2010) Updated the 2009 converter
#           - Added the buildLatLonFormat function and changed the
#             converter to call this function
#
# @author Linda Echo-Hawk
# @version VORTEX2 2009  Updated for the NCAR MGAUS and NSSL MGAUS Sounding Data
#           - Search on "HARD-CODED" to find project specific information
#             that may need to be updated.
#           - Added a check for "Reference Launch Data Source/Time" when
#             value is "unknown/unknown" (2nd data element is undefined)
#  BEWARE:  This s/w assumes the raw data will be in /raw_data/NCAR*/ dirs.
#
# @author L. Cully
...
sub new {
    ...
    $self->{"STATION_FILE"} = sprintf("%s/NCAR_%s_%s_sounding_stationCD.out",$self->{"FINAL_DIR"},
...
sub parse_header {
    ...
    $header->setId(sprintf("NCAR_MGAUS_%02d",$1));
	...
	# Parse the release time from the header lines.
	...
	if ($lines[$i] !~ /^\s*\/$/) 
	{
		...
		{
            # Vaisala WXT510/00:51:12.00 OR unknown/unknown
			# print " LAUNCH: $lines[$i]\n";
			my @launchData = (split(' ',$lines[$i]))[4..5];
			# for the case of "unknown/unknown"
			if (!defined($launchData[1]))
			{
				$data = $launchData[0];
			}
			else
			{
				$data = join " ", @launchData;
			}
		}
...
sub read_data_files {
    ...
    my @dirs = grep(/NCAR\d$/,sort(readdir($RAW)));
    closedir($RAW);
