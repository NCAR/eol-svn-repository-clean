...
##Module------------------------------------------------------------------------
# <p>The GAUS_Converter.pl script is used for converting GAUS radiosonde data
# in the EOL sounding format into the EOL Sounding Composite (ESC) format.</p> 
#
#
# @author Linda Echo-Hawk 2008-10-02 
# @version VOCALS_2008  Updated based on the ISS_Converter.pl for PLOWS_2008-2009.
#          This was used for the Iquique GAUS Radiosonde Data.
#          NOTE: A change was made to pass the NON-truncated latitude/longitude 
#          into setLatitude/setLongitude in order for output header info to match
#          raw data info for the Release (Launch) Location line.
#          Search for "HARD-CODED" to find areas requiring project-specific changes.  
# 
#
...
printf "\nGAUS_Converter.pl began on ";print scalar localtime;printf "\n";
&main();
printf "\nGAUS_Converter.pl ended on ";print scalar localtime;printf "\n";

##------------------------------------------------------------------------------
# @signature void main()
# <p>Process the GAUS radiosonde data by converting it from the native ASCII
...
# <p>Create a new instance of a GAUS_Converter.</p>
#
# @output $self A new GAUS_Converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
...
sub parseHeader {
	...
	# Note for VOCALS, header info was merely release date
    # my ($site) = (split(/:/,$lines[3]))[1];
	my $site = "R/V Jose Olaya";
    # HARD-CODED
    $header->setId("Olaya_GAUS");
