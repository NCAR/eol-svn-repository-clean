...
#
# Execute command: 
#    GTSBUFR_Radiosonde_Converter.pl [project] [input dir] [output dir]
#
# Examples:
#    GTSBUFR_Radiosonde_Converter.pl SOCRATES ../raw_data ../output >& run2.log &
#    GTSBUFR_Radiosonde_Converter.pl SOCRATES ../raw_data/australia ../output/australia >& runAUS.log &
#    GTSBUFR_Radiosonde_Converter.pl SOCRATES ../raw_data/antarctica ../output/antarctica >& runANT.log &
#    GTSBUFR_Radiosonde_Converter.pl SOCRATES ../raw_data/new_zealand ../output/new_zealand >& runNZ.log &
#
...
sub new 
   ...
   print "ARGV Values: PROJECT: $ARGV[0], Input RAW_DIR: $ARGV[1], OUTPUT_DIR: $ARGV[2]\n";
...
#
# Assumption: Input file name form/example: 201803071200.casey_aq_sounding_1.preproc
##----------------------------------------------------------------------------------
sub parseHeader 
   {
   ...
   if (($headerRecVal{blockNumber} > 0) && $headerRecVal{stationNumber} >0)
     ...
     my @filename_subparts = split /\_/, $filename_parts[1]; # lord, howe, island, au, sounding, 1
     ...
     my $locCountry = $fileArrayLength-3;  # HARDCODED
...
sub parseRawFile 
  ...
  if ($debug)
   ...
   if (defined($header)) 
      ...
      my @filename_parts = split /\./, $file;
      my @filename_subparts = split /\_/, $filename_parts[1]; # E.g., lord, howe, island, au, sounding, 1
      ...
      foreach my $line (@datalines)   # Only process datalines in this loop.
         ...
         if ($input_key eq "timePeriod")  # Each data rec set starts with "timePeriod". ASSUMPTION. 
            ...
            if ($DataRecCt >0)   # Not first record, so not first timePeriod.
               ...
               if ($debug) {print "\nTry to Calc AscRate:  getTime= $getTime, prev_time= $prev_time,  getAlt= $getAlt, prev_alt = $prev_alt\n";}
               ...
               if ($current_time == 0.0) 
                  ...
                  {
                  if ($debug) {print "Is timePeriod rec. Not first rec. timePeriod ne 0.0. Previous Time = $prev_time, Current Time = $current_time. Output previous rec as is.  writePrevRecToOutput = $writePrevRecToOutput\n";}

                  if ($writePrevRecToOutput)   # All recs currently being written to output.
                     {
                     if ($debug) {print "Writing previous rec to output file for prev_time = $prev_time. DataRecCt = $DataRecCt.\n";}

                     printf($OUT $recordOut->toString()); # Write to output file
                     }
                  else
                     {
                     print "WARNING: ParseRawFile( $file )  NOT writing prev rec to output at record = $DataRecCt. prev_time = $prev_time, Current Time= $current_time\n";
                     }                  
