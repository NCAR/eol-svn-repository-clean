...
#
# July 2022 - LEC added in radiosondeType = 124 => "Vaisala RS41/AUTOSONDE". This
#    was new with SWEX 2022 and TORUS 2022.
# June 2024 - LEC added in "no" for country Norway for CAESAR project. Also updated 
#    separate station list file to include 4 stations from Norway (i.e., 01001,
#    01004, 01010, 01028). Added wigos* "notUsed" elements. Note that the 
#    wigosLocalIdentifierCharacter element matches WMO ID in Norway data.  Reinstated
#    code that used the data element of "extendedVerticalSoundingsignificance" that 
#    allows the user to either keep or drop the sounding mandatory levels (i.e., 
#    850,700,500, etc.). This was not used during RELAMPAGO but now used again for
#    CAESAR 2024 Norway sites. 
#
##Module------------------------------------------------------------------------
...
sub parseHeader 
   ...
   if (($headerRecVal{blockNumber} > 0) && $headerRecVal{stationNumber} >0)
     ...
     my $country = "no";  # HARDCODED to only process Norway for CAESAR!!!!
     ...
     #------------------------------------------------------
     # For Norway sites, use blockNumber stationNumber combo
     # to Id the site. This is the WMO number but without
     # the preceeding zero.
     # -----------------------------------------------------
     my $ENMIsiteFlag = 0;

     if ($site eq "ENMI") # For CAESAR 2024 Norway Sites
        {
	#------------------------------------------------------------
	# Form proper WMO ID number for Norway sites (total 5 chars)
	#------------------------------------------------------------
        # Format Block number with up to 2 digits with leading zeroes
	#------------------------------------------------------------
        $BNstr = sprintf("%02d", $BNstr);

	#------------------------------------------------------------
	# Format Station number with up to 3 digits with leading zeroes
	#------------------------------------------------------------
        $SNstr = sprintf("%03d", $SNstr);
	
	$site = $BNstr.$SNstr;

	$ENMIsiteFlag = 1;
	print "Found Norway ENMI site: $site ENMIsiteFlag = $ENMIsiteFlag\n";

        } # End ENMI/Norway sites
     ...
     # ------------------------------------------------------------
     # Unlike NWS sites, the ENMI/Norway sites don't have a 4 letter
     # code for the site names so reset that to be no chars
     # ------------------------------------------------------------
     if ($ENMIsiteFlag) 
        { 
        print "WARNING: Found ENMI/Norway site. Reset Release Site prefix to be blank!";
        $site = $site_name." / ".$BNstr.$SNstr;
        } 
     else
        {     
        print "WARNING: Not a  ENMI/Norway site. Keep full Release Site name.";
   ...
   if ( exists($headerRecVal{balloonType}))  # The Key exists and has value == True
      {
      print "(11)balloon_type = xxx $balloon_type xxx, BType_info = xxx $BType_info xxx.\n";

      $BType_info = $headerRecVal{balloonType};
      $BType_info =~ s/^\s+//; # trim white space

      print "(22) balloon_type = xxx $balloon_type xxx, BType_info = xxx $BType_info xxx.\n";
   ...
   if ($debug) {print "\nBEFORE:  Nominal date: $date_Year $date_Month $date_Day \n";} 

   my $date = sprintf("%04d, %02d, %02d", $date_Year, $date_Month, $date_Day);
   if ($debug) {print "\nAFTER:  Nominal date: $date; $date_Year $date_Month $date_Day \n";} 
   ...
   if ($debug) {print "BEFORE: Nominal time: $time_Hour $time_Minute $time_Second \n\n";} 
   my $time = sprintf("%02d:%02d:%02d", $time_Hour, $time_Minute, $time_Second);
   if ($debug) {print "AFTER: Nominal time: $time, $time_Hour $time_Minute $time_Second \n\n";}

   if ($debug) {print "\nCalling setNominalRelease with : xxx $date xxx $time xxx \n";} 
   #WAS:   $header->setNominalRelease($date,"YYYY, MM, DD",$time,"HH:MM:SS",0);  # HERE NOW
   $header->setNominalRelease($date,"YYYY, MM, DD",$time,"HH:MM:SS",0,0);

   if ($debug) {print "Return from header->setNominalRelease\n";}
   # if ($debug) {exit(1);}
   ...
   #WAS:   $header->setActualRelease($date,"YYYY, MM, DD",$time,"HH:MM:SS",0); # HERE NOW
   $header->setActualRelease($date,"YYYY, MM, DD",$time,"HH:MM:SS",0,0);
...
sub parseRawFile 
   ...
   # Migrated several elements from notUsed to Used sections. 
   #
   # Added wigos* elements to process Norway data.
   # ---------------------------------------------------------------------------
   my %headerElements_notUsed = (
     ...
     "wigosIdentifierSeries" => 1,
     "wigosIssuerOfIdentifier" => 1,
     "wigosIssueNumber" => 1,
     "wigosLocalIdentifierCharacter" => 1,
     );
  ...
  # WAS Following commented out in RELAMPAGO processing
    my %dataElements_notUsed = (
     fake_extendedVerticalSoundingSignificance => 1,
     );
