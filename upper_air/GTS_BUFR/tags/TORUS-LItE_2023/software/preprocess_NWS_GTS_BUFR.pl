...
#--------------------------------------------------------------------------------
# preprocess_NWS_GTS_BUFR.pl
#
# IMPORTANT NOTE:: This software runs on the National Weather Service (NWS) GTS BUFR (binary)
# data as collected in the Field Data Archive (FDA) dataset 100.030. The input BUFR file
# names are not the same as was processed for SOCRATES or RELAMPAGO project's GTS BUFR data.
# The input file names for those previous (non-US) projects was 201901041105.87155_ar_sounding.bufr
# where the NWS GTS BUFR data input files have names like KSLC_202202150000_ius_sounding.bufr.
# This s/w runs on the NWS GTS BUFR input files. 
#--------------------------------------------------------------------------------
#
# The Perl script runs the bufr_dump executable to convert the NWS GTS BUFR "raw" data
...
#
# Execute: 
#    preprocess_NWS_GTS_BUFR.pl <raw_GTS_BUFR_dir> <ascii_output_dir> <preproc_output_dir>
#
# Examples: 
#    preprocess_NWS_GTS_BUFR.pl ../raw_gts_bufr_data/ ../output_ascii_data ../output_preproc_data
#
#    preprocess_NWS_GTS_BUFR.pl ../test_raw_data ../output_ascii_data ../output_preproc_data
#
# Input: 
...
#
# 2. ASSUMPTION: That the raw data are in expected NWS GTS BUFR format (binary)
#
...
#
# Updates: This version created in March 2022 to process NWS GTS BUFR data.
#
#--------------------------------------------------------------------------------
use strict;
...
sub main 
   {
   printf "\npreprocess_NWS_GTS_BUFR.pl began on ";print scalar localtime;printf "\n";
   ...
   if ($#ARGV < 2)
      ...
      print "Usage: preprocess_NWS_GTS_BUFR.pl <raw_GTS_BUFR_dir> <ascii_output_dir> <preproc_output_dir>\n";
      exit(1);
      }
   ...
   # Examples:
   #   my $INPUT_GTS_BUFR_DIR = "../raw_data";
   ...
   # The suffix is expected to be *.bufr for input files. 
   #----------------------------------------------------------------
   ...
   my @files = grep(/\.bufr$/,readdir($INPUT_DIR));
   closedir($INPUT_DIR);

   my $soundingCt = 1;

   #------------------------------------------------------------------------------------
   # Process every BUFR file (*.bufr) in the input directory.
   #
   # Expected form of NWS GTS BUFR Input file names: KSLC_202202150000_ius_sounding.bufr
   #------------------------------------------------------------------------------------
   foreach my $file (sort(@files)) 
       ...
       my @name_parts = split (/.bufr/, $file);    # HARDCODED
       ...
       # WAS: my $cmdbufrdump = sprintf("./bufr_dump %s/%s > %s", 
       #            $INPUT_GTS_BUFR_DIR, $file, $gts_ascii_file ); 
       #----------------------------------------------------------------
       ...
       if ($debug2) 
          ...
          print "DEBUG2:: Opening output file to save ALL SKIPPED input line into single output file = $skiplines_file\n";
    ...
    printf "\npreprocess_NWS_GTS_BUFR.pl ended on ";print scalar localtime;printf "\n";
    }
