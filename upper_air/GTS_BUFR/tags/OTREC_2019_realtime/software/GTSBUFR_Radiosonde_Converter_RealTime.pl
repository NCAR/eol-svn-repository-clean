...
sub parseHeader 
   ...
   if (($headerRecVal{blockNumber} > 0) && $headerRecVal{stationNumber} >0)
     ...
     if (index(lc($file), "nuqu") != -1) 
        ...
        {
        print "\nERROR/WARNING: Unexpected SITE NAME!  Not NUQU, STCZ, or LIMO! country = $country. Turn on DEBUG!\n";
        $site = "UNKNOWN";
        $country = " ";
        $messageID = " ";
        $debug = 1;
  ...
  if ($debug)
   ...
   if (defined($header)) 
     ...
     if (index(lc($file), "nuqu") != -1)
         ...
         {
         print "\nERROR/WARNING: Unexpected SITE NAME!  Not NUQU, STCZ, or LIMO! site = $site\n";
         }
