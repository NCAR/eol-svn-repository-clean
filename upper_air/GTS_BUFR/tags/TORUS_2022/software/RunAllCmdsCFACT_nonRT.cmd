How to process the CFACT 2022 NWS GTS BUFR files into ESC sounding format:
--------------------------------------------------------------------------

0. See dataset 100.030 and copy all required *.tar files. Tar -xvf the *.tar files and keep only the
   stations need to process for the project. For CFACT, those stations are KOI, KSLC, and KLKN (Elko, NV).
...
1. Convert incoming GTS BUFR NWS files to more readable form:
   preprocess_NWS_GTS_BUFR.pl ../raw_gts_bufr_data ../output_ascii_data ../output_preproc_data >& preproc_CFACT.log &

2. Convert more readable ascii (*.asc) form of input data to Class form. 
###      NWS_GTSBUFR_Radiosonde_Converter.pl  CFACT ../output_preproc_data ../output_esc 0 >& runCFACT.log &
