How to process the SWEX 2022 NWS GTS BUFR files into ESC sounding format:

0. See dataset 100.030 and copy all required *.tar files. Tar -xvf the *.tar files and keep only the
   stations need to process for the project. For SWEX, those stations are 
   XXXXXXX
...
1. Convert incoming GTS BUFR NWS files to more readable form:
    ...
    preprocess_NWS_GTS_BUFR.pl   ../raw_gts_bufr_data    ../output_ascii_data   ../output_preproc_data  >&  preproc_SWEX.log &


2. Convert more readable ascii (*.asc) form of input data to Class form. 

    NWS_GTSBUFR_Radiosonde_Converter.pl  SWEX_2022 ../output_preproc_data ../output_esc 0 >& runSWEX.log &
...
3. Strip out any recs with Time=-999.99. This step can be done before or after sorting the data in the step below. Probably best to do before Sort command in step 4 below.
   ...
   StripRecsMissTime.pl /net/work/Projects/SWEX/data_processing/upper_air/radiosonde/NWS_GTS_BUFR/output_esc/
