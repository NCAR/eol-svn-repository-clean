...
# specifically on data from soundings for the HIGHWAY 2019 project.
#
# July 2019:
# This s/w was modified from the GTSBUFR_Radiosonde_Converter.pl s/w to run
# specifically on data from 3 sounding sites for the OTREC 2019 project. 
#
...
#
# August 2019 - LEC updated s/w to process the sounding data for the HIGHWAY 2019 project.
#    Sites/SiteID/type: Nairobi,Kenya/HKNC/unknown.
#    This version of GTS BUFR processing does not "printStationFiles".
#
##Module------------------------------------------------------------------------
...
my $projectName = "xxx"; # HARDCODED. This is an input parameter.
...
sub new 
   ...
   print "ARGV Values: PROJECT: $ARGV[0] (projectName = $projectName), Input RAW_DIR: $ARGV[1], OUTPUT_DIR: $ARGV[2],  KeepEVSSrecs:  $ARGV[3]\n";
...
# WAS: Assumption: Input file name form/example: OTREC_STCZ_*20180307_1200*_1.preproc
# NOW: Assumption: Input file name form/example: HKNC_201908111200_ius_sounding_1.preproc
#     where [SiteID]_YYYYMMDDhhmm_ius_sounding_#.preproc
#     Note that "ius" stands for "i" = bufr, "u" = upperAir, "s" = sounding up to 100mb. 
#     '#' indicates the sounding count number found in the file. If two soundings are found
#     then the user will see one file with "_1.preproc" and another with "_2.preproc".
##----------------------------------------------------------------------------------
sub parseHeader 
   {
   ...
   if (($headerRecVal{blockNumber} > 0) && $headerRecVal{stationNumber} >0)
     ...
     # ---------------------------------------------------------------------------------------------------------
     my @filename_parts;
     my @filename_subparts;

     if ( lc($projectName) eq "otrec")
        {
        #------------------------------
        #  For OTREC, Costa Rica sites:
        #------------------------------
        @filename_parts = split /\./, $file;     # OTREC_NUQU_20171018_194122_RS41_1  preproc
                                                    # OTREC_LIMO_20190805_121400  bfr_1  preproc
                                                    # OTREC_STCZ_20190805_121400  bfr_1  preproc
                                                 
        @filename_subparts = split /\_/, $filename_parts[0]; # OTREC NUQU 20171018 194122 RS41 1
                                                                # OTREC LIMO 20190805 121400 
                                                                # OTREC STCZ 20190805 121400
        } # OTREC project - realtime processing
     elsif ( lc($projectName) eq "highway")
        {
        #--------------------------------------------------
        #  For HIGHWAY, Nairobi, Kenya, Africa (HKNC) site:
        #  HKNC_201908241200_ius_sounding_1.preproc
        #--------------------------------------------------
        @filename_parts = split /\./, $file;     # HKNC_201908241200_ius_sounding_1  preproc
        @filename_subparts = split /\_/, $filename_parts[0]; # HKNC 201908241200 ius sounding 1
        } # HIGHWAY project - realtime processing
     else
        {
        print "WARNING: projectName ($projectName) is UNKNOWN!\n";
        $filename_parts[0] = "UNKNOWN";
        $filename_subparts[0] = "UNKNOWN";
        }
     ...
     if (index(lc($file), "nuqu") != -1) 
     ...
     elsif (index(lc($file), "hknc") != -1)
        { # Nairobi, Kenya, IUS - Lake Victoria - project HIGHWAY
        $site = "Nairobi";
        $country = "Kenya";
        $messageID = "IUS";
        }
        ...
        {
        print "\nERROR/WARNING: Unexpected SITE NAME!  Not NUQU, STCZ, LIMO, or HKNC! country = $country. Turn on DEBUG!\n";
        $site = "UNKNOWN";
        $country = " ";
        $messageID = " ";
        $debug = 1;
  ...
  if ($debug)
   ...
   if (defined($header)) 
     ...
     if ( lc($projectName) eq "otrec")
        ...
        {
        print "projectName ($projectName) is UNKNOWN!\n";
        $filename_parts[0] = "UNKNOWN";
        $filename_subparts[0] = "UNKNOWN";
        }
     ...
     if (index(lc($file), "nuqu") != -1)
        ...
        $site = "Nairobi";
         ...
         {
         print "\nERROR/WARNING: Unexpected SITE NAME!  Not NUQU, STCZ, LIMO, or HKNC! site = $site\n";
         }
