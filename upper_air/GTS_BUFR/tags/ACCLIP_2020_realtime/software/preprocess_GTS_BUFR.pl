...
#
# 25 June 2020 - LEC
# WARNING: This code has been modified to process BUFR data from three Chinese sites
# where multiple soundings from different sites are combo'd into individual files. 
# This code tags off the "subsetNumber" key to detect the beginning of the next
# sounding. Note that using the software from previous projects failed because it
# detects the beginning of a the next sounding in a single file by finding the
# bufrHeaderCentre key. There are multiple bufrHeaderCentre records in the Chinese
# data files but not one bufrHeaderCentre key per sounding. 
# Note that the following stations where processed for the ACCLIP project. 
# Search for ACCLIP throughout this code to find the modifications. 
#    57127 - Hanzhong, China, station number 127 (in dumped *.asc file)
#    57131 - Jinghe, China, station number 131 
#    58362 - Hanzhong, China, station number 362
#
# Also note that these Chinese files are *.bufr files and not *.bfr raw input file
# names so that was also updated.
#
# 11 July 2019 - LEC
...
# by another piece of conversion software. This script only does pre-processing of the 
...
#
# 5.1 IMPORTANT:: ACCLIP Change: For Chinese sites, the subsetNumber key was used to detect
#                 the beginning of a sounding.  Assumes input files are *.bufr
#                 files not *.bfr.
#
...
sub main 
   ...
   my @files = grep(/\.bufr$/,readdir($INPUT_DIR));   # ACCLIP Change .bfr to .bufr
   closedir($INPUT_DIR);
...
###       my $cmdbufrdump = sprintf("./bufr_dump %s/%s > %s", $INPUT_GTS_BUFR_DIR, $file, $gts_ascii_file ); # HARDCODED
       my $cmdbufrdump = sprintf("/opt/local/bufr/bin/bufr_dump %s/%s > %s", $INPUT_GTS_BUFR_DIR, $file, $gts_ascii_file ); # HARDCODED
       ...
       foreach my $line (@lines)
         ...
         #
         # WARNING:
         # Found that some soundings had "ERROR" &/or "ERROR: unreadable message"
         # in the data. Found on the bufrHeaderCentre line which is believed
         # to be the beginning of a new sounding. This s/w still generates
         # the sounding (if there are any data recs) to output. If there are
         # no data records an empty file (.preproc) files is generated so
         ...
         # -----------------------------------------------------------------------
         ## my $soundingTop = "bufrHeaderCentre";  # HARDCODED - Used for all other projects to detect begin of next sonde in file
         my $soundingTop = "subsetNumber";  # HARDCODED - for ACCLIP only to detect beginning of next sounding in file
         ...
         if ( index($line, $soundingTop) != -1)  # Found first record in a sounding
            ...
            if ($soundingCt > 100) {print "ERROR: Exiting due to >100 soundings in a single input file! exit(1)\n"; } # HARDCODED - random limit of 100 set to prevent run away number of soundings in a file  - ACCLIP change from 10 to 100. At least 85 diff stns potentially in ACCLIP raw input files. 
         ...
         #-------------------------------------------------------------------
         ## Was for all other projects: my $system_cmd_return = `grep bufrHeaderCentre $gts_ascii_file | wc -l`; #HARDCODED backticks in cmd
         my $system_cmd_return = `grep subsetNumber $gts_ascii_file | wc -l`; #HARDCODED backticks in cmd
         chomp ($system_cmd_return);  # Cut off \n at end of returned value

         ## Was for all other projects: if ($debug) {print "Execute GREP cmd:: xxx grep bufrHeaderCentre  $gts_ascii_file | wc -l xxx\n";}
         if ($debug) {print "Execute GREP cmd:: xxx grep subsetNumber  $gts_ascii_file | wc -l xxx\n";}
