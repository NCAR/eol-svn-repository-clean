...
##Module------------------------------------------------------------------------
# WARNING: This s/w has NOT BEEN COMPLETELY TESTED and is only set to run on
# realtime data processing. The nominal time is set to be the actual time which
# will NOT work for postprocessing the ACCLIP GTS BUFR (Chinese) data!! The 3 sites
# expected to be processed were/are: 
#    57127 Hanzhong, China (stn 127)
#    57131 Jinghe, China   (stn 131)
#    58362 Shanghai, China (stn 362)
#
# WARNING: The output ESC data for each sounding MUST BE SORTED as it is a mix of times
# with several sections (repeating) the t=0 time. SL believe this is a result of the
# "TAC" messages and the switch to BUFR.
#
# For processing non-RT data after the project, the s/w must be updated to determine
# the correct nominal time. The nominal time is normally taken from the "typical" date/time
# parameters. This does not work since the input data has multiple soundings in a
# single file with the typical date/time only once in an input file that has multiple
# soundings.  Either this s/w must be updated or the preproc s/w must be updated. Neither
# has been done in this version of the data because the RT output only uses the actual
# date/time. 
#
#-------------------------------------------------------------------------------
...
#
# ***This s/w was modified to handle specific ACCLIP Bufr data issues. 
# It was modified to process 3 Chinese sounding sites for the ACCLIP project.
# Search for ACCLIP to find the modifications and notes. ***
#
# ACCLIP IMPORTANT: Search for ACCLIP in this code for the changes.  Note that the ACCLIP Chinese
# sites are in *.bufr files which contain dozens of soundings from dozens of
# different sites in a single file.  This code has been updated in several places
# to handle the oddities of the ACCLIP Chinese data. ** Must run this s/w with the 
# extendedVerticalSoundingSignificane flag (65536) set to one (1). Otherwise the
# mandatory and significant sounding levels are dropped. The Chinese data we have
# received are only mandatory and significant levels!  Also note that the data
# is very mixed in time and there are multiple timePeriod = 0 values. The output
# ESC data MUST be SORTED!
#
...
#
# 1. preprocess_GTS_BUFR.pl [BUFR input dir] [BUFR DUMP ASCII output dir] [PREPROC ASCII output dir] >& preproc_ACCLIP.log &
#
# 2. GTSBUFR_Radiosonde_Converter_RealTime.pl [project] [PREPROC ASCII output dir] [ESC ASCII output dir] 1 >& GTSBUFR_ACCLIP.log &
#
...
#
# 11. This s/w only recognizes specific countries/continents (e.g., Australia, New Zealand, 
...
#     For ACCLIP, set this flag to one (1) to keep mandatory and significant records.
#
...
#
# 25 June 2020 - LEC  Mods for the ACCLIP Chinese sites.
# ACCLIP WARNING: This code has been modified to process BUFR data from three Chinese sites
# where multiple soundings from different sites are combo'd into individual files.
# The preprocessing step divides the combo'd data files into individual sounding files
# so this software does not handle that. 
#
# ACCLIP: It was also determined that the preprocessing code needed to tag off the "subsetNumber" 
# key to detect the beginning of the next sounding. Previous versions of softwared used the 
# bufrHeaderCentre key to find the beginning of the next sounding. This s/w needs to also
# needs to pick out the stationNumber and place that into the output file name for the plotting
# code (run separately).
# Note that the following stations where processed for the ACCLIP project. Search for ACCLIP 
# throughout this code to find the modifications.
#    57127 - Hanzhong, China, station number 127
#    57131 - Jinghe, China, station number 131
#    58362 - Hanzhong, China, station number 362
#
##Module------------------------------------------------------------------------
...
sub parseHeader 
   ...
   #
   #  ACCLIP: Added shipOrMobileLandStationIdentifier key, e.g.,
   #  "shipOrMobileLandStationIdentifier", "value" : "SHANGHAI"
   #
   # --------------------------------------------------------
   ...
   #--------------------------------------------------------
   if ($debug) {print "Debug88 - typicalYear is xxx $headerRecVal{typicalYear} xxx\n";} 
   ...
   if (($headerRecVal{blockNumber} > 0) && $headerRecVal{stationNumber} >0)
     ...
     #
     #
     # ACCLIP: The Chinese ACCLIP data files do/did not have the station code/name in the file name. Must pick
     #    out the site name from the "shipOrMobileLandStationIdentifier" key.  Some of the next section of code
     #    not necessarily needed to ACCLIP but left in if data changes. 
     #
     # ---------------------------------------------------------------------------------------------------------
     my @filename_parts = split /\./, $file;     # OTREC_NUQU_20171018_194122_RS41_1  preproc
                                                 ...
                                                 #
                                                 # ACCLIP: BABJ_201910240000 iuk_sounding bufr_1 preproc
     ...
     if (index(lc($file), "babj") != -1)  # ACCLIP section
        { # Chinese site.
        $country = "China";

        $site = $headerRecVal{shipOrMobileLandStationIdentifier}; # Strip off quotes
        $site =~  s/"//g; # Strip off begin/ending quote found on header recs.

        if ($site eq "null") {$site = "UnknownChineseSite";}

        if ($debug) {print "\nsite xxx $site xxx\n";}

        if ($site eq "Hanzhong")  # ACCLIP
           {
           $messageID = "57127";   
           }
        elsif ($site eq "Jinghe") 
           {
           $messageID = "57131";
           }
        elsif ($site eq "Shanghai")
           {
           $messageID = "58362";
           }
        else # Put space for "UnknownMessageID"
           {
           $messageID = "";
           }

        } # end ACCLIP Site settings

     elsif (index(lc($file), "nuqu") != -1) 
   ...
   #
   # ACCLIP additions to code table below. 
   # ------------------------------------------------------------------
   my %radiosondeType_CODE_TABLE = (
     ...
     131 => "Taiyuan GTS1-1/GFE(L)",
     132 => "Shanghai GTS1/GFE(L)",
     133 => "Nanjing GTS1-2/GFE(L)",
     ...
     197 => "Unknown Chinese Radiosonde Type",
     211 => "Unknown Chinese Radiosonde Type",
   ...
   # Set Nominal Date
   # ------------------
   my $date_Year = trim(eval($headerRecVal{typicalYear}));
   my $date_Month = trim(eval($headerRecVal{typicalMonth}));
   my $date_Day = trim(eval($headerRecVal{typicalDay}));
...
#     extendedVerticalSoundingSignificance => 1,
  ...
  if ($debug2)
   ...
   # ACCLIP - Not all files have Typical date/times so initialize here.
   $headerRecVal{typicalYear} = 1900;
   $headerRecVal{typicalMonth} = 12;
   $headerRecVal{typicalDay} = 30;

   $headerRecVal{typicalHour} = 23;
   $headerRecVal{typicalMinute} = 59;
   $headerRecVal{typicalSecond} = 59;
  ...
  if ($debug)
   ...
   if (defined($header)) 
      ...
      my @filename_parts = split /\./, $file;   # OTREC_NUQU_20190805_121214_RS41_1  preproc
                                                # OTREC_STCZ_20190805_121400   bfr_1   preproc
                                                # OTREC_LIMO_20190805_121400   bfr_1   preproc
                                                # ACCLIP: BJ_202006241200_iuk_sounding bufr_78 preproc
                                                #
...
#                                                              # ACCLIP: BABJ_201910240000  - Not what's wanted.
#      my $fileArrayLength = scalar (@filename_subparts); - non ACCLIP

      # ------------------------
      # For ACCLIP Chinese Sites
      # ------------------------
      my $fileArrayLength = scalar (@filename_parts); # BJ_202006241200_iuk_sounding bufr_78 preproc
      my @filename_subparts = split /\_/, $filename_parts[$fileArrayLength-2]; # ACCLIP bufr_1 gives "bufr 78"

      my $fileArrayLength = scalar (@filename_subparts);
      my $fileSuffixCt = $filename_subparts[$fileArrayLength-1];

      if ($debug) {print "\n(22) filename_parts xxx @filename_parts xxx\n";}
      if ($debug) {print "\n(22) filename_subparts xxx @filename_subparts xxx\n";}
      if ($debug) {print "(22) fileSuffixCt = $fileSuffixCt\n";}
     ...
     if (index(lc($file), "babj") != -1)  # ACCLIP
        { # Chinese site.
        if ($debug) {print "Found ACCLIP Chinese Site.\n";}         
        $site = $headerRecVal{shipOrMobileLandStationIdentifier};
        $site =~  s/"//g; # Strip off begin/ending quote found on header recs.
        if ($site eq "null") {$site = "UnknownChineseSite";}
