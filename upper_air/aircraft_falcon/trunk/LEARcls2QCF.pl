...
#@files = grep(/^D\d{8}\_\d{6}QC\.cls\.new11(|\.gz)$/i,@ARGV);
@files = grep(/^\d{12}\.txt(|\.gz)$/i,@ARGV);
foreach $file(@files){
...
sub filename{
	...
	($year,$mth,$day,$hour,$min,$sec) = ($1,$2,$3,$4,$5,$6);
        $mth = sprintf("%02d",$mth);
    }elsif($$file_ref =~ /^(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})\.txt/){
        ($year,$mth,$day,$hour,$min) = ($1,$2,$3,$4,$5);$sec = "00";
        $mth = sprintf("%02d",$mth);
    }# end if-elsif
    $month = &findmth($mth);
    if($month ne "NULL"){
