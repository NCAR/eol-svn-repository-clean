...
#        WHICH ONE SHOULD BE USED? </p>
#
# BEWARE: User must updated project_id for current project.
#
# @author Joel Clawson
# @version 1.0 It was adapted from the Ron Brown conversion script written
#    by Darren R. Gallant and named RON_BROWN.pl March 8th, 1999.
#
# @author L. Cully
# @version Updated Sep/Oct 2008 by L. Cully. Added header info, comments, etc.
#     Added some informational output statements. Added spacing to make 
#     more readable. Updated references from Ron Brown to be Seward Johnson.
#     More work on the internal documentation needs to be done where there's
#     more time.
#-----------------------------------------------------------------------------
use POSIX;
...
#-----------------
#----------------------------------------------------------------------------
# Commented out by L. Cully because not used. 
# These constants are redefined below internal to sub.
#$wind_tol = 0.0001;
#$degree = 1/$radian;
#----------------------------------------------------------------------------
$pi = 3.14159265;
...
$header[5] = "Radiosonde Serial Number:          ";
...
if(@ARGV < 1){
  print "Usage is seward_johnson.pl file(s)\n";
  exit;
}
...
#---------------------------------------------------------------
#BEWARE: Lines commented out by either J. Clawson or D. Gallant.
#---------------------------------------------------------------
#		    if($rh >= 0 && $rh <= 100 && $dewpt < 1000 && $dewpt > -99.9){
			$outline[4] = $rh; # relative humidity in percent
			$outline[17] = 99;# unchecked Qrh           
			$outline[3] = $dewpt if ($dewpt !~ /\/+/); # DewPt in Celsius
		    if ($outline[3] == 999) { $outline[3] = calc_dp($outline[2],$outline[4]); }
#		    }# end if
	...
	if($WND_FLAG){
		...
		if ($line =~ /RS\-number\s+:\s+(\d+)/) {
		    $INFO{$TIMESTAMP}{"RS-NUM"} = $1;
		}
		if($line =~ /Location/){
		    &location($line);
...
foreach $time (keys %TIMES){
	...
	$outfile = $INFO{$time}{"FILE"};
        unless(open(OUT,">../output/$outfile")){
        }else{
...
sub filename{
	...
	$INFO{$TIMESTAMP}{"GMT"} = "$year, $mth, $dd, $hh:$min:00";
        ...
        $INFO{$TIMESTAMP}{"FILE"} = sprintf("%s_%04d%02d%02d%02d%02d.cls",$stn_id,$year,$mth,$dd,$hh,$min);
	if($hh == 0){
	    $nominal = "00:00:00";
	}elsif($hh > 0 && $hh <= 3){
	    $nominal = "03:00:00";
