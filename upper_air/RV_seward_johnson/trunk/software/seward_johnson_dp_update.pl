#! /usr/bin/perl -w
# program seward_johnson_dp_update.pl
#-----------------------------------------------------------------------------
...
# there appears to be two versions of the Darren Gallant program. This one
# with an updated dewPt calculation.
#
# BEWARE: There appears to be two versions of the Darren Gallant program.
#       This one with an updated dewPt calculation.
...
if(@ARGV < 1){
  print "Usage seward_johnson_dp_update.pl file(s)\n";
  exit;
}
	...
	if($PTU_FLAG){
		...
		if($line =~ /Location/){
			...
			$outline[17] = 99;# unchecked Qrh           
			    ...
			    if ($dewpt =~ /[\/]+/) { 
				$outline[3] = calc_dewpt($temp,$rh);
			    } elsif ($dewpt < 1000 && $dewpt > -99.9) {
				$outline[3] = $dewpt;
			    }
...
print "\nFINI - End seward_johnson_dp_update.pl\n\n";
