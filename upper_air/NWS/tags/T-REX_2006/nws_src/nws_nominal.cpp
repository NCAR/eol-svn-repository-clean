...
void check_met_values(ofstream& log_file, OBSER *data, int temp_interp, int rh_interp, float term_alt) {
  ...
  if (data->time < 300 && data->alt > term_alt) {
    sprintf(outline,"Altitude at press: %6.1f is: %7.1f m.\n",data->press,data->alt);
    log_file << outline;
    data->alt = 99999.0;
  }
}
...
int write_header_data(ofstream& out_file, ofstream& log_file, HEADER *header) {
  ...
  sprintf(out_file_name,"%s_%04d%02d%02d%02d00.cls",header->call_letters,
	  header->nom_rel_year,header->nom_rel_month,header->nom_rel_day,header->nom_rel_hour);
