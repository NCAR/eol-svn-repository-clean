...
sub processTxtFile {
    ...
    open(my $HEAD,sprintf("%s/H%d",getRawDirectory(),$asc_num)) or die("Cannot open header file for $asc_num\n");
    my $headLine = <$HEAD>;
    close($HEAD);
    ...
    my $header = Sounding::ClassHeader->new($WARN_LOG);

    if ($stn_id eq "CHS") {
	$header->setSite("CHS Charleston, SC");
    } else {
	die("Unknown station id: $stn_id\n");
    }
    ...
    $header->setLine("Ascension Number:",substr($headLine,38,4));
    $header->setLine("Sonde Manufacturer:",$manufact->{substr($headLine,49,3)});
    $header->setLine("Sonde Type:",$type->{substr($headLine,52,3)});
    $header->setLine(substr($headLine,55,1) ? "Baroswitch Number:" : "Sonde Serial Number:",
	...
	if ($data[1] != 0 || $data[2] != 0 || $data[3] != 0) {
		...
		my $txtRec = Sounding::ClassRecord->new($WARN_LOG,$filename);
    ...
    my @allRecords = ();
    my @noPressRecords = ();
    open(my $TFILE,sprintf("%s/T%d",getRawDirectory(),$asc_num)) or die("Cannot open TFILE for $asc_num");
    foreach my $line (<$TFILE>) {

	my $tRec = Sounding::ClassRecord->new($WARN_LOG,$filename);
	#$tRec->{"time"} = undef();
	...
	while ($record->getAltitude() < $rec->getAltitude()) {
	...
	if ($record->getAltitude() == $rec->getAltitude()) {
