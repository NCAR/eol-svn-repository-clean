...
public class RRSSoundingConverter {
    ...
    public static final String PROJECT_ID = "BNL_ARM";
	...
	for (File file: metadataFiles) {
		...
		try { 
		    ...
		    log.printf("Unable to successfully parse the metadata file %s.  ParseException:\n%s\n", 
		...
		if (passed) {
			...
			log.printf("Unable to successfully parse the PTU file %s.  ParseException:\n%s\n", 
		...
		if (passed) {
		    try {
			parseWindFile(new File(file.getParentFile(), file.getName().replace("1Meta", "6pGPS")), sounding);
		    } catch (ParseException e) {
			log.printf("Unable to successfully parse the GPS file %s.  ParseException:\n%s\n",
