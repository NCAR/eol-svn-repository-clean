...
my $AUTOQC = "/work/software/RAINEX/library/upper_air/autoQC/bin/nws_autoqc_RAINEX";
my $AUTOQC_CHS = "/work/software/RAINEX/library/upper_air/autoQC/bin/autoqc_RAINEX_5point_avg";
...
sub main {
    ...
    foreach my $file (grep(/\.cls/,readdir($IN))) {
      ...
      system(sprintf("%s %s/%s %s/%s",$file =~ /^CHS_/ ? $AUTOQC_CHS : $AUTOQC,$INPUT_DIR,$file,$OUTPUT_DIR,$file));
