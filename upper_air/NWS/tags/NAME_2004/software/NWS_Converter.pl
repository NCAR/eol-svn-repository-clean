...
#my $VAISALA = "/usr/bin/nice -19 nwsVaisala2000.1";
#my $VIZ = "/usr/bin/nice -19 nwsVIZ2000.2";
my $VAISALA = "/usr/bin/nice -19 ../nwsconvert/bin/nwsVaisala";
my $VIZ = "/usr/bin/nice -19 ../nwsconvert/bin/nwsVIZ";
...
#	processFile($file) if ($file =~ /^23047.*\.07\.gz$/);
#	processFile($file) if ($file =~ /^03160.*\.06\.gz$/);
    }
...
sub correct_winds {
	...
	my $new_file = createFileName($file);
	my $outfile = sprintf("%s/%s.cls",getOutputDirectory(),$new_file);
	...
	my $wndfile = sprintf("%s/%s.wnd",getLogDirectory(),$new_file);
	my $statfile= sprintf("%s/%s.stat",getLogDirectory(),$new_file);
...
##-----------------------------------------------------------------------
# @signature String createFileName(String old_name)
# <p>Convert an old style CLASS file name into a more descriptive name.</p>
#
# @input $old_name The original CLASS file name.
# @output $new_name The new file name.
##-----------------------------------------------------------------------
sub createFileName {
    my $old_name = shift;

    # Get the nominal release time from the file.
    open(my $FILE,$old_name);
    my @line = grep(/Nominal Release Time/,<$FILE>);
    close($FILE);

    # Get the nominal date.
    my @date = split(/, /,(split(/:/,$line[0]))[1]);

    # Get the station id from the file name.
    $old_name =~ /^([A-Z]+)\d+\.cls$/;

    # Define the new file name.
    return sprintf("%s_%04d%02d%02d%02d00",$1,@date);
}
