...
# 30 October 2009 - L. Cully
#   Updated project to process T-PARC_2008 data.
#   Updated stateCode to be 99 for missing. In T-PARC
#   several of the stns are not in the U.S., so just set
#   the country and state codes to Missing. Checked into
#   svn after START08. T-PARC is the latest version of s/w.
#
# 8 October 2010 - L. Cullly
#   Updated project to process VORTEX2_2010 data.
#
...
sub getProjectName() { return "VORTEX2_2010"; }  # HARDCODED Project Name
	...
	for my $line (<$FILE>) {
		...
		if ($line =~ /Release Site Type/) {
		    ...
		    my @data = split(' ',$line);
                    ...
                    # This section needs to be reworked to handle the world, if possible. For now,
                    # set the Country and State codes to missing or 99.
                    # PR = Puerto Rico.
                    #------------------------------------------------------------------------------
                    # Orig Code:
...
sub processTxtFile {
	...
	$station->setStationName($header->getSite());
        ...
        # Check HERE all info for station. 
        # Issue with country and state codes now that processing data from whole world.
        # Set the Country and State codes to be missing or 99.  Original code was 
        # setting the state to XX. Should be 99 for missing.
        # PR = Puerto Rico.
        #------------------------------------------------------------------------------
        # Orig Code:
        $station->setCountry($data[-1] eq "PR" ? $data[-1] : "US");  
        $station->setStateCode($data[-1] eq "PR" ? "99" : $data[-1]);
        $station->setStationId($stn_id);
        $station->setStationName($header->getSite());
        #------------------------------------------------------------------------------
