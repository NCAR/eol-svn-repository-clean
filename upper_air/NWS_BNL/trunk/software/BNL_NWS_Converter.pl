...
#########################################################
#BNL_NWS_Converter.pl
#Header added by: Ben Golden
#Date: 10/28/08
#
#The most recent work on this software was done by Joel
#Clawson. This software is used in converting MicroART
#ASCII files into ESC ASCII files. It should be noted
#that this s/w relies on library dependencies that are
#native to Solaris. It MUST be run from a Solaris OS.
#
#This copy of the s/w is currently in use for BNL. If
#the software is to be used with any other project the
#sub getProjectName() should be altered to return the
#appropriate project name instead of BNL.
#
#For further details regarding the use of this software
#please refer to the "Process NWS BNL Sounding Data page"
#in Plone. This document can currently be found at:
#
#http://www.eol.ucar.edu/about/our-organization/cds/
#dmg/dmg-internal/dmg-documentation/how-to/
#how-to-process-nws-bnl-sounding-data
#
#########################################################
...
#----------------------------------------------------------
my $VAISALA = "/usr/bin/nice -5 ./nwsVaisala"; 
my $VIZ = "/usr/bin/nice -5 ./nwsVIZ";        
...
sub getProjectName() { return "BNL_ARM"; }  # HARDCODED Project Name
...
sub main {
    ...
    #----------------------------------------
    if ($debug){ print "Process all the files in the Raw dir.\n"; }
...
sub convert {
	...
	# Convert the data to the CLASS format.
        # Define the programs that convert the data to CLASS format.
        # 
        # BEWARE: the following executables MUST be made with the
        #  nws_src/MakeNWS_BNL file!!!!! This is CRITICAL so that
        #  the executables will include the code that will produce
        #  output files with nominal dates in the names.
        #
...
sub processTxtFile {
    ...
    my $ATime = substr($headLine,34,4);
    my $NDate = substr($headLine,24,8);
    my $NTime = substr($headLine,32,2);
    print "LEC:: Actual Release  Date (YYYYMMDD) Time (HHMM)= $actDate  $ATime\n";
    print "LEC:: Nominal Release Date (YYYYMMDD) Time (HH) = $NDate $NTime\n";
...
####HERE    my $filename = sprintf("%s_%04d%02d%02d%02d%02d.cls",$stn_id,split(", ",$header->getActualDate()),
#####			   split(":",$header->getActualTime()));

my $filename = sprintf("%s_%04d%02d%02d%02d%02d.cls",$stn_id,split(", ",$header->getNominalDate()),
