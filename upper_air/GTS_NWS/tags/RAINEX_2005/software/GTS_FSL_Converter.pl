...
sub printSounding {
	...
	$station->setStationName(sprintf("%s Slidell, LA",$header->getId(),$header->getSite()));
...
sub readRawFile {
	...
	if ($data[0] == 254) {
	    ...
	    if ($header->getNominalTime() =~ /^00:00/ && $data[6] > 2000) {
