...
##Module------------------------------------------------------------------------
# <p>The GTSBUFR_Radiosonde_Converter.pl script is used for converting 
...
#
# Execute command: 
#    GTSBUFR_Radiosonde_Converter.pl [project] [input dir] [output dir] [Keep EVSS Recs Flag]
#
...
#
# Examples:
#    GTSBUFR_Radiosonde_Converter.pl SOCRATES ../raw_data ../output 0 >& run2.log &
#    GTSBUFR_Radiosonde_Converter.pl SOCRATES ../raw_data/australia ../output/australia 1 >& runAUS.log &
#    GTSBUFR_Radiosonde_Converter.pl SOCRATES ../raw_data/antarctica ../output/antarctica 1 >& runANT.log &
#    GTSBUFR_Radiosonde_Converter.pl SOCRATES ../raw_data/new_zealand ../output/new_zealand 1 >& runNZ.log &
#
...
#
# Feb 2023 - LEC Updated to process SWEX 2022 BUFR data that was FTP'd during 
#    during the field phase. This s/w is NOT like the realtime SWEX version 
#    which was more simple in some ways.  This current non-realtime version
#    brings together all the logic developed during the RELAMPAGO project which
#    allows the user to drop "double" mandatory versus significant records based
#    on flag input. This version has also been updated to recognize the SWEX 2022
#    Santa Barbara Fire Dept Stations which are SBFDHQ (Head Quarters) and 
#    SBFS38 (Station 38). There was also a SBFS18 (Station 18) who collected data
#    before the field phase but that data was not included in the final processed
#    data.   ** IMPORTANT NOTE: During the field phase and near the very end of that
#    time, the Headquarters equipment was moved to Montecito, CA BUT the input
#    file names were not changed to indicate Montecito. The lat/lons in the data
#    do change so that can be used to determine when the site moved to Montecito.**
#
#    There was one input SBFS38 sounding file for SWEX that had a lat of 90.0 and lon of 0.00.
#    This was reset to match the lat/long of the SBFS38 station and it HARDCODED. This
#    must be done else the lat/lon offsets in the preproc file generate bad lat/lons
#    in the output file. 
#
#    This is the s/w version used to convert the SWEX 2022 BUFR data. It brings
#    together the updates applied for the RELAMPAGO project and SWEX. This is the
#    best version to convert general BUFR data to ESC. Start with this code and
#    modify for the current project. 
#
##Module----------------------------------------------------------------------------
package GTSBUFR_Radiosonde_Converter;
use strict;
...
printf "\nGTSBUFR_Radiosonde_Converter.pl began on ";print scalar localtime;printf "\n";

my $debug = 0; 
my $debug2 = 0; # detailed debug

my $KeepEVSSrecs = 1; # User defined Command Line Flag. Initialize to keep mandatory recs. # HARDCODED
my $found_montecito = 0;
my $StnBadLatLon = "SWEX_SBFS38_DFM09_bufr0309052_20220424_015950_1.preproc"; # HARDCODED for SWEX 2022 only

&main();
printf "\nGTSBUFR_Radiosonde_Converter.pl ended on ";print scalar localtime;printf "\n";
...
sub main 
   {
   my $converter = GTSBUFR_Radiosonde_Converter->new();
   $converter->convert();
   } #main()

##------------------------------------------------------------------------------
# @signature GTSBUFR_Radiosonde_Converter new()
# <p>Create a new instance of a GTSBUFR_Radiosonde_Converter.</p>
#
# @output $self A new GTSBUFR_Radiosonde_Converter object.
##------------------------------------------------------------------------------
sub new 
   {
   my $invocant = shift;
   ...
   # --------------------------------------------------------------
   # NETWORK is normally hard coded but for SWEX the
   # following is reset once the station site is known.
   # --------------------------------------------------------------
   # Instead of BUFR, for SWEX use: UCSB for HQ and Montecito sites
   # and use SJSU for for fire station 38.
   # ---------------------------------------------------------------
   $self->{"NETWORK"} = "BUFR Sounding Data/Ascending"; # HARDCODED Default
...
sub buildDefaultStation 
   ...
   #
   # --------------------------------------------------------------------------------------
   $station->setStationName("BUFR");  # HARDCODED - SB new
...
#
# Assumption: Input file name form/example: 
#   Now for SWEX_2022:  SWEX_SBFS38_DFM09_bufr0309052_20220510_200001_1.preproc
#
#   WAS for another project: 201803071200.casey_aq_sounding_1.preproc
#
##----------------------------------------------------------------------------------
sub parseHeader 
   {
   ...
   # --------------------------------------------------------------------
   # THIS SECTION UPDATED FOR SWEX Handling. There were errors during the
   # field phase that caused the wrong values to be put in the BUFR files
   # for the shipOrMobile key value. So, this must be set based on file
   # name. The last 5 HQ files actually have Montecito data. All of the
   # SBFS38 data files have SBFS18 indicated in them and that's wrong so
   # these problems are translated throughout the code. 
   #
   # For SWEX, reset the NETWORK according to the
   # input station. Instead of BUFR in header
   # Data Type line, use UCSG for SBFDHQ & Montecito
   # and use SJSU for station SBFS38.
   #
   # Note that HQ and Montecito input files all have sbfdhq in file name.
   # The data source did NOT correctly indicate Montecito in the files or
   # in the file names. 
   # --------------------------------------------------------------------
   my $dataType = "Unknown";
   my $ReleaseDirection = "Ascending";

   if (index(lc($file), "sbfdhq") != -1) # Should catch both HQ and Montecito stations
      {
#WAS     $self->{"NETWORK"} = "UCSB Sounding Data/Ascending";

      $dataType = "UCSB Sounding Data";
      if (1) {print"Updated NETWORK to be UCSB Sounding Data. WARNING! May need to check for Montecito \n";}
      }
   elsif (index(lc($file), "sbfs38") != -1) # Station SBF 38
      {
#WAS      $self->{"NETWORK"} = "SJSU Sounding Data / Ascending";

      $dataType = "SJSU Sounding Data";
      if (1) {print"Updated NETWORK to be SJSU Sounding Data \n";}
      }
   else
      {
#WAS      $self->{"NETWORK"} = "BUFR Sounding Data / Ascending";

      $dataType = "BUFR Sounding Data";
      if (1) {print"Kept NETWORK as default of BUFR Sounding Data. \n";}
      }

#Was:   $header->setReleaseDirection("Ascending"); # HARDCODED

   $header->setType($dataType);
   $header->setReleaseDirection($ReleaseDirection);
   ...
   # ------------------------------------------------------
   $header->setId("SBFD");  #HARDCODED - Santa Barbara Fire Department   # HERE - Initialize only. Assigned later.
   ...
   #  shipOrMobileLandStationIdentifier,  - May or may not be used depending on the project.
   ...
   #--------------------------------------------------------
   # It's possible that stations have  missing block# and stn# 
   # so can't form WMO ID as expected. Pick stn name and use 
   # HARDCODED elements to set accordingly. NWS likely has
   # info to form WMO ID.
   #
   #--------------------------------------------------------
   # The blockNumber and stationNumber were always Null for
   # the Santa Barbara Fire Dept Bufr data. 
   #--------------------------------------------------------
   if ($headerRecVal{blockNumber} eq "null") {$headerRecVal{blockNumber} = 999; if ($debug) {print "blockNumber is null! Reset to 999.\n";} }
   if ($headerRecVal{stationNumber} eq "null") {$headerRecVal{stationNumber} = 999; if ($debug) {print "stationNumber is null! Reset to 999.\n";}}

   my $BNstr = $headerRecVal{blockNumber};   # Null/Missing for SWEX 2022 Fire Dept Stations.
   my $SNstr = $headerRecVal{stationNumber}; # Null/Missing for SWEX 2022 Fire Dept Stations.

   if (1) {print "BNstr = xxx $BNstr xxx, SNstr = xxx $SNstr xxx\n";}

   #Changed following check because it was a combo of integer and string checks on same values. # HERE
   if (($BNstr > 0) && ($SNstr >0) && ($BNstr != 999) && ($SNstr != 999))
     {
     if (1) {print "\nLegit Values - Not Zero/Not Missing (999); BNstr = xxx $BNstr xxx ; SNstr = xxx $SNstr xxx\n";}

     # --------------------------------------------------------------------------------------------------
     # Extract site name from file name. ASSUMPTION file name format: 
     # --------------------------------------------------------------------------------------------------
     # WAS:    201803071200.casey_aq_sounding_1.preproc
     #
     # WAS:  Note that station name may be longer than single word so must count from the end of word to 
     #       find all site name parts. E.g., 201712251200.macquarie_island_au_sounding_1.preproc, 
     #       201803071200.lord_howe_island_au_sounding_1.preproc. 
     #
     #  ***Code below needs to be checked/verified to work.***
     # --------------------------------------------------------------------------------------------------
     # But Note that the SWEX 2022 data files have the Site ID is in the input file name, but no
     # blockNumber or stationNumber. See "else" section below for SWEX.
     # --------------------------------------------------------------------------------------------------
     my @filename_parts = split /\./, $file; #  201803071200.casey_aq_sounding_1.preproc 
     my @filename_subparts = split /\_/, $filename_parts[1];  # casey aq sounding 1 
                                                             
     if ($debug) {print "\nInput file xxx $file xxx\n"; print "\nfilename_parts xxx @filename_parts xxx\n"; print "\nfilename_subparts xxx @filename_subparts xxx\n";} 

     #--------------------------------------------------------------
     # Was: Non-US projects with Files for non-US sites had country 
     #       in the file names that could be extracted. 
     #--------------------------------------------------------------
     my $fileArrayLength = scalar (@filename_subparts); 
     my $locCountry = $fileArrayLength-3;  # HARDCODED. Must count from end of array to find country. Varying length name.
     my $country = "xxx";
     $country = $filename_subparts[$locCountry];  # aq

     my $messageID = " ";                 # Used for Realtime processing
     ...
     # Form Site name. Can be multiple words long.
     ...
     #
     # Form site name as "site, country / messageID / WBAN#
...
#----------------------------------------------------------------------------------------------
     ...
     {
     if (1) {print "\nMissing or Zero Block and Station Numbers: BNstr = xxx $BNstr xxx ; SNstr = xxx $SNstr xxx\n";}

     # ---------------------------------------------------------------------
     # SWEX-2022 Fire Stations following this "else" path. No blockNumber
     # or stationNumber for SWEX 2022 sites.
     # ---------------------------------------------------------------------
     # Note for Realtime processing of SWEX 2022 data:
     # SWEX_SBFS38_DFM09_bufr0309052_20220510_200001_1.bfr
     #
     # SWEX Sites: SBFDHQ, SBFS18, ISS2, ISS3 where SB sites are GRAW and ISS sites are Vaisala.
     #  Realtime Sample:  SWEX_ISS2_RS41_bufr309052_20200131_211838.bfr
     #
     #------------------------------------
     # Form Site Name - HARDCODED for SWEX
     #------------------------------------
     my $site = "";
     my $country = "";
     my $messageID = "";

     if (1) {print "Missing Block/StnNumbers. Find stn info in input file name. file = $file \n"}

     my $siteEnd = "UNKNOWN Site";
     if (index(lc($file), "sbfdhq") != -1) # Santa Barbara, CA - Fire Station Headquarters
        {
        print "Found SantaBarbara_Fire_Dept_HQ.  File = $file\n";
        ...
        # --------------------------------------------------------------------------------------
        # WARNING:
        # For SWEX 2022, there were Five soundings taken at Montecito Fire Dept Stn2. These were
        # the last 5 taken and they were incorrectly put into "SBFDHQ" named files. This code
        # should correctly rename those five output files to be Montecito.  HARDCODED!
        # --------------------------------------------------------------------------------------
        $found_montecito = 0; # HARDCODED special cases for 5 input files

        if (($file eq "SWEX_SBFDHQ_DFM09_bufr0309052_20220512_230022_1.preproc") || 
            ($file eq "SWEX_SBFDHQ_DFM09_bufr0309052_20220513_020011_1.preproc") || 
            ($file eq "SWEX_SBFDHQ_DFM09_bufr0309052_20220513_050008_1.preproc") || 
            ($file eq "SWEX_SBFDHQ_DFM09_bufr0309052_20220513_080022_1.preproc") || 
            ($file eq "SWEX_SBFDHQ_DFM09_bufr0309052_20220513_094118_1.preproc") ) 
           {
           print "WARNING: Found Montecito Fire Dept Stn2 File = $file\n";
           $found_montecito = 1;
           $site = "Montecito_Fire_Dept_Stn2";
           $siteEnd = "MFDS2";
           $country = "California, US";
           }
        }
     elsif (index(lc($file), "sbfs38") != -1) # Santa Barbara, CA - Fire Station 38
        {
        print "Found SantaBarbara_Fire_Stn38.  File = $file\n";
        $site = "SantaBarbara_Fire_Dept_Stn38";
        $siteEnd = "SBFS38";
        $country = "California, US";
        }
     elsif (index(lc($file), "sbfs18") != -1) # Santa Barbara, CA - Fire Station 18 - Should not be found!
        {
        print "WARNING: Found SantaBarbara_Fire_Stn18.  File = $file. This should NOT happen for SWEX 2022!\n";
        ...
        {
        print "\nERROR/WARNING: Unexpected SWEX 2022 SITE NAME! File = xxx $file xxx.  site = xxx $site xxx. Turn on DEBUG! NOT!!!\n";
        $site = "UNKNOWN";
        $siteEnd = "UNKNOWN";
        $country = "UNKNOWN";
        $debug = 0;
     ...
     print "Found site: xxx $site xxx; country: xxx $country xxx; siteEnd = xxx $siteEnd xxx\n";

     # ------------------------------------------------------------
     # Form and set Site name/ID.
     #
     # For Sites without Block and Station Number use
     # site name as "site, country 
     #     E.g., SantaBarbara_Fire_Dept_HQ / California, US
     # ------------------------------------------------------------
#Non-SWEX was:  $site = $site." / ".$country;
     $site = $site.", ".$country." / ".$siteEnd;
     $header->setSite($site);

     if ($debug) {print "site = xxx $site xxx\n"};

     } # No blockNumber or stationNumber. SWEX 2022 Fire Dept stations did not have block or station numbers.
   ...
   # -------------------------------------------------------------
   # SWEX bad input required:
   # Reset Header lat/lon for one SBFS38 station input sounding
   # that had incorrect lat of 90.0 and lon of 0.0 values.
   # This must be reset properly so that subsequent lat/lon offset
   # values from the input file generate correct lat/lon values. 
   # -------------------------------------------------------------
   if ($file eq $StnBadLatLon) # HARDCODED
      {
      $sfc_lat =  34.4766; # HARDCODED for special case for SWEX 2022
      $sfc_lon = -120.214; # HARDCODED for special case for SWEX 2022

      print "Found Bad Lat/Lons in file $StnBadLatLon . Reset lat/lon.\n";
      }
...
#####################################
# HERE - Add code to translate RH Sensor Type - See NWS_GTSBUFR_Radiosonde_Converter_UpdatedSondeType.pl linke 2380
#####################################
   #-----------------------------------------
   ...
   $header->setLine(6,"Radiosonde Type/RH Sensor Type:", $sondeType." / ".$RH_Type);


   # --------------------------------------------------------------------
   # Set Ground Station S/w, Radiosonde Freq, Serial Number, if not null.
   # add any extra slash lines to end of hdr.
   # --------------------------------------------------------------------
   ...
   #----------------------------------------------------------------
   # For SWEX 2022, the following were the same for all soundings:
   # pressureSensorType of 1 maps to "Derived from GPS"
   # temperatureSensorType of 5 maps to "Chip thermistor"
   #
   # The following code not tested for non-SWEX data. It should be
   # verified in that cases.
   #----------------------------------------------------------------
   my $pressType = "N/A";
   my $tempType = "N/A";

   my %pressureSensorType_CODE_TABLE = (
      1 => "Derived from GPS",
     unknown => "UNKNOWN_PRESSURE_SENSOR_TYPE",
     null => "UNKNOWN_PRESSURE_SENSOR_TYPE_NULL",
     );

   my %temperatureSensorType_CODE_TABLE = (
      5 => "Chip thermistor",
     unknown => "UNKNOWN_TEMPERATURE_SENSOR_TYPE",
     null => "UNKNOWN_TEMPERATURE_SENSOR_TYPE_NULL",
     );

   # Set the Pressure Sensor Type for Header
   my $pressureSensorType = $headerRecVal{pressureSensorType};
   $pressureSensorType =~ s/^\s+//; # trim white space
   if ($debug) {print "pressureSensorType = $pressureSensorType\n"};

   $pressType = $pressureSensorType_CODE_TABLE{eval $pressureSensorType};
   $pressType =~ s/^\s+//; # trim white space
   if ($debug) {print "pressType = $pressType\n"};

   if (($pressType ne "N/A") && ($pressType ne "UNKNOWN_PRESSURE_SENSOR_TYPE") && ($pressType ne "UNKNOWN_PRESSURE_SENSOR_TYPE_NULL") )
      {
      $header->setLine(9,"Pressure Sensor Type: ", "$pressType");
      }
   else
      {
      $header->setLine(9,"/", " ");
   ...
   # Set the Temperature Sensor Type for Header
   my $temperatureSensorType = $headerRecVal{temperatureSensorType};
   $temperatureSensorType =~ s/^\s+//; # trim white space
   if ($debug) {print "temperatureSensorType = $temperatureSensorType\n"};

   $tempType = $temperatureSensorType_CODE_TABLE{eval $temperatureSensorType};
   $tempType =~ s/^\s+//; # trim white space
   if ($debug) {print "tempType = $tempType\n"};

   if (($tempType ne "N/A") && ($tempType ne "UNKNOWN_TEMPERATURE_SENSOR_TYPE") && ($tempType ne "UNKNOWN_TEMPERATURE_SENSOR_TYPE_NULL") )
      { $header->setLine(10,"Temperature Sensor Type: ", "$tempType"); }
   else
      { $header->setLine(10,"/", " "); }


# WAS: Add 2 lines with slashes, if unknown pressure and temperature sensor types.
#   $header->setLine(9,"/", " ");
#   $header->setLine(10,"/", " ");
...
sub parseRawFile 
   ...
   #-------------------------------------------------------
   # These 23 hdr elements are recognized AND but NOT Used.
   #-------------------------------------------------------
   ...
   #-------------------------------------------------
   # These 8 Footer elements are recognized AND Used.
   #-------------------------------------------------
   ...
   #-------------------------------------------------------
   # These 26 hdr elements are recognized AND but NOT Used.
   #-------------------------------------------------------
...
#     extendedVerticalSoundingSignificance => 1,
  ...
  if ($debug)
   ...
   if (defined($header)) 
      ...
      # both types.
      #
      # Input File Names for SWEX:
      #   SWEX_SBFS38_DFM09_bufr0309052_20220510_200001_1.preproc
      # -----------------------------------------------------
      my $outfile;
      my ($hour, $min, $sec) = split (/:/, $header->getActualTime());

      my @filename_parts = split /\./, $file; # SWEX_SBFS38_DFM09_bufr0309052_20220510_200001_1   preproc
      my @filename_subparts = split /\_/, $filename_parts[0]; # SWEX SBFS38 DFM09 bufr0309052 20220510 200001 1
...
#--------------------------------------------------------------------
      ...
      my $NS = $filename_subparts[1];  # e.g., SBFDHQ or SBFS38 for SWEX 2022. Last 5 HQ sondes are Montecito.
      ...
      if ($pibal)  # HARDCODED 
         {
         $outfile = sprintf("SWEX_%s_%04d%02d%02d%02d%02d_%02d_PIBAL.cls",  
                      $NS,
         ...
         {
         $outfile = sprintf("SWEX_%s_%04d%02d%02d%02d%02d_%02d_SONDE.cls",
                      $NS,
                      split(/,/,$header->getActualDate()),
                      $hour, $min, $fileSuffixCt);
         }
         ...
         if ($input_key eq "timePeriod")  # Each data rec set starts with "timePeriod". ASSUMPTION. 
            ...
            if ($DataRecCt >0)   # Not first record, so not first timePeriod.
               ...
               if ($value ne "null")  # zero lat is legit
                  ...
                  if ($file eq $StnBadLatLon)
                     {
####                 $headerLat =  34.4766; # HARDCODED for special case for SWEX 2022
                     if ($debug) {print "WARNING: POSSIBLE Bad Lat in $StnBadLatLon .\n";}
                     }

                  #-----------------------------
                  # Set Latitude on data record.
                  #------------------------------
               ...
               if ($value ne "null")   # zero lon is legit
                  ...
                  if ($file eq $StnBadLatLon)
                     {
####                 $headerLon = -120.214; # HARDCODED for special case
                     if ($debug) {print "WARNING: POSSIBLE Bad Lons in $StnBadLatLon.\n";}
                     }

                  #------------------------------
                  # Set Longitude on data record.
                  #------------------------------
            ...
            if ($value eq "null") { $value = 888; print "WARNING: Resetting null extendedVerticalSoundingSignificance to be 888!\n";}
