...
#
# Updates: 
#    L. Cully Jan 2023 - Updated code to work on SWEX Santa Barbara Fire Dept
#    station data. Incoming data files have names of the form:
#    ../raw_data/SWEX_SBFDHQ_DFM09_bufr0309052_20220330_205434.bufr . Original
#    raw data from FTP had the suffix *.bfr. See code to parse input file names
#    around line 233. Search for "SB new" to find changes specific to SWEX processing.
#    Also, the SWEX data did not come from the GTS but was FTP'd directly to EOL.
#    Even so, I left "GTS" throughout the code.
#
#--------------------------------------------------------------------------------
use strict;
...
sub main 
   ...
   my @files = grep(/\.bfr$/,readdir($INPUT_DIR));  # SB new
   closedir($INPUT_DIR);
   ...
   foreach my $file (sort(@files)) 
       ...
       #
       # (SB new)
       # For SWEX Santa Barbara Fire Dept BUFR data:       
       #   SWEX_SBFDHQ_DFM09_bufr0309052_20220330_205434.bfr
       #
       #----------------------------------------------------------------
       if ($debug) {print "-----------------------------\n";}
       printf "\nprocessing GTS BUFR file: $file \n";

       my @name_parts = split (/.bfr/, $file);    # HARDCODED SB new - was *.bufr
       ...
       #----------------------------------------------------------------
#Was:       my $cmdbufrdump = sprintf("./bufr_dump %s/%s > %s", $INPUT_GTS_BUFR_DIR, $file, $gts_ascii_file ); # HARDCODED
