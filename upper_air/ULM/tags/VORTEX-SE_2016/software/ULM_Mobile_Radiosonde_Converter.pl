...
sub parseHeader {
	...
	my $groundStationSoftware = "iMetOS-II software version 03.48.1C.";
	...
	foreach my $line (@lines) 
	{
		...
		if ($index >= 3)
		{
			...
			if ($temp !~ /^999$/)
			{
				...
				# if ($mixing_ratio != 0) # THIS MIGHT BE WRONG
				# Scot says do not change zero value MR to missing
				if ($mixing_ratio !~ /^999$/)
				{
					...
					# Scot has indicated we should set RH and dewpoint to missing if MR <= 0.1
		    	    $record->setRelativeHumidity($calculated_RH) unless ($mixing_ratio <= 0.1);
			...
			if ($mixing_ratio != 0) 
			{
			    ...
			    elsif (($calculated_RH) && ($calculated_RH !~ /^-/))
			    {
					...
					# Scot has indicated we should set RH and dewpoint to missing if MR <= 0.1
	    		    $record->setDewPoint($dewpoint,"C") unless ($mixing_ratio <= 0.1);    
			    }
			}
