...
#
# @author Alley Robinson 13 July 2018
# @version VORTEX-SE_2018 ULM Mobile Sounding
# 	The differences between the 2017 and the 2018 data are:
# 		1. There are no spaces between data values. Just comma separated only
# 		2. There are no Z's in the file names 
# 		3. You will use ULM_sfc_alt_2018.txt. There are no spaces there either.
#
#
#
#
...
sub readDataFiles {
    ...
    my @files = grep(/^2018.+txt$/,sort(readdir($RAW)));
    closedir($RAW);
...
sub readSurfaceValuesFile {
    ...
    open(my $FILE, sprintf("ULM_sfc_alt_2018.txt")) or die("Can't read file into array\n");
