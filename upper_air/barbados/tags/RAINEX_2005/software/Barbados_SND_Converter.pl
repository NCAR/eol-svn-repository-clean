...
##Module-------------------------------------------------------------------------
# <p>The Barbados_SND_Converter.pl script is used for converting the sounding data
# from the Barbados site from the raw MicroArt format to the CLASS format.</p>
#
# @author Joel Clawson
# @version RAINEX 1.0 This was originally created for the RAINEX project and was
# adapted from the Belize converter.
##Module-------------------------------------------------------------------------
package Barbados_SND_Converter;
use strict;
...
sub getNetworkName { return "Barbados"; }
...
##------------------------------------------------------------------------------
sub main {
    my $converter = Barbados_SND_Converter->new();
    $converter->convert();
}
...
##------------------------------------------------------------------------------
# @signature Barbados_SND_Converter new()
# <p>Create a new Barbados_SND_Converter instance.</p>
#
...
sub read_data {
	...
	# Handle special cases of line lengths and unexpected line lengths
	if (length($line) == 40) {
	    printf($WARN "%s: Line beginning with %s has an extra character.  Assuming it is an extra 9 in position 31.\n",
		   $filename,substr($line,0,9));
	    $line = sprintf("%s%s",substr($line,0,31),substr($line,32));
	} elsif (length($line) > 39 || length($line) < 38) {
	    printf("%s: Unknown line length: %d\n",$filename,length($line));
	    die();
	}
...
sub read_header {
    ...
    $station->setCountry("BB");
    #$station->setStateCode("XX");
    $station->setNetworkIdNumber(99);
    $station->setPlatformIdNumber(344);

    my $header = Sounding::ClassHeader->new($WARN,$station);
    $header->setType("Barbados Sounding");
    ...
    $station->setStationId("TBPB");
    $station->setStationName(sprintf("%s Barbados",
				     trim(substr($lines[0],1,8))));
