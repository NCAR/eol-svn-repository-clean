...
#
##Module-------------------------------------------------------------------------
# <p>The CACTI_NetCDF_Converter.pl script is used for converting the ARM NetCDF
...
#
#
# @author Linda Echo-Hawk 2 July 2019
# @version RELAMPAGO CACTI data conversion 
#          Cloud, Aerosol, and Complex Terrain Interactions (CACTI) field campaign
#          in the Sierras de Córdoba mountain range of north-central Argentina
#          - Added code to determine correct facility ID based on the facility_id
#            global attribute.
#          - Determined the sonde ID from the serial_number global attribute.
#          - Determine release time from file name. Nominal time is the same 
#            as the release time.
#
#
# @author Linda Echo-Hawk Sept 2018
# @SOCRATES
# ARMnetCDF_to_ESC.pl --limit --num_soundings=8 >&! 3-hourly_results.txt
...
sub processHeaderInfo
{
	...
	# -------------------------------------------------------------
    printf(OUTFILE "Data Type:                         ARM Sounding Data/Ascending\n");
    printf(OUTFILE "Project ID:                        RELAMPAGO-CACTI\n");
	...
	printf(OUTFILE "Radiosonde System Software:        MW41\n");
...
sub readRawFile {
    ...
    if (!$limit)
	{
        ...
        $outfile = sprintf "%s/%s_%s_%04d%02d%02d%02d%02d%02d%s", $self->{"VERBOSE_OUTPUT_DIR"},
	                       $self->{"NETWORK"}, $facility, $year, 
