...
#
# @usage ARMnetCDF_to_ESC.pl [--limit] [--max=10]
...
#
#
# @author Linda Echo-Hawk 2012-01_13
...
# HARD-CODED values
#-------------------------------------------------
sub getNetworkName { return "Gan_ARM";  }                        
...
sub processHeaderInfo
{
	...
	# -------------------------------------------------------------
    # NOTE: The following code was used in VORTEX2
    # Soundings are taken four times per day, and nominal times
    # will be 00, 06, 12, or 18 hours.  Determine the closest
    # nominal to the actual time and get the multiplier 
	# -------------------------------------------------------------
...
#	} 
	...
	# -------------------------------------------------------------
    # NOTES FROM VORTEX2 ARM CART soundings
	# (86400 s/day) / (4 drops/day) = 21600 s/drop 
	# my $nomFactor = 21600; # 4 drops/day 4*21600 = 86400 s/day
	# -------------------------------------------------------------
	my $nomFactor = 10800; # 8 drops/day 8*10800 = 86400 s/day
	...
	# -------------------------------------------------------------
    # NOTE:  Release Location was incorrect due to close proximity with the equator. 
    # It has been found that latitudes of -0 degress display with a "N" instead of "S"
    # and it is believed to be a bug in the perl library code.  Since these soundings
    # were all released from the same location, the values were hard-coded.
	# -------------------------------------------------------------


    # print out the header lines.
    printf(OUTFILE "Data Type:                         ARM AMF Radiosonde/Ascending\n");
    printf(OUTFILE "Project ID:                        DYNAMO\n");
    printf(OUTFILE "Release Site Type/Site ID:         %s\n", $global_atts{"facility_id"});
    printf(OUTFILE "Release Location (lon,lat,alt):    073 09.00'E, 00 41.40'S, 73.150, -0.690, 1.0\n");
