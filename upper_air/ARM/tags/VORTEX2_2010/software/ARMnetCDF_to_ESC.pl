...
# sounding data files from the C1 Lamont, Oklahoma facility.
#
...
sub processHeaderInfo
{
    ...
    # HARD-CODED value
	my $sondeType = "Vaisala RS92-KL (Loran-C windfinding, dual humicap)";
    ...
    printf(OUTFILE "Data Type:                         ARM Soundings/Ascending\n");
    printf(OUTFILE "Project ID:                        VORTEX2\n");
...
sub get_global_atts 
{
    ...
    # site_id = "sgp"; facility_id =  "C1: Lamont, Oklahoma"; serial_number = "E1152121"
...
sub readRawFile {
	...
	my $facilityId = $global_atts{"facility_id"};
   	$facilityId =~ s/: Lamont, Oklahoma//g;
     
	my $outfile; 
    if (!$limit)
	{
        $outfile = sprintf "%s/%s_%s_%s_%04d%02d%02d%02d%02d%s", getVerboseOutputDirectory(),
	                       $global_atts{"site_id"}, $facilityId, $sndType, $year, 
	...
	{
        $outfile = sprintf "%s/%s_%s_%s_%04d%02d%02d%02d%02d%s", getSkewtOutputDirectory(),
	                       $global_atts{"site_id"}, $facilityId, $sndType, $year, 
		   				   $month, $day, $hour, $minute, $ext; 
        open (OUTFILE,">$outfile") 
    ...
    #-----------------------------------------------------------------
    # All soundings come from the C1 Lamont, Oklahoma facility
    #-----------------------------------------------------------------
...
sub build_default_station {
    ...
    # Platform 113, ARM CART Sonde
    $station->setPlatformIdNumber(113);
