...
$comments = qq("IHOP_2002 ARM-CART Sndings");
...
foreach $month(sort @MONTHS){
    ...
    chdir $ebufr_dir;
    $count = 0;
    $mergefile = "ARM-CART_".$mth.$year.".ebufr";
    #$tarfile = "ARM-CART_".$mth.$year.".tar";
    print "$mergefile\n";
    opendir(ETC,"$file_dir") || die "Can't open $file_dir directory!\n";
    @files = (grep(/^S(C|B)\d$month\d{4}\.ebufr/i,readdir(ETC)));
    closedir(ETC);
    if(scalar(@files)){
	chdir $file_dir;
