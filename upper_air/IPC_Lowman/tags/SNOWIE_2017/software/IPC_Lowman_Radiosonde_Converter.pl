...
##Module------------------------------------------------------------------------
# <p>The IPC_Lowman_Radiosonde_Converter.pl script is used for converting high 
# resolution radiosonde ascii data to the EOL Sounding Composite
# (ESC) format.</p> 
#
# @usage IPC_Lowman_Radiosonde_Converter.pl >&! results.txt
#                       
# @author Linda Echo-Hawk 2017-12-29 
# @version SNOWIE Created based on the IPC_Crouch_Radiosonde_Converter.pl script
...
#            script.
#
# @use     IPC_Lowman_Radiosonde_Converter.pl >&! results.txt
#         
#
...
#         
##Module------------------------------------------------------------------------
package IPC_Lowman_Radiosonde_Converter;
use strict;
...
printf "\nIPC_Lowman_Radiosonde_Converter.pl began on ";print scalar localtime;printf "\n";
my $debug = 0;


&main();
printf "\nIPC_Lowman_Radiosonde_Converter.pl ended on ";print scalar localtime;printf "\n";

my $filename;

##------------------------------------------------------------------------------
# @signature void main()
# <p>Process the IPC Lowman radiosonde data by converting it from 
# the native ASCII format into the ESC format.</p>
##------------------------------------------------------------------------------
sub main {
    my $converter = IPC_Lowman_Radiosonde_Converter->new();
    $converter->convert();
}

##------------------------------------------------------------------------------
# @signature IPC_Lowman_Radiosonde_Converter new()
# <p>Create a new instance of a IPC_Lowman_Radiosonde_Converter.</p>
#
# @output $self A new IPC_Lowman_Radiosonde_Converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
...
sub buildDefaultStation {
    ...
    $station->setStationName("IPC_Lowman");
    # HARD-CODED
...
sub parseHeader {
    ...
    $header->setId("IPC_Lowman");
	$header->setSite("Lowman, ID/KLOW");

    # -------------------------------------------------
    # Get the header lat/lon data
    # -------------------------------------------------
	# Lowman lat/lon/alt provided by Scot L.
	my $lat = 44.100;
	my $lon = -115.459;
	my $alt = 1315.0;
...
sub parseRawFile {
	...
	#
	# NOTE: This code not used for Lowman
	# --------------------------------------------
	my @record_list = ();
	# --------------------------------------------
...
sub readDataFiles {
    ...
    my @files = grep(/^KLOW.+\.txt/,sort(readdir($RAW)));
    closedir($RAW);
