...
##Module------------------------------------------------------------------------
# <p>The TTU_Mobile_Radiosonde_Converter.pl script is used for converting 
# high resolution radiosonde data from a csv format to the EOL Sounding 
# Composite (ESC) format.</p> 
#
# @author Linda Echo-Hawk 10 Nov 2016
# @version VORTEX-SE TTU Mobile Soundings
#          based on the Purdue Mobile Soundings Converter
#          - A separate text file (TTU_elev.txt) is read in to find
#            the surface lat/lon and elevation for each raw data file.
#          - The location from the text file served as the Site
#            value for the header.
#          - Data are typically at 2 second intervals above the zero
#            second record, but this should be checked and not assumed.
#          - Four raw data files had some wind direction values 
#            that were >360. Scot instructed me to subtract 360 
#            from these (after consulting with the data source).
...
#
##Module------------------------------------------------------------------------
package TTU_Mobile_Radiosonde_Converter;
use strict;
...
printf "\nTTU_Mobile_Radiosonde_Converter.pl began on ";print scalar localtime;printf "\n";
my $debug = 0;
my $debug_geopotential_height = 0;

&main();
printf "\nTTU_Mobile_Radiosonde_Converter.pl ended on ";print scalar localtime;printf "\n";

my $filename;

##------------------------------------------------------------------------------
# @signature void main()
# <p>Process the TTU Mobile radiosonde data by converting it from 
# the native ASCII format into the ESC format.</p>
##------------------------------------------------------------------------------
sub main {
    my $converter = TTU_Mobile_Radiosonde_Converter->new();
    $converter->convert();
}

##------------------------------------------------------------------------------
# @signature TTU_Mobile_Radiosonde_Converter new()
# <p>Create a new instance of a TTU_Mobile_Radiosonde_Converter.</p>
#
# @output $self A new TTU_Mobile_Radiosonde_Converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
...
# <p>Create a default station for the TTU network using the 
...
sub parseHeader {
    ...
    $header->setType("TTU Mobile Sounding Data");
	...
	foreach my $line(@surfaceValues)
	{
		...
		# raw data file name example: 20160314_0602Z_TTU_LexingtonAL.txt
		# sfc_alt ($line):
		# 20160314_0536Z_Purdue_CollinwoodTN.txt 35.18040, -87.73949 321m (Purdue)
		# 20160314_0602Z_TTU_LexingtonAL.txt 34.961200, -87.370340 229m (TTU)
		# $location_data[0] = 20160314_0602Z_TTU_LexingtonAL.txt
		# $location_data[1] = 34.961200
		# $location_data[2] = -87.370340
		# $location_data[3] = 229m
		...
		if ($file =~ /$file_match_name/i)
		{
			...
			if ($citystate =~ /pyror/i)
			{
				$citystate = "Pryor Field Decatur, AL";
			}
			...
			($sfc_lat, $units) = split(",", $location_data[1]);
            ($sfc_lon, $units) = split(",", $location_data[2]);
			
			# $sfc_lat = trim($location_data[1]);
			# $sfc_lon = trim($location_data[2]);

			print "SFC LAT: $sfc_lat  SFC LON: $sfc_lon\n";
...
#	}
	...
	# -------------------------------------------------
    my $sondeType = "Vaisala RS-92-SGPD";
	$header->setLine(5,"Radiosonde Type:", ($sondeType));
	my $groundStationSoftware = "Vaisala DigiCORA III MW31";
...
sub parseRawFile {
	...
	foreach my $line (@lines) 
	{
		...
		if ($index >= 3)
		{
			...
			{
				print "MISSING TIME AT INDEX: $index\n";

			}
			# For TTU, the time on each record is the UTC 
			...
			# Data are typically at 2 second intervals 
			# above that (don't make that assumption though)
			# so will need to derive the time increments.
			#-------------------------------------
			if ($index == 3)
			{
		    	$record->setTime($fake_surface_time);
...
#				}
			...
			if ($data[7])
			{
				if ($data[8])
				{
					# Four files have wind direction > 360
					if ($data[8] >= 360)
					{
						$data[8] -= 360;
						$record->setWindDirection($data[8]);
					}
					else
					{
						$record->setWindDirection($data[8]);
			...
			elsif ($index > 3)
			{
    	        ...
    	        { 
					...
					{
						print "No previous record yet assigned\n";
					}
            	}
                # print "INDEX: $index\n";
...
sub readSurfaceValuesFile {
    ...
    open(my $FILE, sprintf("TTU_elev.txt")) or die("Can't read file into array\n");
