...
#
##Module-------------------------------------------------------------------------
# <p>The MMS_NetCDF_Converter.pl script is used for converting the Gan MMS NetCDF
...
#
#
# @author Linda Echo-Hawk 2013-09-19
# @version DYNAMO for the MMS Gan NetCDF soundings
# @use MMS_NetCDF_Converter.pl --limit --num_soundings=8 >&! results.txt
#          - Based on the ARMnetCDF_to_ESC.pl script, all notes below apply
#          - For the Release Location header line, I hard-coded in the "S" 
#            for the negative zero latitude at the Gan site. The  separate
#            script WriteLatLonHeader.pl still needed to be run on files in 
#            the /final and /dayfiles directories to take care of the 
#            negative zero latitude problem.
#
#
...
printf "\nMMS_NetCDF_Converter.pl began on ";print scalar localtime;printf "\n";
...
printf "\nMMS_NetCDF_Converter.pl ended on ";print scalar localtime;printf "\n";
...
##------------------------------------------------------------------------------
sub main {
    my $converter = MMS_NetCDF_Converter->new();
    $converter->convert();
}
...
##------------------------------------------------------------------------------
# @signature MMS_NetCDF_Converter new()
# <p>Create a new MMS_NetCDF_Converter instance.</p>
#
...
sub processHeaderInfo
{
	...
	# -------------------------------------------------------------
    printf(OUTFILE "Data Type:                         MMS Radiosonde/Ascending\n");
    printf(OUTFILE "Project ID:                        DYNAMO\n");
    printf(OUTFILE "Release Site Type/Site ID:         %s/43599\n", $global_atts{"facility_id"});
    printf(OUTFILE "Release Location (lon,lat,alt):    %03d %05.2f'%s, %02d %05.2f'S, %.3f, %.3f, %.1f\n",
	     			abs($lon_deg),$lon_min,$lon_dir,abs($lat_deg),$lat_min,$baselon,$baselat,$basealt);  
    # Because Gan has a "negative zero" latitude, we need to hard-code in the "S" direction due to a
	# problem with the perl libs 
    # printf(OUTFILE "Release Location (lon,lat,alt):    %03d %05.2f'%s, %02d %05.2f'%s, %.3f, %.3f, %.1f\n",
...
sub build_default_station {
	...
	# 415 	Radiosonde, Vaisala RS92-SGP
