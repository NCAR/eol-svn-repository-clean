#! /usr/bin/perl -w
# BEWARE: DO NOT USE THIS SCRIPT TO RUN THE AUTOQC POST 2007. 
#         USE THE ant autoqc command in the build.xml file.
#     This runs an older version of the autoqc!!!!
##Module-----------------------------------------------------------------------
...
my $AUTOQC = "/work/software/TREX/library/upper_air/autoQC/bin/autoqc_drop_TREX";
...
sub main {
	...
	system(sprintf("%s %s/%s %s/%s",$AUTOQC,$INPUT_DIR,$file,$OUTPUT_DIR,$out_file));
