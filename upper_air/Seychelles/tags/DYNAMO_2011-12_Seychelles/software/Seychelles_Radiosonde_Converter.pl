...
##Module------------------------------------------------------------------------
# <p>The Seychelles_Radiosonde_Converter.pl script converts radiosonde
...
#
# @author Linda Echo-Hawk, 19 Sept 2012
# @version DYNAMO 2011-12, for Seychelles sounding data
#           - Revised the Nairobi converter
#
# @author Linda Echo-Hawk, 17 Sept 2012
# @version DYNAMO_2011-12 Created for the Seychelles soundings based on
...
#
##Module------------------------------------------------------------------------
package Seychelles_Radiosonde_Converter;
use strict;
...
my $debugKey = 0;
 ...
 printf "\nSeychelles_Radiosonde_Converter.pl began on ";print scalar localtime;printf "\n";  
&main();
 printf "\nSeychelles_Radiosonde_Converter.pl ended on ";print scalar localtime;printf "\n";

##------------------------------------------------------------------------------
# @signature void main()
# <p>Process the Seychelles radiosonde/thermosonde data by converting it
# from the native ASCII format into the ESC format.</p>
##------------------------------------------------------------------------------
sub main {
    my $converter = Seychelles_Radiosonde_Converter->new();
    $converter->convert();
}

##------------------------------------------------------------------------------
# @signature Seychelles_Radiosonde_Converter new()
# <p>Create a new instance of a Seychelles_Radiosonde_Converter.</p>
#
# @output $self A new Seychelles_Radiosonde_Converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
    ...
    $self->{"PROJECT"} = "DYNAMO";
    $self->{"NETWORK"} = "Seychelles";

    $self->{"FINAL_DIR"} = "../final";
    $self->{"OUTPUT_DIR"} = "../test_output";
    $self->{"RAW_DIR"} = "../test_raw_data";
...
sub buildDefaultStation {
    ...
    $station->setStationName("Seychelles");
...
#        Seychelles data, the key is the date portion of the raw data filenames.
...
sub parseHeader {
    ...
    $header->setType("Seychelles Radiosonde");
    $header->setProject($self->{"PROJECT"});
	# The Id will be the prefix of the output file
    $header->setId("Seychelles");
	# "Release Site Type/Site ID:" header line
    $header->setSite("Seychelles/63985");
	...
	foreach my $line (@headerlines) 
	{
		...
		if ($line =~ /Started at/)
		{
			...
			print "RELESASE DATE: $date   TIME: $time\n";
		...
		if ($line =~ /Clouds/)
		{
			...
			print "CLOUDS: $values[2]\n";
            $header->setLine(7,"Clouds: ", $values[2]);
		...
		if ($line =~ /Pressure/)
		{
            ...
            $header->setLine(8,"Ground Check Pressure:    ", $GroundCheckPress);
			$groundCheck = 1;
		}
		if ($line =~ /Temperature/)
		{
            ...
            $header->setLine(9,"Ground Check Temperature: ", $GroundCheckTemp);
			$groundCheck = 1;
		}
		if ($line =~ /Humidity/)
		{
            ...
            $header->setLine(10,"Ground Check Humidity: ", $GroundCheckHumidity);
			$groundCheck = 1;
		}
		if (!$groundCheck)
		{
			my $groundCheckValue = "omitted";
			$header->setLine(8,"Ground Check: ", $groundCheckValue);
		}
        # Add a header line for the ground station software (info from Scot)
		my $station_software = "Digicora II/MW21";
		...
		# print "SAVING HEADER FOR FILE: $file\n";
