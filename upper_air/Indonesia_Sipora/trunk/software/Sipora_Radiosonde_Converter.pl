...
##Module------------------------------------------------------------------------
# <p>The Sipora_Converter.pl script is used for converting high
...
# @version DYNAMO 2011 for Sipora
...
printf "\nSipora_Converter.pl began on ";print scalar localtime;printf "\n";
my $debug = 0;
&main();
printf "\nSipora_Converter.pl ended on ";print scalar localtime;printf "\n";

my $filename;

##------------------------------------------------------------------------------
# @signature void main()
# <p>Process the Sipora radiosonde data by converting it from 
# the native ASCII format into the ESC format.</p>
##------------------------------------------------------------------------------
sub main {
    my $converter = Sipora_Converter->new();
    $converter->convert();
}

##------------------------------------------------------------------------------
# @signature Sipora_Converter new()
# <p>Create a new instance of a Sipora_Converter.</p>
#
# @output $self A new Sipora_Converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
...
sub buildDefaultStation {
    ...
    $station->setReportingFrequency("3 hours");
...
sub parseHeader {
    ...
    $header->setType("HARIMAU 2011 Radiosonde");
    ...
    $header->setSite("Sipora, Indonesia");
    ...
    # BEWARE: Expects filename to be similar to: 11112909EDT.tsv 
    # ----------------------------------------------------------
	...
	if ($filename =~ /(\d{2})(\d{2})(\d{2})(\d{2})/)
	{
		$date = sprintf("20%02d, %02d, %02d", $1, $2, $3);
		$time = sprintf("%02d:00:00", $4);
...
sub readDataFiles {
    ...
    my @files = grep(/^\d{8}EDT\.tsv/,sort(readdir($RAW)));
    closedir($RAW);
