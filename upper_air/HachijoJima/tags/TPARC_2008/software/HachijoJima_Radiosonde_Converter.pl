...
sub load_header_info {
    ...
    foreach my $windfile (@files)
	{
        ...
        open(my $FILE,sprintf("%s/%s",$self->{"RAW_DIR"},$windfile)) or die("Can't read $windfile\n");
	    my @lines = <$FILE>;
	    close ($FILE);         

		my $sounding = $windfile;
      	$sounding =~ /(\d{8})/;
	    $sounding = uc($1);        

        my $header = ClassHeader->new($self->{"WARN"},$self->get_station());
			...
			if ($index < 21)
	        {
			    ...
			    if ($line =~ /RS-Number/i)
			    {
                    ...
                    $label = "Sonde Id/Sonde Type";
			  	    ...
			  	    $contents[1] = "PTU GPS";
					# Input line 12 should be line 6 in output, so subtract 6
			        $header->setLine(($index-6), trim($label).":",trim(join("/",@contents))); 
		        }
				if ($line =~ /Pressure/)
				{
					chomp $line;
...
##------------------------------------------------------------------------------
# @signature void parse_ptu_file(FileHandle WARN, String file)
...
sub read_data_files {
    ...
    foreach my $ptufile (@files) {
        $self->parse_ptu_file($WARN,$ptufile);
    }
    rewinddir($RAW);
