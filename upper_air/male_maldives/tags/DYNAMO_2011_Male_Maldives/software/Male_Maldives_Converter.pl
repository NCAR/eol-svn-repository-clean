...
##Module------------------------------------------------------------------------
# <p>The Male_Maldives_Converter.pl script is used for converting high
# resolution radiosonde data from ASCII formatted data) to the EOL Sounding 
# Composite (ESC) format.</p> 
#
#
# @author Linda Echo-Hawk 2012-04-09
# @version DYNAMO 2011 for Male Maldives Soundings
#    This code was created by modifying the RVSagarKanya_Converter.pl script.
#    - Changed all references in code from R/V Sagar Kanya to Male, Maldives.
#    - Added code to calculate nominal time as requested by Scot.  BEWARE: Code
#      is specific to Male timeframe (Sept-Dec 2011) releasing soundings 8x/day.
# This code makes the following assumptions:
#  - That the raw data file names shall be in the form
#        "43555_yyyymmddhhmm.tsv" where yyyy = year, mm = month, dd = day, hh=hour,
#         mm = minute. 43555 is the Male, Maldives station identifier.
...
##Module------------------------------------------------------------------------
package Male_Maldives_Converter;
use strict;
...
printf "\nMale_Maldives_Converter.pl began on ";print scalar localtime;printf "\n";
my $debug = 0;
&main();
printf "\nMale_Maldives_Converter.pl ended on ";print scalar localtime;printf "\n";
...
##------------------------------------------------------------------------------
sub main {
    my $converter = Male_Maldives_Converter->new();
    $converter->convert();
}
...
sub parseHeader {
    ...
    $header->setSite("Male, Maldives/43555");
		...
		if (($index > 0) && ($index < 11))
	    {
		    if ($line !~ /^\s*\/\s*$/) 
		    {
			    if ($line =~ /RS-Number/i)
			    {
					...
					$header->setLine(6,"Ground Station Software:", "Digicora Version 3.64");
		        }
	        }
	    } 
    ...
    # BEWARE: Expects filename to be similar to: 43555_201112142332.tsv 
    # ------------------------------------------------------------------
	...
	# --------------------------------------------------------------
	# Soundings are taken 8 times per day, and
	# nominal times will be 00, 03, 06, 09, 12, 15, 18 or 21 hours.
	# --------------------------------------------------------------
    my $tempDate = $date;
