...
##Module---------------------------------------------------------------------
# <p>The ISS_SND_Converter converts data from the ISS site into the
# CLASS format.</p>
#
# @author Joel Clawson
# @version NAME This was originally created for NAME.
##Module---------------------------------------------------------------------
package ISS_SND_Converter;
use strict;
...
##---------------------------------------------------------------------------
sub main {
    my $converter = ISS_SND_Converter->new();
    $converter->convert();
}
...
##---------------------------------------------------------------------------
# @signature ISS_SND_Converter new()
...
sub readRawDataFiles {
    ...
    my @stns = grep(/^ISS/,readdir($RAW));
    closedir($RAW);

    foreach my $dir (@stns) {

	opendir(my $DIR,sprintf("%s/%s",getRawDataDirectory(),$dir)) or
	    die("Cannot open raw data directory.\n");
	my @files = grep(/\.cls$/,readdir($DIR));
	closedir($DIR);

	foreach my $file (@files) {
	    open(my $FILE,sprintf("%s/%s/%s",getRawDataDirectory(),$dir,$file)) or 
		die("Can't open file: $file\n");
	    
	    printf("Processing: %s ...\n",$file);
	    
	    $self->readRawFile($FILE,$dir);
...
##---------------------------------------------------------------------------
# @signature void readRawFile(FileHandle FILE, String dir)
...
#    printf("Variable Columns: (%s %s) (%s %s)\n",$var_name1,$var_unit1,$var_name2,$var_unit2);
	...
	if ($dir =~ /ISS2/) { $stn_name = "Puerto Penasco,"; }
	elsif ($dir =~ /ISS3/) { $stn_name = "Kino Bay,"; }
	elsif ($dir =~ /ISS4/) { $stn_name = "Los Mochis,"; }
    }
    $stn_type = "ISS" if ($stn_type =~ /GLASS/i);
