...
##Module------------------------------------------------------------------------
# <p>The USA_Radiosonde_Converter.pl script is used for converting 
...
# @version Meso 2018-19 USA Radiosonde Data
# 	   - The first record is the surface record
# 	   - Header altitude is taken from the surface
# 	     record HGHT column
# 	   - Lat/Lon values were provided for the Mobile
# 	     AL location by Scot L.
# 	   - Assume the data are at 5-sec. intervals for
# 	     the file names of the form *SHARPpy.txt and 
# 	     at 10-sec. intervals for the file names of the
# 	     form *SHARPPY.txt
# 	   - Need to derive RH and wind components
# 	   - Neet to convert wind speed from knots to m/s
# 	   - A "readSurfaceValuesFile" function exists to read 
# 	     in surface values from a separate file, but is not 
# 	     called since the values were provided by Scot L. 
# 	     and no separate file was used.
# 	   - The file names differ in that some have SHARPPY all
# 	     in upper case, while others use a lower case "py" 
# 	     (e.g., SHARPpy). The "readDataFiles" function uses a
# 	     case-insensitve match to find all the files.
#
...
#
##Module------------------------------------------------------------------------
package USA_Radiosonde_Converter;
use strict;
...
printf "\nUSA_Radiosonde_Converter.pl began on ";print scalar localtime;printf "\n";
my $debug = 0;

&main();
printf "\nUSA_Radiosonde_Converter.pl ended on ";print scalar localtime;printf "\n";

my $filename;

##------------------------------------------------------------------------------
# @signature void main()
# <p>Process the USA radiosonde data by converting it from 
# the native ASCII format into the ESC format.</p>
##------------------------------------------------------------------------------
sub main {
    my $converter = USA_Radiosonde_Converter->new();
    $converter->convert();
}

##------------------------------------------------------------------------------
# @signature USA_Radiosonde_Converter new()
# <p>Create a new instance of a USA_Radiosonde_Converter.</p>
#
# @output $self A new USA_Radiosonde_Converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
...
# <p>Create a default station for the USA network using the 
...
sub parseHeader {
    ...
    $header->setType("USA Radiosonde Data");
	...
	# Expects filename similar to 1804062231_SHARPpy:
	# upperair.USA_sonde.201902071200.Mobile_AL_SHARPpy.txt OR
	# upperair.USA_sonde.201902071800.Mobile_AL_SHARPPY.txt 
    # ----------------------------------------------------------
    my $date;
	my $time;
...
sub parseRawFile {
	...
	foreach my $line (@lines) 
	{
		...
		if ($index >= 6)
		{
			...
			# with $index > 6
			# NOTE: 5 second increments for SHARPpy files and 
			# 	10 second increments for SHARPPY files
			#-------------------------------------
			my $increment;
			if($file =~ /SHARPpy/) {$increment = 5}
			elsif($file =~ /SHARPPY/) {$increment = 10}
...
sub readSurfaceValuesFile {
    ...
    open(my $FILE, sprintf("USA_sfc_alt.txt")) or die("Can't read file into array\n");
