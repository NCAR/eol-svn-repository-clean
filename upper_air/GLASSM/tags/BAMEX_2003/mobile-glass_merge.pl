#!/bin/perl/ -w
# Program mobile-glass_merge.pl Darren R. Gallant JOSS September 1st, 2000
# merges ebufr files
$TRUE = 1;$FALSE = 0;$LEAP = $FALSE;
$main_dir = "/work/1R/BAMEX_SND/GLASS";
$ebufr_dir =  "$main_dir/ebufr_files/high_res";
$day_dir = "$ebufr_dir/month_files";
$ebufsort = "/home/gallant/bin/ebufsort";
$merge = "/home/gallant/bin/merge_Solaris";
$comments = qq("BAMEX MGLASS Soundings");
%months = (1=>"01",2=>"02",3=>"03",4=>"04",5=>"05",6=>"06",7=>"07",8=>"08",9=>"09","a"=>"10","b"=>"11","c"=>"12");
@MONTHS = ("5","6","7");
#@MONTHS = (6);
$year = "2003";$file_dir = "$ebufr_dir";
foreach $month(sort @MONTHS){
    $mth = $months{$month};print $month,"\n";
    chdir $ebufr_dir;
    $count = 0;
    $mergefile = "MCLASS_".$mth.$year.".ebufr";
    $tarfile = "MCLASS_".$mth.$year.".tar";
    print "$mergefile $tarfile\n";
    opendir(ETC,"$file_dir") || die "Can't open $file_dir directory!\n";
    @files = (grep(/^bx\w$month\d{6}\.ebufr/i,readdir(ETC)));
    closedir(ETC);
    if(scalar(@files)){
	chdir $file_dir;
