...
sub getNetworkName { return "BELIZE"; }
...
##------------------------------------------------------------------------------
sub main {
    my $converter = Belize_SND_Converter->new();
    $converter->convert();
}
...
sub read_data {
    ...
    my $RAW;
    if (-z sprintf("%s/R%03d",getRawDirectory(),$asc_no)) {
	open($RAW,sprintf("%s/T%03d",getRawDirectory(),$asc_no)) or
	    die(sprintf("Cannot open T%03d\n",$asc_no));
    } else {
	open($RAW,sprintf("%s/R%03d",getRawDirectory(),$asc_no)) or
...
sub read_header {
    ...
    $station->setCountry("BH");
    #$station->setStateCode("XX");
    $station->setNetworkIdNumber(99);
    $station->setPlatformIdNumber(315);

    my $header = Sounding::ClassHeader->new($WARN,$station);
    $header->setType("Belize Sounding");
    $header->setProject($self->getProjectName());
    $header->setLine("Ascension No:",$asc_no);

    if (getProjectName() =~ /NAME/ && ($asc_no == 169)) {
	$station->setStationId("MZBZ");
	$station->setLatitude(1729,"DDMM");
	$station->setLongitude("-08818","-DDDMM");
	$station->setElevation(5,"m");

	$header->setActualRelease("20040623","YYYYMMDD","1111","HHMM",0);
    } else {
	...
	$station->setStationId("MZBZ");
	$station->setStationName(sprintf("%s Belize City, Belize",
