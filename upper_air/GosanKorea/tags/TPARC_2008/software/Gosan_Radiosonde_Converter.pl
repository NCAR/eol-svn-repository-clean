...
##Module------------------------------------------------------------------------
# <p>The Gosan_Radiosonde_Converter.pl script converts radiosonde
...
#            HachijoJima_Radiosonde_Converter.pl scipt for T-PARC. 
...
#
##Module------------------------------------------------------------------------
package Gosan_Radiosonde_Converter;
use strict;
...
my $debugHeader = 0;
 ...
 printf "\nGosan_Radiosonde_Converter.pl began on ";print scalar localtime;printf "\n";  
&main();
 printf "\nGosan_Radiosonde_Converter.pl ended on ";print scalar localtime;printf "\n";

##------------------------------------------------------------------------------
# @signature void main()
# <p>Process the Gosan radiosonde/thermosonde data by converting it
# from the native ASCII format into the ESC format.</p>
##------------------------------------------------------------------------------
sub main {
    my $converter = Gosan_Radiosonde_Converter->new();
    $converter->convert();
}

##------------------------------------------------------------------------------
# @signature Gosan_Radiosonde_Converter new()
# <p>Create a new instance of a Gosan_Radiosonde_Converter.</p>
#
# @output $self A new Gosan_Radiosonde_Converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
...
sub buildDefaultStation {
    ...
    $station->setStationName("Gosan, Korea");
	$station->setLatLongAccuracy(3);
    $station->setStateCode("99");
    $station->setCountry("Korea");
    $station->setReportingFrequency("12 hourly");
    $station->setNetworkIdNumber("47185");
...
#        Gosan data, the key is the date portion of the raw data filenames.
...
sub parseHeader {
	...
	foreach my $line (@headerlines) 
	{
	    ...
	    if ($line =~ /RS-Number/i)
	    {
            ...
            $label = "Sonde Id/Sonde Type";
	  	    ...
	  	    # Scot provided Radiosonde Type below
	   	    $contents[1] = "Vaisala RS80-15L with Loran-C wind-finding";
		...
		if (!$groundCheck)
		{
			...
			$header->setLine(6,"Ground Check: ", $groundCheckValue);
		}
...
sub readDataFiles {
    ...
    my @files = grep(/RS_WND/i,sort(readdir($RAW)));
    foreach my $windfile (@files) {
        $self->parseWindFile($WARN,$windfile);
    }
    rewinddir($RAW);
    
    @files = grep(/RS_PTU/i,sort(readdir($RAW)));
