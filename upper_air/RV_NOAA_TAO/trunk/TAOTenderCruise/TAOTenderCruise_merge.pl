#!/bin/perl/ -w
# Program TAOTenderCruise_merge.pl Darren R. Gallant JOSS September 1st, 2000
# merges ebufr files
$TRUE = 1;$FALSE = 0;$LEAP = $FALSE;
$main_dir = "/work/1R/PACS_SND/ship/TAOTenderCruise";
...
9=>"09","a"=>"10","b"=>"11","c"=>"12");
@MONTHS = ("a","b","c");
@YEARS = ("1995","1996","1997","1999","2000","2001","2002");
#@YEARS = ("1995");
#@MONTHS = (6);
foreach $year(sort @YEARS){
    ...
    chdir $ebufr_dir;
    $count = 0;
    $mergefile = "TAO_".$year.".ebufr";
    $tarfile = "SNCR_".$year.".tar";
    print "$mergefile $tarfile\n";
    opendir(ETC,"$file_dir") || die "Can't open $file_dir directory!\n";
    @files = (grep(/^TAO(\d{7}|(a|b|c)\d{6})\.ebufr/i,readdir(ETC)));
    closedir(ETC);
    if(scalar(@files)){
	chdir $file_dir;
