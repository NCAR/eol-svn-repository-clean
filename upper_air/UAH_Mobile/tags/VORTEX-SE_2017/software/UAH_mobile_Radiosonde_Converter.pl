...
# 	   - Calculate ascent rate based off of calculated geopotential height
...
my $date;
my $time;
my $my_time; #variable to separate different vertical resolutions within the NACC files
my $my_month;
my $my_day;
my $increment;
...
sub parseHeader {
	...
	if ($file =~ /Haleyville/)
	{
		$header->setSite("Haleyville AL");
		$sfc_elev = 284;

	}
	elsif ($file =~ /Geraldine/)
	{
		$header->setSite("Geraldine AL");
		$sfc_elev = 360;
	}
	elsif ($file =~ /GuntersvilleMuni/)
	{
		$header->setSite("Guntersville Muni AL");
		$sfc_elev = 185;
		$increment = 5;
	}
	elsif ($file =~ /Guntersville/)
	{
		$header->setSite("Guntersville AL");
		$sfc_elev = 233;
		$increment = 5;
	}
	elsif ($file =~ /Moulton/)
	...
	elsif ($file =~ /Macedonia/)
	{
		$header->setSite("Macedonia AL");
		$sfc_elev = 390;
	}
	elsif ($file =~ /NACC/)
	{
		$header->setSite("NACC AL");
		$sfc_elev = 392;
		if (($file =~ /0403/) || (($file =~ /0405/) && ($my_time <= 1715)) || ((($my_month == 4) && ($my_day >= 26)) || ($my_month == 5))){
			$increment = 5;
		}
	}
	elsif ($file =~ /Courtland/)
	{
	...
	elsif ($file =~ /Brownsferry/)
	{
		$header->setSite("Brownsferry AL");
		$sfc_elev = 201;
	}
	elsif ($file =~ /Athens/)
	{
		$header->setSite("Athens AL");
		$sfc_elev = 212;
	}
	else
	{
   ...
   # ----------------------------------------------------------
	...
	foreach my $line (@lines) 
	{
		...
		if ($index >= 3)
		{
			...
			# use 1 or 3 second increments, 
			# depending on the type of radiosonde,
			# so increment $raw_data_time by 1 or 3
