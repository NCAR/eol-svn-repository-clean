...
sub parseHeader {
	...
	if ($file =~ /Falkville/)
	{
		$header->setSite("Falkville AL");
		if ($file =~ /201803192208/){
			$noSurfaceData = 1;
		}
		else{
			$sfc_elev = 191;
	...
	elsif ($file =~ /Hollywood/)
	{
		$header->setSite("Hollywood AL");
		$sfc_elev = 191;
		$noSurfaceData = 0;
	}
	elsif ($file =~ /Athens/)
	...
	{
		$header->setSite("Salem AL");
		$sfc_elev = 232;
		$noSurfaceData = 0;
	}
	elsif ($file =~ /Lawrenceburg/)
	{
		$header->setSite("Lawrenceburg TN");
		$sfc_elev = 274;
		$noSurfaceData = 0;
	}
	elsif ($file =~ /Littlesville/)
	{
		$header->setSite("Littlesville AL");
		$sfc_elev = 155;
		$noSurfaceData = 0;
	}
	elsif ($file =~ /Belle_Mina/)
	{
		$header->setSite("Belle Mina AL");
		$sfc_elev = 187;
		$noSurfaceData = 0;
	}
	elsif ($file =~ /Decatur/)
	...
	elsif ($file =~ /GoodHope/)
	{
		...
		$header->setSite("Jasper AL");
		$sfc_elev = 136;
	}
	elsif ($file =~ /Somerville/) {
		$header->setSite("Somerville AL");
		$noSurfaceData = 1;
	}
	else
	{
