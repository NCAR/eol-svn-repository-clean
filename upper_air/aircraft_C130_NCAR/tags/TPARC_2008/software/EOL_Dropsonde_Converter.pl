...
##Module---------------------------------------------------------------------
# <p>The EOL_Dropsonde_Converter converts data from the EOL Aircraft into the
# CLASS format.</p>
#
# @author Joel Clawson
...
sub getNetworkName { return "USAF_C130"; }
...
sub readRawFile {
	...
	# "Release Site Type/Site ID:" Header (called Project Name/Platform in raw data)
 	my ($stationInfo, $planeInfo) = (split(',',(split(/:/,<$FILE>))[1]))[0..1];
	# print "Plane:    $planeInfo\n";
	# print "Station:  $stationInfo\n";
	# my $aircraft = getNetworkName(); 
	my ($flight,$aircraft) = (split('\/', $planeInfo));
	# aircraft name on raw data is Lockheed C-130J, s/b USAF C-130
	if ($aircraft =~ /Lockheed C-130J/)
	{
		$aircraft =~ s/Lockheed C-130J/USAF C-130/;
	}

    my $stn_id = (split(' ', $stationInfo))[0];
    $stn_id = trim($stn_id);
    $stn_id =~ s/,//g;       
    # for T-PARC 2008 files in RF23 had bad header information
	if ($stn_id =~ /AF3060117winvest/)
	{
		$stn_id =~ s/AF3060117winvest/AF306/;
	}
