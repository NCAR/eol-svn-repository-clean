...
#
#
# @author Linda Echo-Hawk 2012/05/02
# @version PREDICT USAF C-130 dropsonde processing
#          - The converter has modified to look for raw data with 
#            a *.eol extension.
#          - The converter has been revised to look for the PREDICT
#            Release Site Type Location.  This may need to be modified
...
sub readRawFile {
	...
	# for PREDICT there are two possible forms:
	# Project Name/Platform:  5302 0804A FIONA, 100903084920309/Lockheed C-130J, ---
	# Project Name/Platform:   10083107309 0607A EARL, 10083107309 0607A EARL/Lockheed C-130J, ---
	# where "0607A" is the flight name and "EARL" is the system and output should be:
	# Release Site Type/Site ID:         Lockheed C-130J AF309/10083107309 0607A EARL 
	# $id = 10090308308 1607A EARL
	# $flight = 10090116302 0308A FIONA
	# $aircraft = Lockheed C-130J
	# $stn_id = ---
	# --------------------------------------------------------------
	# my ($stationInfo, $stn_id) = (split(',',(split(/:/,<$FILE>))[1]))[1..2];
	my ($id,$stationInfo, $stn_id) = (split(',',(split(/:/,<$FILE>))[1]));
	my ($flight,$aircraft) = (split('\/', $stationInfo));
	$flight = trim($flight);
	# $id is 5302 0804A FIONA or 10090308308 1607A EARL
	my ($mission_id) = (split ' ', $id)[1];
	...
	# --------------------------------------------------------------
    # Flights for PREDICT (from Scot L.)
	#
	# Mission ID - Aircraft
	# ---------------------
	# 0607A - AF309
	...
	# --------------------------------------------------------------
	if (($mission_id =~ /0316A/) || ($mission_id =~ /01GGA/) ||
		($mission_id =~ /0408A/) || ($mission_id =~ /0804A/) ||
		($mission_id =~ /0308A/) || ($mission_id =~ /0907A/))
	{
		$stn_id = "AF302";
	}
	elsif ($mission_id =~ /1707A/)
	{
		$stn_id = "AF303";
	}
	elsif (($mission_id =~ /0508A/) || ($mission_id =~ /1007A/))
	{
		$stn_id = "AF306";
	}
	elsif (($mission_id =~ /0216A/) || ($mission_id =~ /1607A/) ||
	       ($mission_id =~ /0807A/))
	{
		$stn_id = "AF308";
	}
	elsif (($mission_id =~ /0208A/) || ($mission_id =~ /0607A/))
	{
		$stn_id = "AF309";
	}
	else
	{
		print "WARN: Unknown station id \n";
	}
    ...
    # QCODIAC platform number 386 = "Aircraft, USAF C-130"
    # BEWARE:  Select either USAF C-130 or NCAR C-130
	$station->setPlatformIdNumber(386);
