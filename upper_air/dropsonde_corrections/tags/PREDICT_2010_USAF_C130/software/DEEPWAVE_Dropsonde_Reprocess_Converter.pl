...
##Module------------------------------------------------------------------------
# <p>The DEEPWAVE_Dropsonde_Reprocess_Converter.pl script reads in corrected 
# EOL dropsonde data and the original ESC converted version of the data and
...
# @version 8 December 2016 for DEEPWAVE reprocessing of EOL dropsonde data
...
#          - A HARD-CODED fix was added for DEEPWAVE for the case 
#            of two files for June 6, 2014 where the file name 
#            times vary enough that they can't be found using the 
#            existing algorithm.
#
# use  DEEPWAVE_Dropsonde_Reprocess_Converter.pl >&! results.txt
#
##Module------------------------------------------------------------------------
    ...
    foreach my $eol_file (@eol_files) {
		...
		foreach my $esc_file (@esc_files)
		{
			if ($esc_file =~ /(\d{8})(\d{4})(\d{2})/)
			{
				...
				# A HARD-CODED fix was added for the case of the files for June 6, 2014
				# where the file name times vary enough that they can't be found using
				# the existing algorithm
				if ((($esc_date == $eol_date) && ($esc_time == $eol_time)) ||
					(($eol_file =~ /^D20140606_073100/) && ($esc_file =~ /20140606073059/)))
				{
