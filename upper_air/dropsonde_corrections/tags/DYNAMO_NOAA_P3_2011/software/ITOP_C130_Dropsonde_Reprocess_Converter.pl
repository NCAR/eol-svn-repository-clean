...
##Module------------------------------------------------------------------------
# <p>The ITOP_C130_Dropsonde_Reprocess_Converter.pl script reads in corrected EOL
...
# @version 27 December 2016 for reprocessing of EOL dropsonde data
#          for the ITOP C-130 dropsondes.
...
#          - Because there are differences in the seconds portion of
#            the file names between the ESC and EOL files, comparisons
...
#          - Because there were two planes some files were difficult
#            to match with the existing algorithm. Some hard-coded 
#            checks were added for these files. 
#          - A check was added to see if the eol file had one line
#            more than the ESC file it was matched with. If this was
#            not the case, a warning was issued that this might not
#            be the correct file.
#
# use  ITOP_C130_Dropsonde_Reprocess_Converter.pl >&! results.txt
#
##Module------------------------------------------------------------------------
package ITOP_C130_Dropsonde_Reprocess_Converter;
use strict;
...
sub parseSoundingFiles
{
	...
	while ($rev_eol_lines[$n])
	{
		...
		else
		{
			...
			# if ($eol_text[0] =~ /--------/)
			# for ITOP USAF 7 dashes instead of 8
			if ($eol_text[0] =~   /-------/)
			{
    ...
    foreach my $eol_file (@eol_files) {
		...
		foreach my $esc_file (@esc_files)
		{
			if ($esc_file =~ /(\d{8})(\d{4})(\d{2})/)
			{
				...
				{
					if (($eol_file =~ /^D20100829_012312/) && ($esc_file !~ /AF304_20100829012312/))
					{
						next;
					}
					if (($eol_file =~ /^D20100923_203659/) && ($esc_file !~ /AF307_20100923203659/))
					{
						next;
    				}
					if (($eol_file =~ /^D20100829_010757/) && ($esc_file !~ /AF304_20100829010757/))
					{
						next;
					}
					if (($eol_file =~ /^D20101014_042247/) && ($esc_file !~ /AF307_20101014042247/))
					{
						next;
					}
					if (($eol_file =~ /^D20101016_234328/) && ($esc_file !~ /AF307_20101016234328/))
					{
						next;
					}
					if (($eol_file =~ /^D20101017_011832/) && ($esc_file !~ /AF307_20101017011832/))
					{
						next;
					}
					if (($eol_file =~ /^D20101017_120943/) && ($esc_file !~ /AF304_20101017120943/))
					{
						next;
					}
