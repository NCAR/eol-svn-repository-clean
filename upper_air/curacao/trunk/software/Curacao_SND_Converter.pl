...
##Module-------------------------------------------------------------------------
# <p>The Curacao_SND_Converter.pl script is used for converting the sounding data
# from the Curacao site from the raw MicroArt variant to the CLASS format.</p>
#
# @author Joel Clawson
# @version RAINEX 1.0 This was originally created for the RAINEX project.  It was
# adapted from the Belize converter.
##Module-------------------------------------------------------------------------
package Curacao_SND_Converter;
use strict;
...
sub getNetworkName { return "Curacao"; }
...
##------------------------------------------------------------------------------
sub main {
    my $converter = Curacao_SND_Converter->new();
    $converter->convert();
}
...
##------------------------------------------------------------------------------
# @signature Curacao_SND_Converter new()
# <p>Create a new Curacao_SND_Converter instance.</p>
#
...
sub read_header {
    ...
    $station->setPlatformIdNumber(345);

    my $header = Sounding::ClassHeader->new($WARN,$station);
    $header->setType("Curacao Sounding");
    ...
    $station->setStationId("TNCC");
    $station->setStationName(sprintf("%s Curacao",trim(substr($lines[0],1,8))));
