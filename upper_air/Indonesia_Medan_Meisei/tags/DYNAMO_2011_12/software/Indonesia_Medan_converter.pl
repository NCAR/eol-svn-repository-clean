...
##Module------------------------------------------------------------------------
# <p>The Indonesia_Medan_converter.pl script is used for converting 
# high resolution radiosonde data to the EOL Sounding Composite (ESC) format.
#
# @author L.E. Cully October 2012
# @version DYNAMO  Created to convert Indonesia Medan Sonde (Meisei) data.
...
#          times in the Medan data and to assume that every record is 
...
#            Medan_DYNAMO_sounding_stationCD.out station file to the
#            ../final directory.
...
#           Note that Medan's local time data is -8 hrs off from UTC. LT-8=UTC.
#
#  WARNING: To see debug output, set $debug = 1.
#
#  Execute:
#
# @use     Indonesia_Medan_converter.pl >& output.log
#
##Module------------------------------------------------------------------------
package Indonesia_Medan_converter;
use strict;
...
printf "\nIndonesia_Medan_converter.pl began on ";print scalar localtime;printf "\n";
my $debug = 0;
my $debug_bad_asc = 0;

&main();
printf "\nIndonesia_Medan_converter.pl ended on ";print scalar localtime;printf "\n";
...
##------------------------------------------------------------------------------
# @signature void main()
# <p>Process the Indonesia Medan radiosonde data by converting it from 
# the native ASCII format into the ESC format.</p>
##------------------------------------------------------------------------------
sub main {
    my $converter = Indonesia_Medan_converter->new();
    $converter->convert();
} # main()

##------------------------------------------------------------------------------
# @signature Indonesia_Medan_converter new()
# <p>Create a new instance of a Indonesia_Medan_converter.</p>
#
# @output $self A new Indonesia_Medan_converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
...
sub parseHeader {
    ...
    # line: "Data Type:  Medan/Ascending"
    #-------------------------------------------------
    ...
    #-------------------------------------------------
    $header->setId("Indonesia_Medan");        #HARDCODED

    #-------------------------------------------------
    # This info goes in the 
    # "Release Site Type/Site ID:" header line
    #-------------------------------------------------
    $header->setSite("Medan, Indonesia/96035");  #HARDCODED
