...
#
# @author L. Cully
# @version Updated Sep/Oct 2008 by L. Cully to use latest sounding libraries.
# Handled OS differences.
#
# BEWARE: This s/w assumes the raw data will be in /raw_data/*.mrs  or *.MRS files.
# BEWARE: Update all hardcoded values. Check the code carefully for these.
##Module-------------------------------------------------------------------------
...
sub read_header {
	...
	if ($line =~ /^\s*station\s*:\s*(\S+)\s+(.+)$/i) {
	    ...
	    $header->setLine(6, "Wind Finding Methodology:",$1);
	} elsif ($line =~ /^\s*rs\-number\s*:\s*(\d+)\s*$/i) {
	    $header->setLine(5, "Radiosonde Serial Number:",$1);
	} elsif ($line =~ /^\s*$/) {
