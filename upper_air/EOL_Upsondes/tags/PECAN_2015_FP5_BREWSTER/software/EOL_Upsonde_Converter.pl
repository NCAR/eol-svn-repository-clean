...
#
# @usage EOL_Upsonde_Converter.pl [--remove_descending]
#        --remove_descending  If Scot has reviewed the converted data
#                   and asks that data collected after the balloon has
#                   burst and the sonde is descending, use this option
#
#
# @author Linda Echo-Hawk
# @version PECAN - 1 April 2016 FP5 Brewster, KS Radiosonde Data
#         - Modified hard-coded values for Brewster, KS 
#
# @author Linda Echo-Hawk
# @version PECAN - 15 March 2016 FP4 Minden, NE Radiosonde Data
#         - For the Fixed PISA Site the mobility flag was un-set
#
#
# @author Linda Echo-Hawk
# @version PECAN - 9 March 2016 NSSL1 and NSSL2 MGAUS
#         - Added cmd line switch to active the code to remove the 
#           descending data after the sonde balloon breaks
#         - Added "seconds" to the output file name to keep files
#           from overwriting each other for NSSL1 and NSSL2
#         - Set the mobility flag for the default station
#
# @author Brooks Snyder
# @version DEEPWAVE - 15 December 2014
#          - Changed from MGAUS back to GAUS
#          - Turned mobile flag off
#
# @author Linda Echo-Hawk
# @version DC3 - 13 February 2013 for the EOL MGAUS Colorado Soundings
...
my $result;
# remove the descending data after the sonde ballon bursts
my $remove_descending;
# "limit:i" i is optional, "limit=i" i is required
$result = GetOptions("remove_descending" => \$remove_descending);

if ($remove_descending)
{
	printf("Removing descending data after sonde balloon bursts.\n")
}
...
sub parseHeader {
    ...
    # Parse the release direction of the sounding.
...
sub parseRawFile {
	...
	# --------------------------------------------
	# Remove the last records in the file that 
	# are descending after the balloon burst
	# (ascent rate is negative).
    # --------------------------------------------
	if ($remove_descending)
	{
