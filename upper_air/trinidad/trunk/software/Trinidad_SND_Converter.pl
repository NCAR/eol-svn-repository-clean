...
##Module-------------------------------------------------------------------------
# <p>The Trinidad_SND_Converter.pl script is used for converting the sounding data
# from the NWS Trinidad site from the raw MicroArt variant to the CLASS format.</p>
#
# @author Joel Clawson
# @version RAINEX 1.0 This was originally created for the RAINEX project.  It was
# adapted from the Belize converter.
#
# @author Linda Cully 2008/09/30
#   Updated s/w library dirs depending on OS. BEWARE that this s/w expects the
#   raw data to be /raw_data/R* directories. Created build.xml file that can now be 
#   used to check format, autoqc, and do extraction to 5mb. 
#
#   BEWARE: Search for all hardcoded values before running. Search the code carefully.
##Module-------------------------------------------------------------------------
package Trinidad_SND_Converter;
...
sub getNetworkName { return "Trinidad"; }
...
##------------------------------------------------------------------------------
sub main {
    my $converter = Trinidad_SND_Converter->new();
    $converter->convert();
}
...
##------------------------------------------------------------------------------
# @signature Trinidad_SND_Converter new()
# <p>Create a new Trinidad_SND_Converter instance.</p>
#
...
sub read_data {
    ...
    open(my $RAW,sprintf("%s/R%s",getRawDirectory(),$asc_no)) or
	die(sprintf("Cannot open R%s\n",$asc_no));
...
sub read_header {
    ...
    $station->setCountry("TD");
    #$station->setStateCode("XX");
    $station->setNetworkIdNumber(99);
    $station->setPlatformIdNumber(347);

    my $header = ClassHeader->new($WARN,$station);
    $header->setType("Trinidad Sounding");
    $header->setProject($self->getProjectName());
    $header->setLine(5,"Ascension No:",$asc_no);

    open(my $RAW,sprintf("%s/H%s",getRawDirectory(),$asc_no)) or
	die(sprintf("Cannot open raw file: H%s\n",$asc_no));
    my @lines = <$RAW>;
    close($RAW);
    
    if (scalar(@lines) > 1) {
	die(sprintf("Too many lines in header file: H%s\n",$asc_no));
    }
    
    $station->setStationId("TTPP");
    $station->setStationName(sprintf("%s Trinidad",trim(substr($lines[0],1,8))));
    ...
    $header->setLine(6, substr($lines[0],55,1) ? "Baroswitch Number:" : "Sonde Serial Number:",
