...
#
# @author L. Cully
#   Updated s/w to use latest sounding libraries. Handled OS dependency.
#
##Module-----------------------------------------------------------------
package Averager;
use strict;
...
#	    printf("Avrg: %s\n",$averaged_record->toString());
	...
	if ($record->getTime() < 0 || $record->getTime() == $expected_time) {
		...
		my $new_record = ClassRecord->new($LOG,$infile);
...
sub create_averaged_record {
    ...
    my $averaged_record = ClassRecord->new($LOG,$filename,$last_out_record);
...
sub create_record {
    ...
    my $filename = shift;
    my $record = ClassRecord->new($LOG,$filename,$prev_record);
