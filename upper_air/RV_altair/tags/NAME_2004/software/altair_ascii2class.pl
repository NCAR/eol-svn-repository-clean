...
##Module---------------------------------------------------------------
# <p>The altair_ascii2class.pl is a script that converts ASCII PTU and/or 
# WND sounding files into the sounding CLASS format for the Altair ship.</p>
#
# @author Joel Clawson
# @version 1.1 This was update to handle ////// dew point values and to
# print out the correct information for the Altair ship and the NAME 
# project.  It also has an addition to drop all data after there are three
# lines of data with negative ascension rates.  This is to remove the
# data after the balloon has popped.
#
# @author Darren R. Gallant JOSS
# @version 1.0 This was the RON_BROWN.pl script.
##Module---------------------------------------------------------------

use POSIX;
$TRUE = 1;$FALSE = 0;
$stn_name = "R/V Altair";$stn_id = "XCWH";
...
$header[9] = "Radiosonde Serial Number:          ";
...
foreach $file(@files){
	...
	if($PTU_FLAG){
		...
		if ($line =~ /rs.number\s*:\s*(\d+)/i) {
		    $INFO{$TIMESTAMP}{"SERIAL"} = $1;
		}
		if($line =~ /Location/){
	            &location($line);
		}# end if
...
sub writeheader{
   ...
   printf OUT ("%s%s\n",$header[5],$_[0]);
   printf OUT ("%s%s\n",$header[9],$INFO{$_[0]}{"SERIAL"});
   for $i(0..3){ printf OUT ("%s\n","/");}
...
sub writefile{
	...
	if($FLAG{$_[0]}{"PTU"} && $FLAG{$_[0]}{"WND"}){
	    ...
	    if (length($outline[9]) > 5) { $outline[9] = 999.0; }
	...
	if ($outline[9] != 999.0) {
	    # Check for three negative ascend rates and quit.
	    # This is the condition for a balloon pop.
	    if ($outline[9] < 0) { $ascend_count++; }
	    else { $ascend_count = 0; }
	}
	last if ($ascend_count >= 3);	
	
    }# end foreach
    if($POSITION){&stereographic(@OUTFILE);}
    close(OUT);
}# end sub writefile
