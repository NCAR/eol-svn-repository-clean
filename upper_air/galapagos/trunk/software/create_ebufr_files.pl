...
my $PROJECT = "PACS";
my $PROGRAM = "/work/software/PACS/library/upper_air/EBUFR/bin/class2ebufr_Solaris";

my $INPUT_DIR = "../final";
my $OUTPUT_DIR = "../codiac";
...
sub main {
	...
	push(@{ $dates{sprintf("%04d%02d",$1,$2)}}, sprintf("%s/%s",$INPUT_DIR,$file));
    }
...
sub create_ebufr_file {
    ...
    move($file,sprintf("%s_Galapagos_%s",$PROJECT,$file));
