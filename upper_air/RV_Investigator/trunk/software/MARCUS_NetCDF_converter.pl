...
#
##Module-------------------------------------------------------------------------
# <p>The MARCUS_NetCDF_converter.pl is based on the ARMnetCDF_to_ESC.pl script 
# which is used for converting the ARM-CART NetCDF sounding data files to the 
# EOL Sounding Composite (ESC) Format.
#
# @usage MARCUS_NetCDF_converter.pl [--limit] [--num_soundings=8] [--max=10]
...
#
#
# @author Linda Echo-Hawk Sept 2018
# @version SOCRATES
# MARCUS_NetCDF_converter.pl --limit --num_soundings=8 >&! 3-hourly_results.txt
#            Based on the ARMnetCDF_to_ESC.pl converter
#          - Added functionality to calculate geopotential height.
#          - The sonde serial number is from the global attributes 
#            section of the NetCDF data file.
...
#
# @author Linda Echo-Hawk Sept 2015
# @version PECAN
...
#
##Module-------------------------------------------------------------------------
# package MARCUS_NetCDF_converter;
...
printf "\nMARCUS_NetCDF_converter.pl began on ";print scalar localtime;printf "\n";
...
printf "\nMARCUS_NetCDF_converter.pl ended on ";print scalar localtime;printf "\n";
...
##------------------------------------------------------------------------------
sub main {
    # my $converter = MARCUS_NetCDF_Converter->new();
...
##------------------------------------------------------------------------------
# @signature MARCUS_NetCDF_converter new()
# <p>Create a new MARCUS_NetCDF_converter instance.</p>
#
...
sub processHeaderInfo
{
	...
	# -------------------------------------------------------------
	# NOTE: In July 2013 the netCDF file convention has changed and
	# the release time must be obtained by adding the $basetime and
	# $nomOffset variables.
	# 
    # OBSOLETE: Determine the actual release time and date from the file's
    # "base_time" variable 
    # NOTE: For Gan $base_time = 1316671260 ;
	# -------------------------------------------------------------
    print "base_time = $basetime\n";
	
	my $releaseTime = $basetime + $nomOffset;
	print "release time = $releaseTime or base $basetime + offset $nomOffset\n";
	my ($rd, $rt) = getDateFromSeconds($releaseTime);
	print "\tFROM getDateFromSeconds: release date = $rd  time = $rt\n";

    # ---------------------------------------------
	# This has been changed to use the $releaseTime
    # ---------------------------------------------
	...
	my ($btYear, $btMonth, $btDay) = split(/\//, $rd);
	my ($btHour, $btMinute, $btSecond) = split(/:/, $rt);
	...
	# -------------------------------------------------------------
	# NOTE: Instead of base_time, we must now subtract from 
	# $releaseTime
	# -------------------------------------------------------------
    ...
    # Change this to release time instead of base time 
    my $nom_time = $releaseTime - $nomOffset + ($nomZone * $nomFactor);
	...
	# -------------------------------------------------------------
	# Compare file's release time (base_time + offset) with the
	# file name date/time
	#
    # OBSOLETE: Compare file's base_time value with the file name date/time
	# -------------------------------------------------------------               
	if (($year != $btYear) || ($month != $btMonth) || ($day != $btDay))
	{
		print "WARNING: File release time (base_time+offset) DATE does not match file name DATE\n";
	}
	elsif (($hour != $btHour) || ($minute != $btMinute) || ($second != $btSecond))
	{
		print "WARNING: File release time (base_time+offset) TIME does not match file name TIME\n";
	}
	...
	# -------------------------------------------------------------
    printf(OUTFILE "Data Type:                         DOE/ARM Sounding Data/Ascending\n");
    printf(OUTFILE "Project ID:                        SOCRATES - MARCUS\n");
    ...
    printf(OUTFILE "Radiosonde Serial Number:          %s\n", $global_atts{"serial_number"});
    printf(OUTFILE "Radiosonde Type:                   %s\n", $sondeType);
	printf(OUTFILE "Ground Station Software:           Vaisala DigiCORA 3.64\n");
    printf(OUTFILE "/\n/\n/\n");
...
sub parse_data_rec
{
	...
	# Note that the last three parms in calculateAltitude are the
	# pressure, temp, and dewpt (and the "1" indicates verbose output)
    ...
    foreach (my $recnum=0; $recnum < $recdimLen; $recnum++) 
	{
		...
		{
			$output{Time}[$i] = (getVar("time_offset",$var,$i) - $nomOffset); #Time offset from base time
			...
			{     
                # if ($recnum < 3) # for all records
                if ($recnum == 1000) # a quick test
				{
					if ($debug_geopotential_height)
					{
						# print "prev_press < 9990.0 - NOT missing\n"; 
						print "At Time $output{Time}[$i] \n";
						print "\tPREVIOUS press $output{Press}[$i-1] \n";
						print "\tPREVIOUS Temp $output{Temp}[$i-1] \n";
						print "\tPREVIOUS Dewpt $output{Dewpt}[$i-1] \n";
						print "\tPREVIOUS Alt $output{Alt}[$i-1] \n";
						print "\tCURRENT Press $output{Press}[$i] \n";
						print "\tCURRENT Temp $output{Temp}[$i] \n";
						print "\tCURRENT Dewpt $output{Dewpt}[$i] \n";
						print "\tCURRENT RAW ALT $output{Alt}[$i] \n";
					}
				}
			}
            ...
            if ($output{Time}[$i] > 1) 
            {
				...
				if (($output{Press}[$i-1] != 9999.0) && ($output{Temp}[$i-1] != 999.0) &&
					($output{Dewpt}[$i-1] != 999.0) && ($output{Alt}[$i-1] != 99999.0))
				{
					$geopotential_height = calculateAltitude($output{Press}[$i-1],
					                                         $output{Temp}[$i-1],
															 $output{Dewpt}[$i-1],
															 $output{Alt}[$i-1],
															 $output{Press}[$i],
					...
					{
						$output{Alt}[$i] = $geopotential_height;
						# if ($recnum == 1000)
						#{
						#    print "\tGeopotential ht $output{Alt}[$i] at time $output{Time}[$i]\n";
						#}
					}
					else
					{
			...
			} # end geopotential height calculation
    	...
    	# For the 2-second data soundings, 5000 records is the max.
    	if ($limit)
		{
            # HARD-CODED value
			if ($recnum < 5000)
...
sub readRawFile {
	...
	# marsondewnpnM1.b1.20180101.120000.cdf
	# Old example from ARM below:
