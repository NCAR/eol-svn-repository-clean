...
##Module------------------------------------------------------------------------
# <p>The Indonesia_Cengkareng_converter.pl script is used for converting 
# high resolution radiosonde data to the EOL Sounding Composite (ESC) format.
#
# @author L.E. Cully August/September 2012
# @version DYNAMO  Created to convert Indonesia Cengkareng Sonde (Meisei) data.
...
#          times in the Cengkareng data and to assume that every record is 
...
#            Cengkareng_DYNAMO_sounding_stationCD.out station file to the
#            ../final directory.
...
#           Note that Cengkareng's local time data is -7 hrs off from UTC. LT-7=UTC.
#
#  WARNING: To see debug output, set $debug = 1.
#
#  Execute:
#
# @use     Indonesia_Cengkareng_converter.pl >& output.log
#
##Module------------------------------------------------------------------------
package Indonesia_Cengkareng_converter;
use strict;
...
printf "\nIndonesia_Cengkareng_converter.pl began on ";print scalar localtime;printf "\n";
my $debug = 0;
my $debug_bad_asc = 0;

&main();
printf "\nIndonesia_Cengkareng_converter.pl ended on ";print scalar localtime;printf "\n";
...
##------------------------------------------------------------------------------
# @signature void main()
# <p>Process the Indonesia Cengkareng radiosonde data by converting it from 
# the native ASCII format into the ESC format.</p>
##------------------------------------------------------------------------------
sub main {
    my $converter = Indonesia_Cengkareng_converter->new();
    $converter->convert();
} # main()

##------------------------------------------------------------------------------
# @signature Indonesia_Cengkareng_converter new()
# <p>Create a new instance of a Indonesia_Cengkareng_converter.</p>
#
# @output $self A new Indonesia_Cengkareng_converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
    ...
    # HARDCODED
    $self->{"PROJECT"} = "DYNAMO";
    $self->{"NETWORK"} = "Indonesia_Jakarta_Cengkareng";
...
sub parseHeader {
    ...
    # line: "Data Type:  Cengkareng/Ascending"
    #-------------------------------------------------
    ...
    #-------------------------------------------------
    $header->setId("Indonesia_Jakarta_Cengkareng");        #HARDCODED

    #-------------------------------------------------
    # This info goes in the 
    # "Release Site Type/Site ID:" header line
    #-------------------------------------------------
    $header->setSite("Jakarta-Cengkareng, Indonesia/96749");  #HARDCODED
