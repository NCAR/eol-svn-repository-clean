#! /usr/bin/perl -w
##Module------------------------------------------------------------------------
# <p>The Hobart_Converter.pl script is used for converting high
...
printf "\nHobart_Converter.pl began on ";print scalar localtime;printf "\n";
my $debug = 0;
&main();
printf "\nHobart_Converter.pl ended on ";print scalar localtime;printf "\n";

##------------------------------------------------------------------------------
# @signature void main()
# <p>Process the Hobart radiosonde data by converting it from 
# the native ASCII format into the ESC format.</p>
##------------------------------------------------------------------------------
sub main {
    my $converter = Hobart_Converter->new();
    $converter->convert();
}

##------------------------------------------------------------------------------
# @signature Hobart_Converter new()
# <p>Create a new instance of a Hobart_Converter.</p>
#
# @output $self A new Hobart_Converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
...
sub parseHeader {
    ...
    $header->setSite("Hobart, Australia/94975");
	...
	if ($file =~ /94975_(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})/)
	{
...
sub readDataFiles {
    ...
    my @files = grep(/^94975_\d{14}\.tsv/,sort(readdir($RAW)));
    closedir($RAW);
