#! /usr/bin/perl -w
##Module------------------------------------------------------------------------
# <p>The CSU_Mobile_Radiosonde_Converter.pl script is used for converting high
# resolution radiosonde data from ASCII formatted data to the EOL Sounding 
# Composite (ESC) format.</p> 
#
# @author Linda Echo-Hawk
# @version RELAMPAGO 2018
#          based on the PECAN CSU_Mobile_2015_Sounding_Converter.pl
#           - The converter expects filenames in the format:
#             edt_YYYYMMDD_HHmm.txt (e.g., edt_20181031_1513.txt)
#             where YY=year, MM=month, DD=day, HH=hour, mm=minute.
#           - The file contains header info on lines 1-27. Actual data starts
#             on line 31.
#           - The radiosonde type and ID is obtained from the header information.
...
#           - Missing values are represented by "999.999999" in the raw data.
#           - Some raw data files have records that go from time=0 to time=2 and
#             skip time=1.
#
#
# @author Linda Echo-Hawk
...
sub parseHeader {
	...
	foreach my $line (@headerlines) 
	{
	    ...
	    if ($line =~ /Software version/i)
	    {
			...
			my $version = join " ", trim($sw_version[2]), trim($sw_version[3]);
            #-----------------------------------------------------------
            # "Software version                              MW41 2.2.1"
            #-----------------------------------------------------------
			my $sonde_id_label = "Ground Station Softwarer";
            $header->setLine(7,"Ground Station Software: ", $version);
	    }



		# ---------------------------------
		# If $index == 30 then this is
...
sub parseRawFile {
	...
	foreach my $line (@lines) {
	    ...
	    if (($index < 30) || ($line =~ /^\s*$/)) { $index++; next; }
	    
	    my @data = split(' ',$line);
	    my $record = ClassRecord->new($WARN,$file);

		# -------------------------------------------------
		# time   $data[0]       windspeed   $data[6]
		# height $data[1](N/A)  wind dir    $data[7]
		# press  $data[2]       ascent      $data[8]
		# temp   $data[3]       lat         $data[9]
		# dewpt  $data[4]       lon         $data[10]
		# RH     $data[5]       geopot ht   $data[11]
		# -------------------------------------------------
        
		$record->setTime($data[0]);
	    $record->setPressure($data[2],"mb") if ($data[2] !~ /^999/);
	    $record->setTemperature(($data[3]),"C") if ($data[3] !~ /^999/);    
		$record->setDewPoint(($data[4]),"C") if ($data[4] !~ /^999/);
	    $record->setRelativeHumidity($data[5]) if ($data[5] !~ /^999/);
	    $record->setWindSpeed($data[6],"m/s") if ($data[6] !~ /^999/);
	    $record->setWindDirection($data[7]) if ($data[7] !~ /^999/);
        $record->setAscensionRate($data[8],"m/s") if ($data[8] !~ /^999/);
        $record->setLatitude($data[9],$self->buildLatlonFormat($data[9])) if ($data[9] !~ /^999/);
        $record->setLongitude($data[10],$self->buildLatlonFormat($data[10])) if ($data[10] !~ /^999/);
		$record->setAltitude($data[11],"m") if ($data[11] !~ /^999/);
