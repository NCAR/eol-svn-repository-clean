...
##Module------------------------------------------------------------------------
# <p>The MSU_iMet_Mobile_Radiosonde_Converter.pl script is used for converting 
# high resolution radiosonde data from a csv format to the EOL Sounding 
# Composite (ESC) format.</p> 
#
#
# @author Linda Echo-Hawk 20 October 2016
# @version VORTEX-SE_2016 MSU iMet Soundings
#          based on ULM_Radiosonde_Converter.pl
#          - A separate text file (MSU_iMet_locs.txt) provided a list
#            of locations and their surface lat/lon/elev values. This
#            file was read into the converter.
#          - Release location elevation is taken from the MSU_iMet_locs.txt
#            file while lat/lon info is taken from the surface record.
#          - Latitude, longitude, and time after release are all missing 
#            after the surface record in the raw files. For the time we 
#            assume all data records are at 10 second intervals.  The 
#            Lat and Long remain missing above the surface.
...
#
##Module------------------------------------------------------------------------
package MSU_iMet_Mobile_Radiosonde_Converter;
use strict;
...
printf "\nMSU_iMet_Mobile_Radiosonde_Converter.pl began on ";print scalar localtime;printf "\n";
my $debug = 0;
my $debug_geopotential_height = 0;

&main();
printf "\nMSU_iMet_Mobile_Radiosonde_Converter.pl ended on ";print scalar localtime;printf "\n";

my $filename;

##------------------------------------------------------------------------------
# @signature void main()
# <p>Process the MSU_iMet Mobile radiosonde data by converting it from 
# the native ASCII format into the ESC format.</p>
##------------------------------------------------------------------------------
sub main {
    my $converter = MSU_iMet_Mobile_Radiosonde_Converter->new();
    $converter->convert();
}

##------------------------------------------------------------------------------
# @signature MSU_iMet_Mobile_Radiosonde_Converter new()
# <p>Create a new instance of a MSU_iMet_Mobile_Radiosonde_Converter.</p>
#
# @output $self A new MSU_iMet_Mobile_Radiosonde_Converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
...
# <p>Create a default station for the MSU_iMet network using the 
...
sub buildDefaultStation {
    ...
    $station->setStationName("MSU_iMet");
    # HARD-CODED
...
sub parseHeader {
    ...
    $header->setType("MSU_iMet Mobile Sounding Data");
	...
	foreach my $line(@surfaceValues)
	{
		...
		# raw data file name example: 2016-03-14_0530Z_MSU_IMet.txt
		# sfc_alt ($line):
		# 2016-03-14_0530Z_MSU_IMet.txt 35.244472 -88.159861 137m Olivet, TN
		# $location_data[0] = 2016-03-14_0530Z_MSU_IMet.txt
		# $location_data[1] = 35.244472
		# $location_data[2] = -88.159861
		# $location_data[3] = 137m
		# $location_data[4] = Olivet,
		# $location_data[5] = TN
	    
		# my @location_data = split(" ", $line);
		my ($filename,$lat,$lon,$elev,@rest) = split(" ", $line);
	...
	my $sfc_lat = $headerData[0];
	my $sfc_lon = $headerData[1];
	...
	my $groundStationSoftware = "iMet OS II software version 3.49.03C";
...
sub parseRawFile {
	...
	foreach my $line (@lines) 
	{
		...
		if ($index >= 3)
		{
			...
			# 10 second increments, so increment
...
sub readSurfaceValuesFile {
    ...
    open(my $FILE, sprintf("MSU_iMet_locs.txt")) or die("Can't read file into array\n");
