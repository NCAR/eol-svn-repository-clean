...
##Module------------------------------------------------------------------------
# <p>The MSU_Radiosonde_Converter.pl script is used for converting 
# high resolution radiosonde data from the SHARPpy ascii format to the 
# EOL Sounding Composite (ESC) format.</p> 
#
# @author Summer Stafford August 2019
# @version Meso 2018-19 MSU Radiosonde Data
#	- The first record is the surface record.
#	- Assume the data are at 10-sec intervals.
#	- Lat/Lon values were provided for each of the
#	  two locations (Memphis, TN and Starkville, MS)
#	  by Scot L.
#	- Need to derive RH and wind components and convert
#	  wind speed to m/s
#	- For all 2018 files and some 2019 Memphis files
#	  (201902060600,  201902061200, and 201902221200), the
#	  converter calculates the geopotential heights using 
#	  surface elevations given my Scot L.
#	- The converter uses a command line switch to determine
#	  whether or not to calculate geopotential height- the data
#	  files were located in two directories, one which needed
#	  the height calculated and one which did not
#
...
#
##Module------------------------------------------------------------------------
package MSU_Radiosonde_Converter;
use strict;
...
printf "\nMSU_Radiosonde_Converter.pl began on ";print scalar localtime;printf "\n";
my $debug = 0;
my $debug_geopotential_height = 0;

# Read command line arguments
my $result;
# $calc_geopot only when in the calc_geopot_raw data directory, else use altitude given
# Default is to not calculate
my $calc_geopot;
$result = GetOptions("calc" => \$calc_geopot);

if($calc_geopot){
	printf("\nProcessing records using calculated geopotential height.");
}
else {
	printf("\nProcessing records using given altitude.");
}

&main();
printf "\nMSU_Radiosonde_Converter.pl ended on ";print scalar localtime;printf "\n";
...
##------------------------------------------------------------------------------
sub main {
    my $converter = MSU_Radiosonde_Converter->new();
    $converter->convert();
}

##------------------------------------------------------------------------------
# @signature MSU_Radiosonde_Converter new()
# <p>Create a new instance of a MSU_Radiosonde_Converter.</p>
#
# @output $self A new MSU_Radiosonde_Converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
    ...
    if($calc_geopot){
	$self->{"RAW_DIR"} = "../calc_geopot_raw_data"
    }
    else{
...
sub convert {
    my ($self) = @_;
    #printf "\n Directory used: ";print $self->{"RAW_DIR"};printf "\n";
...
sub parseHeader {
    ...
    $header->setType("MSU Radiosonde Data");
	...
	#-----------------------------------------------------------
	# Calculate geopotential height for other files based on command line switch	
	# ----------------------------------------------------------
	else{	
		if($file =~ /Starkville/) {$sfc_elev = 84}
		elsif($file =~ /Memphis/) {$sfc_elev = 88}
		$header->setAltitude($sfc_elev,"m"); 
 	}	
    # -------------------------------------------------
   	# Other header info provided by Scot
	# -------------------------------------------------
    my $sondeType = "iMet-4A";
	$header->setLine(5,"Radiosonde Type:", ($sondeType));
	my $groundStationSoftware = "iMetOS-II Version 3.124";
	...
	# upperair.MSU_sonde.201810311800.Starkville_MS_SHARPpy.txt OR
	# upperair.MSU_sonde.201904190600.Memphis_TN_SHARPpy.txt  OR
	# upperair.MSU_sonde.201904190600.Starkville_MS_SHARPpy.txt    
# ----------------------------------------------------------
    my $date;
	my $time;
...
sub parseRawFile {
	...
	foreach my $line (@lines) 
	{
		...
		if ($index >= 6)
		{
			...
			# TODO calculate geopotential height for 2018 & specific Memphis soundings
			#--------------------------------------------
			my $temp;
			my $height;
			my $dewpoint;
			...
			# use 10 second increments, so increment
			# $raw_data_time by ten for lines
			# with $index > 6
			#-------------------------------------
			if ($index == 6)
			{                                                 
		...
		if ($calc_geopot){
	    #---------------------------------------------------------
	    # Calculation geopotential height (this code from VORTEX2 SUNY)
	    #
	    ...
	    if ($index == 6) {
		    ...
		    if (($previous_record->getPressure() < 9990.0) && ($record->getPressure() < 9990.0)) {
	    	    ...
	    	    # $geopotential_height = calculateAltitude($previous_record->getPressure(), $previous_record->getTemperature(), 
                    #					undef, $previous_record->getAltitude(), $data[4], $data[5], undef, 1);
