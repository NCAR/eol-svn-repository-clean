...
#
# @author L. Echo-Hawk 30 March 2010
# @version PLOWS 2009-2010
#          Due to issues found with the release location values, Scot Loehrer 
#          provided lat/lon/alt values to be used and code was added to use
#          these new values rather than the ones in the raw data.
#
# @author L. Cully
...
printf "\nUMO_SND_Converter began on ";print scalar localtime;printf "\n";
&main();
printf "\nUMO_SND_Converter ended on ";print scalar localtime;printf "\n";
#---------------------------------------------------------
...
sub read_header {
    ...
    while ($line ne "--------------------------------------------------------------------------\r\n") {
        ...
        if ($line_parts[0] eq "PHYSICAL")
		   ...
		   # Original code commented out to 2009-2010 data.
           #---------------------------------------------------
           # my $lat = $line_parts[4];
           ...
           # ------------------------------------------
		   # Due to issues with 2009-2010 lat/lon data,
           # Scot provided higher resolution data 
		   # ------------------------------------------
           my $orig_lat = $line_parts[4];
           my $lat;
		   my $lon;
		   my $alt;
		   if ($orig_lat =~ /38.57'N/)
		   ...
		   elsif ($orig_lat =~ /38.59'N/)
		   {
			   $lat = "38 58 30";
			   $lon = "93 57 10";
			   $alt = 284;
		   }
 		   elsif ($orig_lat =~ /42.00'N/)
		   {
			   $lat = "42 00 28";
			   $lon = "93 34 56";
			   $alt = 268;
		   }       
  		   elsif ($orig_lat =~ /39.42'N/)
		   {
			   $lat = "39 41 41";
			   $lon = "86 36 57";
			   $alt = 253;
		   }  
  		   elsif ($orig_lat =~ /41.49'N/)
		   {
			   $lat = "41 48 60";
			   $lon = "90 14 24";
			   $alt = 179;
		   }  
  		   elsif (($orig_lat =~ /34.17'N/) || ($orig_lat =~ /34.30'N/)) 
		   {
			   $lat = "34 29 48";
			   $lon = "87 16 58";
			   $alt = 203;
		   }  
   		   elsif ($orig_lat =~ /42.50'N/)
		   {
			   $lat = "42 50 04";
			   $lon = "88 45 35";
			   $alt = 259;
		   }  
   		   elsif (($orig_lat =~ /38.54'N/) || ($orig_lat =~ /40.16'N/)) 
		   {
			   $lat = "40 15 31";
			   $lon = "89 13 24";
			   $alt = 216;
		   ...
		   {
			   print "WARNING:  No replacement lat/lon data inserted\n";
		   }
           ...
           $station->setElevation($alt,"m"); # raw in meters
           $header->setAltitude($alt,"m");
           $first_record->setAltitude($alt,"m");
           # ---------------------------------------------------
           # End substitute in new lat/lon/alt values
           # ---------------------------------------------------
...
sub create_record {
    ...
    #   specific years.
	#   NOTE:  Also valid for 2009-2010 data.
    #-----------------------------------------------------------------
    if ( $data[4] > 1800 )
       {
