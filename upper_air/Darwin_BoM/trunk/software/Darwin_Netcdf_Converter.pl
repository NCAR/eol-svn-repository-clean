...
#
##Module-------------------------------------------------------------------------
# <p>The Darwin_Netcdf_Converter.pl script is used for converting the ARM
# NetCDF sounding data files to the EOL Sounding Composite (ESC) Format.
#
# @usage Darwin_Netcdf_Converter.pl [--limit] [--num_soundings=8] [--max=10]
...
#
# @author Linda Echo-Hawk 2012-05-14
# @version DYNAMO for processing of Darwin ARM 2011-12 soundings, created 
#          ARMnetCDF_to_ESC.pl script.  We received these soundings from ARM, 
#          but the Darwin site is actually run by the Australian Bureau of 
#          Meteorology. The data are in the ARM netcdf format. 
# @use Darwin_Netcdf_Converter.pl --limit >&! results.txt
#          NOTE:  Soundings are 6-hourly (4 per day)
#          - Revised the nominal time code to work correctly for 6-hourly
#          - Improved nominal time code documentatoin
#
# BEWARE:  The SCUDS skew-t generator cannot handles files with times >9999 
#          seconds (per Scot Loehrer to Linda Cully, 2008-11-12).
#
#
##Module-------------------------------------------------------------------------
package Darwin_Netcdf_Converter;
use strict 'vars';
...
printf "\nDarwin_Netcdf_Converter.pl began on ";print scalar localtime;printf "\n";
...
printf "\nDarwin_Netcdf_Converter.pl ended on ";print scalar localtime;printf "\n";
...
##------------------------------------------------------------------------------
sub main {
    my $converter = Darwin_Netcdf_Converter->new();
    $converter->convert();
}
...
##------------------------------------------------------------------------------
# @signature Darwin_Netcdf_Converter new()
# <p>Create a new Darwin_Netcdf_Converter instance.</p>
#
...
sub processHeaderInfo
{
    ...
    # HARD-CODED value
	my $sondeType = "Vaisala RS92-15 with radar-tracked winds";
    ...
    else
	{
		...
		# nominal to the actual time and get the multiplier.
	    # -------------------------------------------------------------
	...
	#
	#
    # __________  __________________  __________________  __________________	________
    #           ||                  ||                  ||                  ||
    # ||--|--|--||--|--|--||--|--|--||--|--|--||--|--|--||--|--|--||--|--|--||--|--|--||
    ...
    #
    #
    # ($basetime - $nomOffset) + ($nomZone * $nomFactor)
    # |______________________|   |_______|   |_________|
    #   sets the time back       determined   number of
    #   to midnight             in the code   seconds in 6 hours
    #                           |_______________________|
    # 	   					    advances the time in seconds
    #   					    to the correct nominal time
    #
	# -------------------------------------------------------------
	my $nomFactor;
	if ($three_hourly)
	...
	# --------------------------------------------------------------
	# See explanation above for computing $nom_time
    # --------------------------------------------------------------
	...
	# -------------------------------------------------------------
    printf(OUTFILE "Data Type:                         BoM Radiosonde/Ascending\n");
	...
	printf(OUTFILE "Ground StationSoftware:            Digicora III\n");
    printf(OUTFILE "/\n/\n/\n/\n");
...
sub parse_data_rec
{
    ...
    foreach (my $recnum=0; $recnum < $recdimLen; $recnum++) 
	{
    	...
    	for (my $i = $recnum; $i < $recdimLen && $i <= $recnum+1; $i++) 
		{
            ...
            # Handle dewpoints with >5 chars, e.g., -100.0a
...
sub readRawFile {
    ...
    if (!$limit)
	{
        ...
        $outfile = sprintf "%s/%s_%04d%02d%02d%02d%02d%s", getVerboseOutputDirectory(),
	                       getNetworkName(), $year, 
	...
	else
	{
        ...
        $outfile = sprintf "%s/%s_%04d%02d%02d%02d%02d%s", getOutputDirectory(),
	                       getNetworkName(), $year, 
		   				   $month, $day, $hour, $minute, $ext; 
        open (OUTFILE,">$outfile") 
    ...
    #-----------------------------------------------------------------
    my $station = $self->{"stations"}->getStation(getNetworkName(),$self->{"NETWORK"}, 											$baselat, $baselon, $basealt);
    if (!defined($station)) {
