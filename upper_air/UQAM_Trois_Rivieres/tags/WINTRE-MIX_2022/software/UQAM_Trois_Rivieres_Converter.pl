...
##Module---------------------------------------------------------------------
# <p>The UQAM_Trois_Rivieres_Converter.pl script is used for converting WINTRE-MIX
# Canadian University of Quebec at Montreal (UQAM) sounding data from its raw
# ASCII Text format into the EOL Sounding Composite (ESC) format.</p>
#
# INPUT: UQAM_Trois_Rivieres raw data in the following format/order. See sample 
...
#   2.   UTC_Time: The time at which the sounding was launched 
#   3.   FltTime: The elapsed time in seconds since the sounding was launched
#   4.   UTC Lat/N: The latitudinal position of the sounding 
#   5.   UTC Lon/E: The longitudinal position of the sounding
#   6.   Ascent*: The ascent rate of the sounding in m min-1 or Meters per minute
#   7.   Alt_MSL*: The altitude of the sounding in meters above mean sea level 
#   8.   Press: The pressure measured by the radiosonde in hPa  = mb2
#   9.   GPM_AGL: The geopotential meters of the sounding above ground level 
#   10.  GPM_MSL: The geopotential meters above mean sea level  -  **SL says use this for Alt**
#   11.  Temp: The temperature measured by the radiosonde in degrees Celsius
#   12.  DP: The dew point measured by the radiosonde in degrees Celsius
#   13.  RelHum: The relative humidity measured by the radiosonde in %
#   14.  WSpeed: The wind speed measured by the radiosonde in ms-1 or Meters per second
...
#
#   2022-02-23     4:00:28 AM            0.0   46o20'58.2"N  072o4'52.5"W
#             50.0        1000.28            0.0           50.0          -2.80
#             -3.1          98.10            6.0             60
#
...
#
# WARNING: For the WINTRE-MIX Field Collected Sounding dataset readme document regarding
#      the Trois Rivieres data, "There were some instances during the first IOP, and 
#      during a couple of other UQAM soundings, where station measurements of wind were 
#      unavailable. In these instances, the wind is listed as 0 m/s with a direction of 0 Degrees. 
#      These instances should be treated as missing data". 
#
#      The lat/lon/elev in the output header is set to be the same as the lat/lon/elev
#      on the first data record with time equal 0.0 seconds.
#
# @author Linda Cully  1 August 2022
# @version WINTRE-MIX (2022)
#
##Module---------------------------------------------------------------------
package UQAM_Trois_Rivieres;
use strict;
...
printf "\nUQAM_Trois_Rivieres_Converter.pl began on ";print scalar localtime;printf "\n";
&main();
printf "\n\nUQAM_Trois_Rivieres_Converter.pl ended on ";print scalar localtime;printf "\n";
...
sub main 
   {
   my $converter = UQAM_Trois_Rivieres->new();
   $converter->convert();
...
sub new {
   ...
   $self->{"NETWORK"} = "UQAM Trois Rivieres QC";
...
sub printSounding 
   ...
   if (!defined($station)) 
      ...
      $station->setPlatformIdNumber(99); # Unknown for UQAM Trois Riveries
...
sub readRawDataFiles 
   ...
   #    "upperair.sounding.YYYYMMDDhhmm.UQAM-Trois-Rivieres.txt" and an example is
   #    "upperair.sounding.202202222200.UQAM-Trois-Rivieres.txt"
   # ----------------------------------------------------------------------------------------
   my @files = grep(/\.UQAM-Trois-Rivieres.txt$/,readdir($RAW)); # Only process Trois Rivieres input
   closedir($RAW);
...
# output (*.cls) file.</p>
#
# Form of input file name: "upperair.sounding.202202222200.UQAM-Trois-Rivieres.txt"
#
...
sub readRawFile {
   ...
   my $file_prefix = "UQAM_Trois_Rivieres_";  # prefix to output file names
   ...
   # HARDCODED project specific values
   # ----------------------------------
   $header->setType("UQAM Sounding Data ");
   $header->setReleaseDirection(" Ascending");
   $header->setProject($self->{"PROJECT"});

   $header->setLine(5,"Radiosonde Frequency:", "403MHz"); # Updated
   $header->setLine(6,"Radiosonde Type/RH Sensor Type:", "iMet-4 radiosonde/thin-film capacitive polymer"); # Updated
   $header->setLine(7,"Ground Station Software:", "iMet-3050A iMetOS-II software version 3.133.0"); # Updated
   ...
   # -------------------------------------------------------------------------------
   # Originally, code set header lat/lon/elev to fixed values of
   # Trois-Rivieres, QC, Canada: 46.349836, -72.581353 (elev: 52 m). These values 
   # are now set to the lat/lon/elev of the first data record since that is the
   # release location.  See far below for where not set in header. 
   # Old Code:
   # my $fixedLat = 46.349836;  # decimal degrees - HARDCODED
   # my $fixedLon = -72.581353; # decimal degrees - HARDCODED
   # my $fixedElev = 52.0;      # meters - HARCODED
   # -------------------------------------------------------------------------------

   # Site is fixed as Trois-Rivieres, QC, Canada: 46.349836, -72.581353 (elev: 52 m)
   $header->setSite("UQAM");
   $header->setId("Trois Rivieres, QC");
   $header->setSite(sprintf("%s %s", $header->getSite(), $header->getId()));

   # ----------------------------------------------------------
   # Output file names: "UQAM_Trois_Rivieres_YYYYMMDDHHMM.cls:"
   # ----------------------------------------------------------
   my $output_filename = sprintf("%s%s.cls", $file_prefix, $filename_parts[2]);
   ...
   foreach my $line (<$FILE>) 
      ...
      $record->setTime(trim($data[3]));  # Time from the data record since launch. Seconds since launch.

      # ------------------------------------------------------------------
      # Parse the latitude and longitude values to output decimal lat/lon.
      # ------------------------------------------------------------------
      my @lat = split(//, $data[4]); # 46o20'57.5"N  - Split into chars
      ...
      my @lon =  split(//, $data[5]); # 072o34'52.9"W - Split into chars
      ...
      $record->setPressure(trim($data[8]),"mbar") unless (trim($data[8]) == 99999);
      $record->setAltitude(trim($data[10]),"m") unless (trim($data[10]) == 99999);    # GPM_MSL - Per SL

      $record->setTemperature(trim($data[11]),"C") unless (trim($data[11]) == 99999);
      $record->setDewPoint(trim($data[12]),"C") unless (trim($data[12]) == 99999);
      $record->setRelativeHumidity(trim($data[13])) unless (trim($data[13]) == 99999);
      ...
      # ---------------------------------------------------------------------------
      # See the WARNING section of this software's header saying that in some cases
      # a wind speed and wind direction of 0.0 should be treated as Missing.
      # ---------------------------------------------------------------------------
      if ( (trim($data[14]) == 0.0) && (trim($data[15]) == 0.0))
         {
         if ($debug) {print "WARNING: Found Wind Speed/Wind Direction both ZERO! At numline = $numline .\n";}
         if ($debug) {print "WARNING: RESET Wind Speed/Wind Direction to MISSING! At numline = $numline .\n";}
 
         if ($numline == 4)
            {
            print "WARNING: RESETTING Wind Speed/Wind Dir to MISSING on First Data Line! Both were zero. At numline = $numline .\n";
            $data[14] = 99999; $data[15] = 99999; # Reset to Missing. See header comments above. 

            } # If First data line
         } # If Wind Speed and Wind Direction that are both zero = missing
...
#         if ($debug) {print "Wind Speed/Wind Direction NOT missing so calcUV at numline = $numline .\n";}
      ...
      if ($numline == 4)
            ...
            $record->setAscensionRate( (trim($data[6])/60.0),"m/s"); # Else set to incoming value in raw data
            }
