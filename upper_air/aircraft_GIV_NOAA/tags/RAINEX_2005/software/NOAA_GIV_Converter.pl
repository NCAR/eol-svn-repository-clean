...
##Module------------------------------------------------------------------------
package NOAA_GIV_Converter;
use strict;
use lib "/net/work/software/RAINEX/library/conversion_modules/Version7";
use lib "/work/software/RAINEX/library/conversion_modules/Version7";
...
##------------------------------------------------------------------------------
sub main {
    my $converter = NOAA_GIV_Converter->new();
    $converter->convert();
}

##------------------------------------------------------------------------------
# @signature NOAA_GIV_Converter new()
# <p>Create a new instance of a NOAA_GIV_Converter.</p>
#
# @output $self A new NOAA_GIV_Converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
...
sub build_default_station {
    ...
    $station->setPlatformIdNumber(373);
...
sub parse_header {
    ...
    $header->setType("NOAA G-IV Dropsonde");
...
sub parse_raw_file {
	...
	my $record = Sounding::ClassRecord->new($WARN,$file,@records ? $records[-1] : undef());
