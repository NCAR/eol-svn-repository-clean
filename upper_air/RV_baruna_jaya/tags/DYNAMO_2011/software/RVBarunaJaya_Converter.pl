...
##Module------------------------------------------------------------------------
# <p>The RVBarunaJaya_Converter.pl script is used for converting high
# resolution radiosonde data from ASCII TSV format to the EOL Sounding Composite 
# (ESC) format.</p> 
#
# @author L. Cully 2012-02-13
# @version DYNAMO 2011 for R/V Baruna Jaya
#   This code was created by copying the Sagar Kanya DYNAMO 2011 s/w and making
#   the following mods to process the Baruna Jaya data. 
#    - Changed all references in code from Sagar Kanya to Baruna Jaya
#    - Note that the Sagar Kanya s/w was basically the same as the 
#      West Texas Mesonet s/w.
#    - IMPORTANT: The R/V Baruna Jaya data has an additional coln of Elevation.
...
#         mm = minute. FLEDT is the call number for the Research Vessel Baruna Jaya
#  - Note that the data in these files is slightly different than that of the
#         Sagar Kanya. In the Baruna Jaya Vaisala "Digicora 3" format, instead
...
#  - That the call/site ID is YEAU.
#  - That the radiosonde type is Vaisala RS92-SGPD.
#  - That the ground station software is Digicora III/MW31/ver3.61.
#  - That the full name of this ship platform is R/V Baruna Jaya III.
#
#  - User should Search for HARDCODED.
##Module------------------------------------------------------------------------
package RVBarunaJaya_Converter;
use strict;
...
printf "\nRVBarunaJaya_Converter.pl began on ";print scalar localtime;printf "\n";
my $debug = 0;
&main();
printf "\nRVBarunaJaya_Converter.pl ended on ";print scalar localtime;printf "\n";

my $filename;

##------------------------------------------------------------------------------
# @signature void main()
# <p>Process the R/V Baruna Jaya radiosonde data by converting it from 
# the native ASCII format into the ESC format.</p>
##------------------------------------------------------------------------------
sub main {
    my $converter = RVBarunaJaya_Converter->new();
    $converter->convert();
}

##------------------------------------------------------------------------------
# @signature RVBarunaJaya_Converter new()
# <p>Create a new instance of a RVBarunaJaya_Converter.</p>
#
# @output $self A new RVBarunaJaya_Converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
    ...
    # HARDCODED
    $self->{"PROJECT"} = "DYNAMO";
    $self->{"NETWORK"} = "RV_Baruna_Jaya_III";
...
# <p>Create a default station for the RV Baruna Jaya network using the 
...
sub buildDefaultStation {
    ...
    # platform, YEAU, RV_Baruna_Jaya - unknown
...
sub parseHeader {
    ...
    $header->setType("RV Baruna Jaya III");
      ...
      if (($index > 0) && ($index < 11))
         ...
         if ($line !~ /^\s*\/\s*$/) 
            ...
            if ($line =~ /RS-Number/i)
               {
               chomp ($line);
               chop ($line); # Chop off control M found in Baruna Jaya data
       ...
       #--------------------------------------------------------------
       # $header->setLine((6), trim("Ground Station Software").":",trim(join("/","Digicora III/MW31/ver3.61")));
       $header->setLine((6), "Ground Station Software:",trim(join("/","Digicora III/MW31/ver3.61")));
       ...
       if ($index < 40) 
          ...
          # For Baruna Jaya, the lat/lon are in the 15/16 colns.
          #--------------------------------------------------------------
          my @data = split(' ',$line);
