...
$stn_name = "Santa Cruz, Bolivia";$stn_id = "SACZ";
...
#$year_from_file = substr($file,0,4);
@metdata = ();
...
#   	}# end if
	...
	if($line =~ /\d{1,2}:\d{2}:\d{2}/){
        ...
        $outline[0] = $time[0]*60*60 + $time[1]*60 + $time[2]; #time in seconds
        $last_time = $outline[0];
	...
	if($outline[0] != 9999.0 && $outline[14] != 99999.0){
      ...
      if($outline[0] == 0.0){
        $outline[11] = -17.656; # initial latitude
        $outline[10] = -63.124; # initial longitude
      	}# end if
...
while(@rev_metdata){
	...
	@items_2 = split(' ',$rev_metdata[3]);
	if($items[9] < 0 || $items_2[9] < 0){
		shift(@rev_metdata);
	}else{
...
sub writeheader{
   ...
   printf OUT ("%s%s\n",$header[2],$site);
	$altitude=373;
   $stn_loc = sprintf("%3d %5.2f%s,%3d %5.2f%s, %7.2f, %6.2f, %7.1f",&calc_pos,$altitude);
   printf OUT ("%s%s\n",$header[3],$stn_loc);
...
sub filename {
	...
	open(OUT,">$main_directory/class_files/$outfile") || die "Can't open $outfile\n";
