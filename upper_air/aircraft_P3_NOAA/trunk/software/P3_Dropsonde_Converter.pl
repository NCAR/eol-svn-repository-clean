...
##Module----------------------------------------------------------------------------
# <p>The P3_Dropsonde_Converter converts NOAA or NRL P-3 aircraft dropsonde data 
# from the EOL format to the EOL Sounding Composite (ESC) format.</p>
#
# @author Linda Echo-Hawk 2012/03/27
# @version DYNAMO 2011 - Created for DYNAMO based on the T-PARC
#          Astra_Dropsonde_Converter.pl
#          - The converter expects raw data files to have a .Wwind extension
#          - Search for "HARD-CODED" to find project-specific information
#          - NOTE: 17 raw data files were missing launch location info, so 
#            they were pre-processed using the script ReplaceMissingLatLonHeaderData.pl
#            to create new raw data files with launch location taken from the
#            first data record.
#          - BEWARE: 58 soundings had negative zero latitude problems.  This 
#            occurs when the latitude is -0.xxx.  The perl libraries incorrectly
#            display the negative zero latitudes as "N" instead of "S".  When 
#            autoqc is run, the "N" is kept in the final directory and the 
#            minus sign is dropped so there is no way to determine that this
#            is incorrect by looking at the /final directory files.  Correcting
#            this is the /output directory does not help because autoqc changes 
#            it for the /final directory and the dayfile generator also changes
#            it.  These 58 soundings were manually corrected in the /dayfiles 
#            directory for DYNAMO NOAA P-3 soundings.  BEWARE!
#
...
#
##Module----------------------------------------------------------------------------
package P3_Dropsonde_Converter;
use strict;
...
printf "\nP3_Dropsonde_Converter began on ";print scalar localtime;printf "\n";
&main();
printf "\nP3_Dropsonde_Converter ended on ";print scalar localtime;printf "\n";    
...
##---------------------------------------------------------------------------
sub main {                        
    my $converter = P3_Dropsonde_Converter->new();
    $converter->convert();
}
...
sub readRawFile {
	...
	# The if statements below were added when it to handle the raw data files that
	# had "missing" location info.  Once these files were pre-processed, the if 
	# statements were probably not needed, but are still a good precautionary measure.
	# ---------------------------------------------------------------------------------
	if ($debug_header)
	{
	...
	if ($lat)
	{
		$header->setLatitude($lat, $self->buildLatlonFormat($lat)) unless ($lat =~ /-999/);
	}
	if ($lon)
	{
		$header->setLongitude($lon, $self->buildLatlonFormat($lon)) unless ($lon =~ /-999/);
	}
	if ($elev)
	{
		$header->setAltitude($elev, "m") unless ($elev =~ /-999/);
	}
    ...
    # QCODIAC platform number 37 = "Aircraft, NOAA P-3"
