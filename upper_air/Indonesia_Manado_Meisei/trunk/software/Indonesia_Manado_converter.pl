...
##Module------------------------------------------------------------------------
# <p>The Indonesia_Manado_converter.pl script is used for converting 
# high resolution radiosonde data to the EOL Sounding Composite (ESC) format.
#
# @author L.E. Cully October 2012
# @version DYNAMO  Created to convert Indonesia Manado Sonde (Meisei) data.
...
#          times in the Manado data and to assume that every record is 
...
#            Manado_DYNAMO_sounding_stationCD.out station file to the
#            ../final directory.
...
#           Note that Manado's local time data is -8 hrs off from UTC. LT-8=UTC.
#
#  WARNING: To see debug output, set $debug = 1.
#
#  Execute:
#
# @use     Indonesia_Manado_converter.pl >& output.log
#
##Module------------------------------------------------------------------------
package Indonesia_Manado_converter;
use strict;
...
printf "\nIndonesia_Manado_converter.pl began on ";print scalar localtime;printf "\n";
my $debug = 0;
my $debug_bad_asc = 0;

&main();
printf "\nIndonesia_Manado_converter.pl ended on ";print scalar localtime;printf "\n";
...
##------------------------------------------------------------------------------
# @signature void main()
# <p>Process the Indonesia Manado radiosonde data by converting it from 
# the native ASCII format into the ESC format.</p>
##------------------------------------------------------------------------------
sub main {
    my $converter = Indonesia_Manado_converter->new();
    $converter->convert();
} # main()

##------------------------------------------------------------------------------
# @signature Indonesia_Manado_converter new()
# <p>Create a new instance of a Indonesia_Manado_converter.</p>
#
# @output $self A new Indonesia_Manado_converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
...
sub parseHeader {
    ...
    # line: "Data Type:  Manado/Ascending"
    #-------------------------------------------------
    ...
    #-------------------------------------------------
    $header->setId("Indonesia_Manado");        #HARDCODED

    #-------------------------------------------------
    # This info goes in the 
    # "Release Site Type/Site ID:" header line
    #-------------------------------------------------
    $header->setSite("Manado, Indonesia/97014");  #HARDCODED
