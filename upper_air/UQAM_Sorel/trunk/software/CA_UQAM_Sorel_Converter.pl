...
##Module------------------------------------------------------------------------
# <p>The CA_UQAM_Sorel_Converter script is used for converting Canadian
# UQAM Sorel site radiosonde data into the EOL Sounding Composite (ESC)
# format(ESC).</p>
#
# INPUT: CA_UQAM_Sorel raw data in the following format/order. See sample
...
#
#  0.0 1017.61 -9.20 86.80 1.0 90 2022-03-01 17:00:16 073â06'38.2"W 46â01'48.8"N 13.0
#
#  OUTPUT: Sounding data files in ESC format.
#
#  Assumptions/Warnings:
#     - Search for "HARD-CODED" to change project related constants.
#     - Search for "SPECIAL CASES" to see changes required for two files that had
#          special formats.
#
...
#
##Module------------------------------------------------------------------------
package CA_UQAM_Sorel_Converter;
use strict;
...
printf "\nCA_UQAM_Sorel_Converter.pl began on ";print scalar localtime;printf "\n\n";  
&main();
printf "\nCA_UQAM_Sorel_Converter.pl ended on ";print scalar localtime;printf "\n"; 
##------------------------------------------------------------------------------
# @signature void main()
...
##------------------------------------------------------------------------------
sub main {
    my $converter = CA_UQAM_Sorel_Converter->new();
    $converter->convert();
}
...
sub parse_header {
    ...
    $header->setType("UQAM Sorel Sounding Data ");   # HARDCODED   - Get from doc for 612.001
    $header->setProject($self->{"PROJECT"});
    $header->setId("CA_UQAM_Sorel");   # HARDCODED
    $header->setSite("Sorel, QC");   # HARDCODED
  
    my $lat = 46.030;
    my $lon = -73.110;
    my $alt = 13;  # Should be in meters (not feet), get from doc 612.001
...
# Generate the sounding header.
	...
	foreach my $line (@lines) {
	    ...
	    $record->setWindDirection($data[5]) if ($data[5] != -999);
            ...
            # SPECIAL CASES
            # The files "upperair.sounding.202202030100.UQAM-Sorel.txt" and "upperair.sounding.202202030230.UQAM-Sorel.txt" have different lat/lon formats
            # The following lines are for these two files

            if (($file eq "upperair.sounding.202202030100.UQAM-Sorel.txt") or ($file eq "upperair.sounding.202202030230.UQAM-Sorel.txt") ) {
               
                print "$file \n";

                my $newlong = $data[8];
                $newlong =~ s/\D+//g;
	        $longDecDeg = (substr($newlong, 3, 2) + substr($newlong,5 ,3)/1000)/60 + substr($newlong, 0, 3);

                my $newlat = $data[9];
                $newlat =~ s/\D+//g;
                $latDecDeg = (substr($newlat, 2, 2) + substr($newlat, 4, 3)/1000)/60 + substr($newlat, 0, 2);
            }
