...
sub parseHeader {
    ...
    $header->setType("NOAA/ATDD Sounding Data");
	...
	if ($file =~ /SE_Montgomery/i)
	{
		$sfc_lon = -86.138;
		$sfc_lat = 32.363;
		$header->setSite("SE Montgomery AL");

	}
	elsif ($file =~ /Montgomery/)
	{
		$sfc_lon = -86.404;
		$sfc_lat = 32.461;
		$header->setSite("Montgomery AL");
	}
	elsif ($file =~ /Sikeston/)
	{
		$sfc_lon = -89.537;
		$sfc_lat = 36.890;
		$header->setSite("Sikeston MO");
	}
	...
	my $groundStationSoftware = "GRAW software package, version 5.10.12.3";
