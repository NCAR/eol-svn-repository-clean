...
#
# @author Linda Echo-Hawk 2011_04_22
...
# Two release locations for Barbados
my $second_location = 0;
...
sub readRawFile {
    ...
    # raw data files.  There were 2 locations for Barbados.  
    # The first 51 files were at the first location, and the
    # next 49 files were at the second location.
	# -----------------------------------------------------------
    # HARD-CODED
	if ($second_location == 0)
	{
		# check for move to second location
		if ($file_name =~ /research.howard_univ.radiosonde_20100908_18Z_barbados.txt/)
	    {
		    $second_location = 1;
	    }
	}
    ...
    foreach my $line (@lines) {
		...
		if ($line !~ /^\s*\/\s*$/)
		{
            ...
            # HARD-CODED value
            # ----------------------------------
			# Note: Barbados soundings are 4-second resolution
		    $time += 4;
...
sub build_default_station {
    ...
    $station->setStationName("Barbados");
