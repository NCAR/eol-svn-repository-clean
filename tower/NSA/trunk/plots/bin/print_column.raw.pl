...
while ( $line = <IN> ) {
  ...
  if ( $line =~ /netcdf/) {
    $line =~ s/^netcdf//g;
    $line =~ /(\w+)(gndrad|mettwr2h|mettwr4h|skyrad|met)(\w\d)\.\w+/;
    ...
    if ( ($line =~ /(\b$output_param\b)\:(\s*\-*\d+)/) ) {
      $parameter = create_parameter($param_ref->{$network}, $timestamp, $line);
      last;
    } # endif
  } # endif
...
# first get a list of parameter names
my $param_name_list = fetch_list_of_names( $param_ref->{$network} );

# finally, print out the parameter value(s)
print_parameter($time_int, $output_param, $timestamp, $parameter, $param_name_list);
...
sub print_parameter {
  ...
  # a reference to a list of parameter names
  my $param_list = shift;
  ...
  if ( !$parameter ) {
    ...
    foreach $name (@$param_list) { print "$name\n"; }
    exit();
  }
  ...
  for ($i=0; $i <= $#$timestamp; $i++) {
    ...
    if ( $min eq '00' || $min eq $time_interval) {
...
sub create_parameter {
  ...
  for ($i=0; $i <= $#$timestamp; $i++) {
    $ts = $timestamp->[$i];
    $value = $data_points[$i];
    $parameter->add_datapoint(Data::DataPoint->new($value, "", $ts, $flag));
  } # end for
...
sub fetch_list_of_names {
  ...
  my $i = 0;
  my @param_list;
  # get the name of the parameters from
  # the hash in the constants file
  foreach $key (sort(keys(%$param_ref)) ) {
    if ( !($key =~ /qc\_/) ) {
      $param_list[$i] = $key;
      $i++;
    } # endif
