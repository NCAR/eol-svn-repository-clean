...
if [ tower_diff.log ] ; then
    echo -e '\n\n\n-------------------------------' >> tower_diff.log
    echo $(date) >> tower_diff.log
    echo -e '\n\n' >> tower_diff.log
else
    echo -e '-------------------------------' > tower_diff.log
    echo $(date) >> tower_diff.log
    echo -e '\n\n' >> tower_diff.log
fi

if [ $(find . -name tower_errors.log | wc -l) -ne 0 ] ; then
    echo ''
else
    touch tower_errors.log
fi
		...
		echo
			...
			if [ $original != $altered2 ] ; then
				...
				stationName=${stationName/$misc\_/}
                                ...
                                #check if $theFile is an original file that already has a $theFile_copy.twr in this directory
                                #ie. $theFile has already been converted
				if [ $(find $dir -name $stationName\_tower.diff) ] ; then
					...
					stationFile=$rhp\_$rsi\_$misc\_$stationName\_\*\_nc.twr
				    fi
				    echo DIFFING $stationFile ...
					#echo diffing ${theFile%.twr}_copy.twr and $dir/$stationFile searching for $stationName
					diff ${theFile%.twr}_copy.twr $dir/$stationFile | grep $stationName > $dir/$stationName'_tower.diff'

					stationDiff=$dir/$stationName'_tower.diff'

					#if [ $(stat -c%s $stationDiff) -ne 0 ]; then
					    #echo Trying to solve diff problems ...
					    #altLine=$(more $dir/tower.log | grep 'Altitude difference for '$stationName)
					    ...
					    #altCurr=${altCurr%'.'}
					    #cp ${theFile%.twr}_copy.twr ${theFile%.twr}_copy_changed.twr
					    #occurences=$(grep -c $altPrev ${theFile%.twr}_copy_changed.twr)
					    #locations=$(grep -on $altPrev ${theFile%.twr}_copy_changed.twr)
					    #i=0
					    ...
					    #sed -i '$arg' ${theFile%.twr}_copy_changed.twr

					    #lines=$(grep -c $stationName ${theFile%.twr}_copy.twr)
					    #j=0
					    #touch ${theFile%.twr}_copy_changed.twr
					    #while [ $j -lt 3 ]; do
						#total=$(more ${theFile%.twr}_copy.twr | grep $stationName)
						...
						#let "j = $j + 1"
					    #done
					    #diff ${theFile%.twr}_copy_changed.twr $dir/$stationFile | grep $stationName > $dir/$stationName'_tower_changed.diff'
					    #stationDiff=$dir/$stationName'_tower_changed.diff'
					    #if [ $(getCount2 $stationDiff) -lt  $(getCount2 $dir/$stationName'_tower.diff') ]; then
						#copyFile=${theFile/$dir/}
						#copyFile=${copyFile%.twr}_copy_changed.twr
						#echo keeping ...
					    #else
						copyFile=${theFile/$dir/}
						copyFile=${copyFile%.twr}_copy.twr
						##rm $dir/$stationName'_tower_changed.diff'
						#rm ${theFile%.twr}_copy_changed.twr
						stationDiff=$dir/$stationName'_tower.diff'
					    #fi
					#fi
					...
					echo $copyFile "s original size = " $copySize >> tower_diff.log
					echo $stationName "s converted size = "$stationSize >> tower_diff.log
					echo $stationName'_tower.diffs size = ' $stationDiffSize >> tower_diff.log
					...
					#echo $(diff ${theFile%.twr}_copy.twr $dir/$stationFile | grep $stationName) >> tower_diff.log
...
echo Finished
echo -e 'Finished\n\n' >> tower_diff.log
more tower_errors.log | xargs echo -e >> tower_diff.log
rm tower_errors.log